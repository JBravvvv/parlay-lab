...
export function SportsbookSelector(){
 ...
 return <div className="mb-2 flex flex-wrap items-center gap-x-3 gap-y-2 rounded-xl border border-white/10 bg-[#111a18] px-2.5 py-1.5 sm:mb-4 sm:px-3 sm:py-2.5">
 <label className="flex items-center gap-2 text-xs font-semibold">Sportsbook<select aria-label="Sportsbook" className="min-h-9 rounded-lg border border-white/15 bg-[#18241f] px-2.5 text-[13px] text-white sm:min-h-11 sm:px-3 sm:text-sm" value={book} onChange={e=>setSportsbook(e.target.value as BookKey)}>{BOOKS.map(b=><option key={b.key} value={b.key}>{b.label}</option>)}</select></label>

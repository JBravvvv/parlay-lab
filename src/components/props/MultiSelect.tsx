...
export function MultiSelect({ label, options, value, onChange, title }: { title?: string; label: string; options: readonly { key: string; label: string }[]; value: readonly string[]; onChange: (value: string[]) => void }) {
  ...
  return <details className="relative min-w-0 flex-1 rounded-lg border border-white/10 bg-surface-2">
    ...
    <div className="absolute left-0 top-full z-30 mt-1 max-h-64 min-w-48 overflow-y-auto rounded-lg border border-white/20 bg-surface p-2 shadow-xl">
      ...
      {options.map(o => <label key={o.key} className="flex cursor-pointer items-center gap-2 py-1 text-[11px]"><input type="checkbox" checked={value.includes(o.key)} onChange={() => onChange(value.includes(o.key) ? value.filter(v => v !== o.key) : [...value, o.key])} />{o.label}</label>)}
    </div>
  </details>;
}

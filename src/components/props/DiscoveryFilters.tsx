...
export function DiscoveryFilters({value,onChange,markets,showSports=true,parlayTypes=false,hideStyles=false,stacked=false,extraControls,hideOdds=false,hideTiming=false,timeBounds}:{value:DiscoveryFilter;onChange:(v:DiscoveryFilter)=>void;markets:readonly {key:string;label:string}[];showSports?:boolean;parlayTypes?:boolean;hideStyles?:boolean;stacked?:boolean;extraControls?:ReactNode;hideOdds?:boolean;hideTiming?:boolean;timeBounds?:GameTimeWindow}) {
 const available=value.sports.length?scopedMarkets(markets,value.sports):showSports?[]:markets;
 return <div className={`discovery-filters space-y-1 px-2 py-1 ${stacked?"discovery-stacked":""} ${extraControls?"discovery-board-controls":""}`}><div className="filter-toolbar-title">Customize your picks <span>Open a filter to choose</span></div><div className="discovery-layout"><div className="discovery-fields flex flex-wrap gap-1">
 ...
 </div><div className="discovery-secondary">{extraControls}{!hideOdds&&<OddsRangeFilter value={value.odds} onChange={odds=>onChange({...value,odds})}/>}<GameTimeRange value={value.timeWindow} bounds={timeBounds} onChange={timeWindow=>onChange({...value,timeWindow})}/></div></div></div>;
}

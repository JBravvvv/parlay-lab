...
export function GameSuggestedPicks({game}:{game:CfbGame}) {
 const L=useLeague();
 const markets=CFB_PROP_MARKETS.map(m=>({key:m.id,label:m.label}));
 const [filter,setFilter]=useState<DiscoveryFilter>({sports:[L.id],timing:["pregame","live"],markets:defaultMarkets(markets,[L.id]),strategies:STRATEGIES.map(s=>s.key),timeWindow:[0,24]});
 ...
 const rows=(props?.rows??[]).filter(r=>r.gameId===game.id && r.cz && r.fair!=null && discoveryMatches({market:r.market,am:r.cz.price,prob:r.fair*100,ev:r.evCz??-Infinity,sport:L.id,start:game.start,started:game.status==="live"},filter) && footballQuoteCurrent(r,props?.pricedAt,Date.now(),L.props.liveRevalidateSec*1000)).sort((a,b)=>(b.evCz??-Infinity)-(a.evCz??-Infinity));
 return <section className="mt-3 rounded-xl border border-indigo-300/30 p-3"><h3 className="font-bold text-indigo-200">Top picks by category</h3><DiscoveryFilters showSports={false} hideStyles markets={markets} value={filter} onChange={setFilter} timeBounds={slateTimeBounds([game.start])}/><div className="mt-2 max-h-72 overflow-y-auto overscroll-contain">{q.isPending ? <p>Loading posted props…</p> : q.isError ? <p>Player props unavailable. Game sides remain in the Model below.</p> : !rows.length ? <p>No current priced player props for this game.</p> : CFB_PROP_MARKETS.map(m=>{const picks=rows.filter(r=>r.market===m.id).slice(0,5);return picks.length ? <div key={m.id} className="mb-3"><h4 className="text-xs font-bold text-violet-300">{m.label}</h4>{picks.map(r=><div key={r.key} className="flex justify-between gap-2 border-b border-white/10 py-2 text-xs"><span>{r.label}</span><span className="shrink-0 text-amber-200">{fmtAmerican(r.cz!.price)} · {r.grade??"—"} · {(r.fair!*100).toFixed(1)}%</span></div>)}</div>:null;})}</div></section>;
}

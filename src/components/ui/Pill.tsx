...
export function Pill({
  ...
  return (
    <button
      className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[12px] font-semibold sm:px-3.5 sm:py-1.5 sm:text-[12.5px] transition-[transform,background,filter,border-color] duration-(--dur-fast) active:scale-[0.96] disabled:opacity-40 ${VARIANTS[variant]} ${className}`}
      {...rest}
    >
      {children}
    </button>

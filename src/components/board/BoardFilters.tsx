...
export function BoardFilters(props: ComponentProps<typeof DiscoveryFilters>) {
  ...
  const changed = [
    ...
    value.markets.length !== markets.length,
  ...
  return <details className="board-filters">
    <summary><span className="board-filter-icon" aria-hidden>☷</span><span><strong>Customize picks</strong><small>{timing} · {hours}</small></span><b>{changed ? `${changed} active` : "Filters"}</b><span className="board-filter-chevron" aria-hidden>⌄</span></summary>

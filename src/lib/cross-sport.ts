...
export type CrossLeg={sport:DiscoverySport;id:string;gameId:string;label:string;sub:string;game:string;cz:number;prob:number;push?:number;book:string;market:string;player?:string;headshot?:string|null;logo?:string|null;position?:string|null;start?:string|null;src?:"model"|"market"};
...
export function crossSportLegs(data:CrossData,book:string,now:number,live?:LiveNowRead):GenLeg<CrossLeg>[] {
 ...
 out.push({id,sport:"mlb",am,dec:amToDec(am),prob,ev:prob/100*amToDec(am)-1,src:"model",side:"o",label,sub,market,gameKey:`mlb:${row.gkey}`,playerKey:`mlb:side:${row.gkey}`,team:team?`mlb:${teamTag(team)}`:null,started:false,alt:false,book:tag(book),start,gameLabel:game,leg:{id,sport:"mlb",gameId:String(row.gkey),label,sub,game,cz:am,prob,book:tag(book),market,start,src:"model",logo:team?teamLogo(team):null}});
 }
 }
 ...
 if(b){const rows=b.rows.map(r=>priceFootballProp(r,book));const p=footballGenPool<CrossLeg>(rows,{market:"anytime_td",markets:FOOTBALL_GEN_MARKETS.map(m=>m.key),includeStarted:true,phase:"mixed"},{mode:"cz",nowMs:now,pricedAt:b.pricedAt,teamOf:r=>footballTeamKey(r,games.get(r.gameId)),quoteOf:r=>r.cz,legOf:(r,q)=>r.fair!=null&&q.line===r.line?{sport,id:`${sport}:${r.key}`,gameId:r.gameId,label:r.player,sub:r.label,game:r.sub,cz:q.price,prob:r.fair*100,book:tag(q.book),market:r.market,player:r.player,headshot:r.headshot,position:r.pos,start:r.kickoff,src:"market",logo:(r.teamId===games.get(r.gameId)?.home.id?games.get(r.gameId)?.home.logo:r.teamId===games.get(r.gameId)?.away.id?games.get(r.gameId)?.away.logo:null)??null}:null});

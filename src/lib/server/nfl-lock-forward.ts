...
export async function forwardNflLock(origin: string, secret: string, fetchImpl: FetchLike = (u, i) => fetch(u, i), slot?: string | null): Promise<NflForwardResult> {
  try {
    const url = new URL("/api/nfl/lock", origin);

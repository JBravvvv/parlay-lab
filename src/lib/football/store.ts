...
/**
 * THE FOOTBALL DEVICE STORE FACTORY (2026-09-08, the NFL build; Josh, verbatim: "NFL needs to be
 * built NOW"). This is the body of the College Football device store (INSTRUCTION 38, 2026-09-05,
 * Josh: "Ledger & Allotted $ for College Football should be separate") lifted into
 * `makeDeviceStore(cfg)`, so the same rails — append-only by date, a lock once per date, grades
 * overlaid through ONE rule, sync merges through the shared kernel — run a second desk under its
 * OWN two localStorage keys, its OWN window events, its OWN sport stamp and its OWN bank base.
 *
 * Every key, event name, sport literal and dollar figure comes off `cfg`: this file names no desk.
 * Each instance owns its own `version` counter, snapshot cache and `subscribe`, so two desks on
 * one page never share state — a write on one leaves the other's snapshot untouched, and a
 * subscriber hears only its own desk's events. The pure helpers above the factory (the grading
 * overlay, the stake-move detector, the upsert rail) take every input as an argument and are
 * shared by reference.
 *
 * Every storage access is guarded, so the helpers import cleanly on the server and under
 ...
 *
 * `useLedger()` is a useSyncExternalStore hook with an EMPTY server snapshot: the first client
 * render matches the SSR HTML, then React swaps in the device's record (the mount gate).
 * Nothing here computes a price or a grade — locking and grading are the pure functions in
 * src/lib/cfb/ledger and src/lib/cfb/grade (shared by both leagues, taking `cfg`); this module
 * only stores what they return.
 *
 * The CFB instance is src/lib/cfb/store.ts (every one of today's named exports, unchanged); the
 * NFL instance is src/lib/nfl/store.ts.
 */
...
/** stable action identities `useLedger()` spreads over the snapshot — safe in effect dependency arrays */
export type DeviceLedgerActions = {
  addAdjustment: (kind: "deposit" | "withdrawal", amt: number, note: string) => BankStore;
  lock: (card: CfbCard, board: CfbBoard) => { entry: CfbLedgerEntry; refused: boolean };
  grade: (date: string, finals: CfbFinals) => CfbGrading | null;
  importText: (text: string) => CfbImportResult;
  exportText: () => string;
  wipe: () => void;
};

/**
 * What a desk's store reads off its league config. The store ITSELF touches only `id` (the sport
 * stamp), `keys`, `events`, `bankBase` and `paper.since` — but `lock` and `grade` hand the config
 * on to the shared money seams (`lockCfbCard` / `gradeCfbEntry`, which stamp the day and time the
 * void window off it), and those take the WHOLE league, so a Pick here would have needed an
 * unsafe cast at the one seam that mints money. The whole config it is; both callers spread it.
 */
export type DeviceStoreConfig = LeagueConfig;

/**
 * One desk's device store. Written out field by field rather than as `DeskStore & {…}` on purpose:
 * `DeskStore` (src/lib/football/league.ts) is typed off the CFB instance's exports, and the CFB
 * instance is built FROM this factory, so a `DeskStore &` here would make each CFB export's type
 * depend on itself. The assignability check at the bottom of this file keeps the two in step.
 */
export type DeviceStore = {
  KEYS: { ledger: string; bank: string };
  CHANGE_EVENT: string;
  SYNC_EVENT: string;
  readRaw: () => { ledger: string; bank: string };
  isEntry: (x: unknown) => x is CfbLedgerEntry;
  entriesOf: (x: unknown) => CfbLedgerEntry[];
  readLedger: () => CfbLedgerEntry[];
  writeLedger: (entries: CfbLedgerEntry[]) => boolean;
  findEntry: (date: string) => CfbLedgerEntry | null;
  upsertEntries: typeof upsertEntries;
  upsertEntry: (entry: CfbLedgerEntry) => { entries: CfbLedgerEntry[]; entry: CfbLedgerEntry; refused: boolean };
  applyGrading: (date: string, grading: CfbGrading) => CfbLedgerEntry | null;
  readBankStore: () => BankStore | null;
  writeBankStore: (store: BankStore) => boolean;
  getBankStore: () => BankStore;
  addBankAdjustment: DeviceLedgerActions["addAdjustment"];
  getBankroll: () => number;
  exposure: (date: string) => number;
  exportLedger: () => string;
  importLedger: (text: string) => CfbImportResult;
  wipeDevice: () => void;
  lock: DeviceLedgerActions["lock"];
  grade: DeviceLedgerActions["grade"];
  useLedger: () => CfbLedgerSnapshot & DeviceLedgerActions;
};

/* ---------- guarded storage (shared by every instance) ---------- */
 ...
 *     places a CFB day is merged with another copy of itself: THIS rail (`upsertEntries`,
 *     reached through `lock`) and the SYNC rail (`mergeLedgers` -> `mergeDay` -> `unionCore`,
 ...
 * 2026-09-06, the closing round's CORE-RAIL defect — (B4) below `upsertEntries`'s own block). The ids `cur` and the merged day SHARE whose stake is not the same
 * number on both — nothing else. Appended ids are not here (there was no stored verdict to
 * invalidate; the REOPEN block in `upsertEntries` covers them), and neither are ids that
 ...
 * in `upsertEntries` cannot catch it either: that fires on a ticket MISSING from the map, and a
 ...
 * should've been running the same $150 per day theoretical Core money and $25 Fun money per day".
 *
 * The union above fixed CORE. `upsertEntries` builds its result as a spread of `cur`, so every
 ...
 *      same core plus funT [`cfb-2026-09-05-topup1-fun-1` @ $25]: `upsertEntries` returned funT
 ...
 *      same way either way), while `upsertEntries(cur=a, inc=b)` kept `fun-1` and named
 ...
 *
 *      HONEST LATENCY NOTE. This branch is NOT reachable from production today: `lock` below
 ...
 *      `upsertEntries([a], b).entry.core` came back as 6 tickets / $150 (the union's allotment
 ...
 *      while `upsertEntries([b], a)` produced the same 7 tickets / $175 with `capBreach`
 ...
 * ONE field of the merged day and copied it across by hand. `upsertEntries` now starts from the
 ...
 * `upsertEntries(...).entry.X` against `mergeLedgers(...)[0].X` for funT ids, fun sum,
 ...
 *
 * `upsertEntries` built its result as `{ ...cur, core, funT, games, grading }` and then copied
 ...
 *      `upsertEntries` returned `undefined`. The phone names a drop and cannot say what it was
 ...
 * than assumed: `importLedger` below writes `entriesOf(mergeLedgers(local, v.entries))` — the
 * merged day WHOLE, with nothing taken back — and `syncCfbNow` (src/lib/cfb/sync.ts) writes
 * `entriesOf(mergeLedgers(local, remote))`, likewise. Two of the three paths that can rewrite a
 ...
 * `sport`, `date` and `locked` are identity: `upsertEntries` only reaches this path for an
 * entry whose `date` already matches, and `isEntry` fixes the other two, so they are the same
...
export function upsertEntries(
  ...
  /* THE FUN BUCKET IS THE SYNC RAIL'S OWN ANSWER — AND SINCE THE CLOSING ROUND SO IS THE CORE
     ...
     `CfbLedgerEntry` declares `locked: true` (./types), which `isEntry` above also enforces on
  ...
  /* RECONCILE THE TAKEBACK (INSTRUCTION 45, 2026-09-06, the closing round's defect C1 — the DEVICE
     ...
     `upsertEntries([cur@daily 150], inc@daily 500)` with cur holding 3 core tickets at $25 and
     ...
     Nothing downstream re-guards either shape: `writeLedger` above runs no money guard, and
...
export function makeDeviceStore(cfg: DeviceStoreConfig): DeviceStore {
  const KEYS = { ledger: cfg.keys.ledger, bank: cfg.keys.bank };
  const CHANGE_EVENT = cfg.events.change;
  /** dispatched by the desk's sync loop after a cloud merge rewrote the device record */
  const SYNC_EVENT = cfg.events.sync;

  /** The un-persisted bank: the league's base with P/L counted from its paper start. The record
      cannot hold a day before `paper.since`, so this window is exact — a device that first opens
      the desk a week in still counts every graded day (mergeBankStores keeps the EARLIER asOf
      anyway, so every synced device converges here). */
  const DEFAULT_BANK: BankStore = { base: cfg.bankBase, asOf: cfg.paper.since, log: [] };

  /** The raw stored strings — ./sync's change detector compares these between ticks. */
  function readRaw(): { ledger: string; bank: string } {
    return { ledger: rawItem(KEYS.ledger), bank: rawItem(KEYS.bank) };
  }
  ...
  function isEntry(x: unknown): x is CfbLedgerEntry {
    if (!x || typeof x !== "object" || Array.isArray(x)) return false;
    const e = x as Partial<CfbLedgerEntry>;
    return e.sport === cfg.id && typeof e.date === "string" && DATE_RE.test(e.date) && e.locked === true && Array.isArray(e.core);
  }


  /** Only CFB entries, funT defaulted, ascending by date. Anything else on the wire is dropped. */
  function entriesOf(x: unknown): CfbLedgerEntry[] {
    if (!Array.isArray(x)) return [];
    return sortByDate(x.filter(isEntry).map((e) => (Array.isArray(e.funT) ? e : { ...e, funT: [] })));
  }

  function readLedger(): CfbLedgerEntry[] {
    const raw = rawItem(KEYS.ledger);
    if (!raw) return [];
    try {
      return entriesOf(JSON.parse(raw));
    } catch {
      return [];
    }
  }
  ...
  function writeLedger(entries: CfbLedgerEntry[]): boolean {
    const ok = setRaw(KEYS.ledger, JSON.stringify(entriesOf(entries)));
    bump();
    return ok;
  }

  function findEntry(date: string): CfbLedgerEntry | null {
    return readLedger().find((e) => e.date === date) ?? null;
  }

  function upsertEntry(entry: CfbLedgerEntry): { entries: CfbLedgerEntry[]; entry: CfbLedgerEntry; refused: boolean } {
    const r = upsertEntries(readLedger(), entry);
    writeLedger(r.entries);
    return r;
  }
   ...
   * `grade` below reaches the record through HERE, not through either overlay, so the gate was
   ...
   * least one ticket in either bucket — quoted from that file this turn — and calls `grade` on
   * each; it is the ONLY caller of `grade` outside this module, and nothing destructures `grade`
   * off `useLedger()`. A two-leg
   ...
   * thinner one.
   */
  function applyGrading(date: string, grading: CfbGrading): CfbLedgerEntry | null {
  ...
  function readBankStore(): BankStore | null {
    const raw = rawItem(KEYS.bank);
    if (!raw) return null;
    try {
  ...
  function writeBankStore(store: BankStore): boolean {
    const ok = setRaw(KEYS.bank, JSON.stringify(store));
    bump();
    return ok;
  }

  /** The persisted bank store, initialized on first use. */
  function getBankStore(): BankStore {
    const cur = readBankStore();
    if (cur) return cur;
    const store: BankStore = { ...DEFAULT_BANK, log: [] };
    writeBankStore(store);
    return store;
  }

  function addBankAdjustment(kind: "deposit" | "withdrawal", amt: number, note: string): BankStore {
  ...
  function getBankroll(): number {
    return cfbBankroll(readBankStore() ?? DEFAULT_BANK, readLedger());
  }

  /** Locked exposure (CORE + FUN stakes) for a slate date, in dollars. */
  function exposure(date: string): number {
    return todayExposure(readLedger(), date);
  }

  /* ---------- export / import / wipe ---------- */

  function exportLedger(): string {
    return JSON.stringify(readLedger(), null, 2);
  }
  ...
  function importLedger(text: string): CfbImportResult {
    let parsed: unknown;
    try {
    ...
    const foreign = v.entries.find((e) => e.sport !== cfg.id);
    if (foreign) return { ok: false, error: `entry ${foreign.date} is not a ${cfg.id} entry (sport must be "${cfg.id}")` };
    const local = readLedger();
    const have = new Set(local.map((e) => e.date));
    const merged = entriesOf(mergeLedgers(local, v.entries));
    const added = merged.filter((e) => !have.has(e.date)).length;
    const overlapped = v.entries.filter((e) => have.has(e.date)).length;
    if (!writeLedger(merged)) return { ok: false, error: "device storage unavailable" };
   ...
   * `applyGrading` now overlays rather than replaces, so the grader's own output and the record
   ...
   */
  function grade(date: string, finals: CfbFinals): CfbGrading | null {
    const entry = findEntry(date);
    if (!entry) return null;
    return applyGrading(date, gradeCfbEntry(entry, finals, Date.now(), cfg))?.grading ?? null;
  }
  ...
  function snapshot(): CfbLedgerSnapshot {
    ...
    cached = compute(readLedger(), readBankStore() ?? DEFAULT_BANK);
  ...
  function subscribe(cb: () => void): () => void {
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === null || e.key === KEYS.ledger || e.key === KEYS.bank) onExternal();
    };
  ...
  const ACTIONS = {
    addAdjustment: addBankAdjustment,
    lock,
    grade,
    importText: importLedger,
    exportText: exportLedger,
    wipe: wipeDevice,
  } as const;

  /** Everything the desk's Ledger / Builder / Settings need, live from the device record. Empty on
      the server and during hydration; the real record appears right after mount. */
  function useLedger(): CfbLedgerSnapshot & DeviceLedgerActions {
...
/* DeviceStore ⊇ DeskStore (src/lib/football/league.ts) — checked here so a handle name drifting
   on either side is a type error in this file rather than in a desk's `desk.ts`. */
type AssertDeskStore<T extends DeskStore> = T;
export type DeviceStoreIsDeskStore = AssertDeskStore<DeviceStore>;

...
- Route `app/api/cfb/props/route.ts` (`?date&bankroll`): builds the slate through the shared helpers in `src/lib/cfb/slate-server.ts`, picks the games that have an odds event and a Caesars side price and are either upcoming (kickoff still ahead) or LIVE — never final / postponed (`selectPropEvents`, live first, then kickoff, then rank; changed 2026-09-05 under INSTRUCTION 40 so in-game prop lines keep populating like the MLB side), then pulls each event's props with the server key. Prod's `/api/odds?u=` proxy refuses player-prop market shapes (403 "market shape not allowed"), which is why this route fetches itself.
...
- Review fixes (2026-09-05, after INSTRUCTION 40): (1) the stored board is fresh only inside min(its own `ttlSec`, the CURRENT slate's window) — `boardFresh` in `props-store.ts` — so a pre-kick board (7200) stops being honoured 600 s after a game inside it kicks off; the first request after kickoff re-prices. (2) A live pull re-prices ONLY what moved: the in-play events (at most `CFB_PROPS.liveMaxEvents` = 6, live first) plus any upcoming game the stored board has no rows for; every other upcoming game's rows are carried over from the stored board while it is inside `revalidateSec` (the board names its games in `priced`). A full 12-event re-price every 10 min would have spent the 1200-credit day in four pulls. (3) The board is written with `EX CFB_PROPS.boardRetainSec` (36 h) — retained past its window — and when the daily budget refuses a pull the route serves the carried rows flagged `stale: true` with the original `generatedAt` (source `redis`, note "showing the last priced lines") instead of `rows: []`; the Builder and Board footnotes print "lines as priced at H:MM — today's props budget is used up". (4) The client `staleTime` is what is LEFT of the window (`cfbPropsStaleMs` in `client.ts`: `ttlSec` less the board's age), not the whole window from the fetch; `cfbCacheLabel` is shared so the Builder and the Board footnotes can never disagree.
...
- `CFB_PROPS` (`src/lib/cfb/rules.ts`): `maxEvents` 12, `revalidateSec` 7200, `liveRevalidateSec` 600 (2026-09-05, the window while any priced game is live), `dailyBudget` 1200, `measuredCreditsPerEvent` 31. `src/lib/cfb/props-store.ts` wraps the shared Redis client: the priced board is stored at `pl:cfb:props:v1:<date>` for `boardRetainSec` (36 h — retained past its window so the budget-out answer can serve it stale; freshness is `boardFresh`, capped by the current slate's window), the day's spend at `pl:cfb:props:spend:v1:<ptDate>` (36 h). Order in the route: Redis read (fresh → answer) → what needs pricing (live events ≤ `liveMaxEvents` + upcoming games not carried) → budget gate (`allowed = floor((dailyBudget − spent) / 31)`) → fetch the allowed events → merge with the carried rows → tally the real `x-requests-used` delta → write back; nothing affordable → the stored rows, `stale: true`. The body reports `source` (`redis` / `fetch` / `none`), `budgeted`, `stale`, `priced` and `spentToday`; without the store env the route runs on the Next data cache alone and `spentToday` is `null`. Worst case without the budget: 12 × 31 × 12 pulls a day = 4,464 credits; with it, 1,200. The footnotes ("cached 2 h", "today's props budget is used up") derive the hours from the constant.

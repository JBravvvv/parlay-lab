...
Validation in progress: populated isolated browser preview passed 375px, 390px and 1280px overflow checks; Top Edges 10 → 20 → Undo 10 → Clear 10; Games metadata remains in the overlay; card prices/stakes/payouts remain visible; Ledger metrics fit in about 165px and the chart starts collapsed. A simulated 59px top inset leaves the logo at y=78.5px with an opaque header at y=0. This is a layout simulation, not a physical iPhone test. No production ledger writes or paid refreshes were performed. Fresh GitHub context-only commits are preserved. Full verification results and exact production SHA follow in the release receipt.

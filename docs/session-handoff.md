...
- **Speed root cause.** `gameHour()` built a new `Intl.DateTimeFormat` per call. `strategyGenerate` runs `generate` 32 (model: 64) times per render, each walking every leg through `inGameTimeWindow`. So any narrowed window cost about 15 s per render on desktop, and 45–99 s per slider tick on production at 4× CPU. Every tick also committed straight into the spec. The fix caches the formatter plus a bounded hour cache (`src/lib/game-time-window.ts`).
- **Seed-independent setup hoisted.** `generate` is split into `prepare(pool, spec)` (rule checks, eligibility, band, capacity, payout reach) plus the seeded walk. `strategyGenerate` hands its runs one `GenMemo`, so the setup runs once. The EV order, mix tiers, spread targets and price order are also reused per ctx. A 300-case seeded fingerprint (25 spec shapes × 12 seeds, with avoid/recent) is byte-identical before and after. On the 6,030-leg bench pool: all strategies 102 → 13 ms, model 235 → 16 ms, single strategy 150–470 → 62–167 ms (the rest is `mixOrder`'s seeded shuffle, which must keep its RNG order).
- **Slider** (`GameTimeRange`). It drags a local draft, and native `change` (release or key step) and blur commit once inside `startTransition`. Step is 30 min. `slateTimeBounds` sets the track floor at 9am PT, or at the half hour of an earlier game when the slate carries one. Both thumbs at the ends emit the `ALL_DAY` [0,24] sentinel, so start-less rows are never dropped. Capped at max-w-[15rem]. `timeBounds` is wired at every call site (ranked list, generator, MLB board/builder/parlays, CFB/NFL board, builder and game picks).
- **Customize Your Picks.** The left-to-right category rail is removed (RankedPicks, the `DiscoveryFilters` `categoryRail` prop, the `.ranked-market-rail` CSS). The Markets dropdown now drives the same filter, and MLB's controlled filter stays in sync through `pick()`.
- **Season Lab** is off the NAV table, so it is gone from the desktop rail and the phone ⋯ More menu. `/season` still loads by URL, and the page and `IconSeason` are untouched.
- **Reveal** (`src/components/props/ParlayReveal.tsx`). On Generate/Regenerate every unlocked slot becomes a reel spinning through up to 48 REAL legs from the current pool. The reels land top to bottom (560 ms + 170 ms per slot) with a bump/glow and an 8 ms vibrate where offered. The combined odds show "···", then count up in decimal to the ticket's own `calc.am`, and the total flashes. Frames are written from requestAnimationFrame with no React render per frame. Tap a reel to skip. Pinned slots never spin. Reduced motion gets the ticket at once. The ticket is fully computed before the first frame.
- **Validation.** tsc PASS. `next build` PASS with no warnings. Full suite 3,732 tests: 3,630 pass, 102 fail = the same 102 baseline failures by name (two nav titles were renamed; they still fail for their pre-existing "The Card" label and `bg-bg/92` reasons). There are zero new failures. New: `tests/builder-speed-reveal.test.ts` (memo equivalence over 30 seeded cases, slider/rail/reveal pins, OddsTicker SSR). Updated: game-time-window (half hours, bounds, ALL_DAY, cache), nav, cfb-season-ui, mobile-density comment, parlay-gen-ui (combined-odds pin follows OddsTicker).
- **Production, DOM-only headless Chrome at 4× CPU on /props** (no screenshots):
  - Slider ticks take 6–27 ms (were 45–99 s).
  - A Generate press paints in about 190 ms with 4 reels, odds "···" and aria-busy set. It settles to +1332 with no reels.
  - Both tracks run 9–24 at step 0.5, 209/240 px wide.
  - There is no /season link and no rail.

# September 26 — Games team pages and live coverage

Games cards now open their pop-out from the header, score, logo, odds, missing-line cells or whitespace. Team-name buttons separately open a team page, including final-game cards and first-half names inside previews. Delegated click handling excludes explicit controls and portalled content so closing a sheet cannot reopen the underlying card; sandbox pick buttons keep their existing actions.

Team pages cover NFL, CFB and MLB with Schedule, Roster and Stats. Results, upcoming games and full-season views come from public ESPN/MLB feeds. A schedule score opens its game inside the same sheet; the sticky Back to team schedule action preserves schedule filters, scroll and keyboard focus. The normal sheet close returns to the original Games position. Rosters include search and provider portraits with logo fallback; football stats include opponent comparisons. Partial feeds and refresh failures are disclosed without inventing values.

Live/final football cards now open actual ESPN game coverage instead of pregame model details: quarter scores, passing/rushing/receiving, defense/special teams, team comparisons, scoring summaries and newest-first play-by-play with current-drive handling. MLB retains its preview/box score and adds play-by-play, scoring filters and pitch sequences. Live coverage checks every 30 seconds; the last received data remains visible after a refresh failure. New routes are read-only and use public providers, with numeric ID/sport checks, timeouts and no odds API calls.

Validation: 181 focused checks PASS across 10 suites, including real NFL/CFB/MLB provider fixtures, team normalization, existing game/preview behavior, portal/control tap boundaries, missing-line targets and failed live refreshes. An additional first-half suite had 22 passes and 3 existing failures in unchanged market-count/ticket-source assertions (not changed in this task). Prior full-suite baseline remains documented below; no new full-suite pass claim. Local iPhone-size browser at 390px verified Falcons schedule/roster/stats, Sep 24 ATL @ GB box score and play-by-play, Back to schedule, close and a logo tap opening game details; one dialog, document width 390/viewport 390. Temporary review page removed. Local preview compilation was slow; one build encountered a leftover generated preview type reference, then generated .next/types was removed before the final build. Final build, deployment and additional production checks are recorded in the matching external release-verification.json. Physical iPhone standalone not directly tested. No pricing, allocation or ledger changes.

# September 26 — scoped markets, fun-ticket floor, table paging and smaller iPhone content

Owner's latest six-point correction implemented. Football defaults are exactly six sides (ML/spread/total, full-game and 1H), Anytime TD 1+, passing yards/TDs O/U, receiving yards O/U and rushing yards O/U. Receptions, alternates, First TD and 2+/3+ TD ladders remain opt-in. Shared discovery menus restrict categories to selected sports; NFL + CFB does not expose MLB, explicitly adding MLB does. Sport changes preserve still-valid selections and add only the new sport's defaults.

Board's SMALL graded tables now show ten globally sorted rows with Load 10 More, Undo and Clear. Search, category, odds and other filter changes reset pagination; Clear only resets loaded rows. Top Edges is a compact four-card phone preview (ten on desktop) with no pagination. Category/per-pick odds controls cover Board (including cross-sport/live), Generated Parlays, ranked lists, football By game, football game suggestions and The Card. Card filters change visibility, never allocation or recorded tickets. Shared filters explicitly label categories and odds.

Phone main content uses CSS zoom 0.7 below 768px; the opaque fixed header and bottom navigation stay full-sized and desktop stays at zoom 1. Generator action row is static on phones so the scaled sticky offset cannot hide ticket content. Two-column Top Edges cards stay inside 375/390px bounds. Native pinch zoom remains available.

Football fun-ticket search now considers other qualifying sides across distinct games rather than the five shortest ML favorites. Requires the existing configured 3–5 legs and decimal 4–40 (+300 to +3900) payout band; no -1600 fallback. Uses bounded candidate search, not a guaranteed globally optimal combination. No qualifying ticket leaves the fun allocation unspent. Existing locked tickets/ledger records are unchanged.

Validation: production build including TypeScript PASS; 360/360 focused checks across 12 suites PASS. Additional UI/generator checks: 23 legacy failures also reproduced on clean a31c2dd (QueryClient setup and outdated UI expectations); the only new mismatch was a helper caption below the previous 11px source floor and was corrected/rechecked. No full-suite rerun is claimed; September 25 full-suite baseline had 107 failures. Populated isolated component preview verified at 375/390px, with a simulated 59px top inset; desktop 1440px zoom remains 1. Small table 10→20→30→Undo20→Clear10 verified; odds filtering and explicit MLB opt-in verified; generator shows exactly 11 default categories with no baseball. Preview route was removed before production build. Physical iPhone standalone rendering still requires owner confirmation. No ledger writes, locks or manual paid refreshes used for QA. Exact deployment and archive receipt follows in the external release-verification.json.

# September 25 — compact, colorful mobile picks

Games retain the three main prices in shorter cards and open their additional context in Game details. Top Edges retains the full eligible list and pages 10 at a time with Undo/Clear. Generator controls use compact paired columns, with Games/Positions/Advanced grouped into small disclosures and Regenerate accessible while scrolling. Board parlay filters combine category, tier and timing alongside the main filters. Every Pick Today has a small category rail immediately below Customize your picks. Its search stays visible in ranked mode and now filters the complete local/cross-sport list, including accent-insensitive names and matchup text. Picks use colored rims, opaque tinted surfaces and distinct odds controls; selected prices remain visibly selected. The Card restores full tickets and gives singles one clear player/pick identity, odds and payout. Ledger metrics and day cards are tighter, with expandable charts. The fixed phone header paints a solid safe-area background; its content and spacer include the device inset.

Live follow-through: production browser confirmed 131px Games cards, 129.5px Card singles, 274px expanded generator settings and Top Edges 10 → 20 → Undo 10 → Clear 10. An older unlayered chip-row CSS rule overrode the ranked view hidden utility; the obsolete outer rail now mounts only in By game, leaving the category rail solely below Customize your picks. Follow-up build and TypeScript PASS; 85/87 affected tests pass, with the same two baseline cfb-props-ui failures (old market-label and polling expectations). Full suite was run before this small conditional-render fix; no full rerun is claimed after it.

Validation: production build and TypeScript PASS. Final full serial suite: 3,559 passed / 107 failed / 3,666 total across 241 files. Baseline at the upstream context tip: 3,552 passed / 112 failed / 3,664 total; comparison by test name found ZERO new failures. The remaining failures are pre-existing UI expectations, test setup and operational checks; the full suite is not green. Focused release checks: 78/78 PASS. Populated isolated browser preview passed 375px, 390px and 1280px overflow checks; Top Edges 10 → 20 → Undo 10 → Clear 10; Games metadata remains in the overlay; card prices/stakes/payouts remain visible; Ledger metrics fit in about 165px and the chart starts collapsed. A simulated 59px top inset leaves the logo at y=78.5px with an opaque header at y=0. This is a layout simulation, not a physical iPhone test. No production ledger writes or paid refreshes were performed. Fresh GitHub context-only commits are preserved. Full verification results and exact production SHA follow in the release receipt.

# September 23 evening — Board navigation and layout redesign

Owner rejected the crowded side-by-side filter/stat layout and bottom Board chooser. Replacing with a persistent Board / Generated Parlays top toggle, direct bottom Board navigation, slim phone counts, always-visible category and expandable additional filters. Query-based view switching keeps the same Board mounted so its selected date and filters survive. Legacy /board/parlays redirects to the new view. Desktop retains full summary tiles. Pricing, allocation, ranking and settlement are unchanged. Validation: production build and TypeScript pass. 125/128 tests pass across 7 suites; the three failures in nfl-pages assert the obsolete landing hero (HEAD already redirects /games), unrelated to this patch. Local 390x844 Board and expanded filters fit without overflow; view round trip preserves search, bottom Board returns the default active Board view. Desktop 1280x800 also fits. Final production SHA and archive verification are recorded in external release-verification.json.

# September 23 — iPhone-first release

Implemented locally: crisp opaque phone header/visible sport and icon controls; mobile order Games / Board / Parlay Builder / The Card / Stats / Ledger; Board chooser and /board/parlays route for all sports; grouped Sides/Props/Alt Props with base-only default; Top Edges responds to current category/discovery; contained carousel margins; generated football tickets use complete stacked slips; main Games hides 1H and opens shared modal, with current prop suggestions; MLB preview/boxscore uses same reusable detail body in overlay; 10-row pagination with Load 10 More/Undo/Clear; Ledger experiment bottom collapsed. Alternate rush/receiving yards now use actual additional market keys. Verified ESPN lists Zonovan Knight as Bam Knight, ID 4427728; image alias links both.

New NFL paper policy nfl-variety-v2 from 2026-09-23: future dates fully allocate $350 across available distinct games; reserves up to 3 supported fresh O/U props, remaining slots compete on EV. Ranked probability-weighted whole-dollar stakes with $50 cap except thin-slate full-budget exception. No fabricated odds. Props limited to half-point passing yards/TDs, rushing yards, receptions and receiving yards, verified matchup team, DK quote, per-game age <=30 minutes. Player forecasts remain market-consensus estimates. Client queries props separately to avoid inflating shared Games payloads; server lock forces fresh props through the existing handler before allocating. Official ESPN final summary stats settle props on both client finals and server grading paths; missing athletes/cells stay pending, never zero. Existing void policy retained and original historical tickets untouched. Historical v1 tickets remain unchanged; new policy applies to future NFL slate dates from September 23.

Verification: TypeScript and final production build PASS. 488 affected tests across 15 suites PASS, covering allocation/settlement, lock routes, props, Board and generator. Browser 390x844 verified crisp header, nav/Board chooser, dedicated Generated Parlays route, grouped category selection, side-by-side controls/summary, and complete MLB preview overlay. Fixed-body modal scroll lock restores the exact 667px prior position after dismissal. Local odds credentials are absent, so populated production visual review remains pending. Landscape 844x390 has no document overflow; modal dismissal confirmed complete. Deployment and archive SHA receipt will be recorded in external release-verification.json. No real-money actions or ledger locks performed.

# September 22 UI release — production verified at cc84b97
...
Production verified 2026-09-22: https://parlay-a5x9av809-jbravvvvs-projects.vercel.app Ready, /api/version = cc84b971042eb31cd3c1631b4ce20ad67e06ae57. Root opens Games in production browser; NFL Board shows new First Sunday Six link, accent filters and gold odds. Remote automated context refresh commits preserved by rebase. Documentation-only release receipt follows.

Verification: production build passes; 141 focused tests pass (7 files), including live quote regression. Browser: fresh local origin 127.0.0.1:3600, Games redirect; 390x844 Stats shows 13 full rows without name truncation, dropdown fits x107..357 in width390 with Select All/Clear/checkboxes visible. Desktop generator reviewed. A previously registered localhost service worker served old development chunks; use 127.0.0.1 for clean local QA. Production static assets are content hashed. Real iPhone safe-area values cannot be emulated by viewport alone; CSS uses env safe-area insets.

Roster UI code 54b7774 is now deployed to both staging and production: emerald gradient panels, semantic filter borders, full-name compact Stats and notch-safe shell. Full 1330 tests pass, build passes, engine drift guard clean. Staging dpl_3u2VrZNF7eDHKYJKF4y5Gb6AKBfY was browser-verified first, then production fast-forwarded and scripts/deploy.sh deployed dpl_6C5yLThMoU3KBzSGdEhaWaAsQzX7 (roster-lab-phi.vercel.app). Production browser shows new 198px full-name Stats column at 390px width, 100 rows and no page overflow. Roster release documentation: docs/ui-refresh-2026-09-22.md. Both handoff packages synchronized at session close. No actual betting transactions or pricing model edits.
...
Validation: 98/98 tests across Board UI, Board compact, discovery release and ranked-pick suites PASS; production build including TypeScript PASS. Local browser confirmed NFL category selection/close and shared multi-select Clear, Select All and outside dismissal; desktop expanded menu visually inspected. Local has no odds feed, so generated-ticket content not visually verified there. First Sunday Six preceding release e7b71992ce08a3960b19d4cf7a2e246a0c1e29ef is Vercel Ready (parlay-g4jbr4dir); final combined receipt follows this UI release in release-verification.json.
...
Implemented shared generator Bet Type → The Model (MLB/NFL/CFB, including mixed sports). It searches 64 seeded candidate combinations, ranks available estimates against price plus probability/diversity, admits negative EV without grade gates, and retains existing odds/market/timing/position/game/leg/payout constraints, exclusions, locks/history. No optimal-combination guarantee; missing/settled/stale prices are still excluded. Mode is saved/restored with recipes. Football props remain market-consensus estimates, not simulated player forecasts; UI discloses source and unmodeled same-game correlation. No odds/grade values are upgraded to make a ticket look better. Existing style selection remains available separately.

Fresh-price fixes: authenticated manual football pulls bypass event-count caps as well as cached/empty-event reuse, then read game slate AFTER updated props/1H lines are stored. MLB manual live pull now bypasses empty-market hold as well as quote-age reuse. Three-book quotes (DraftKings, Caesars/williamhill_us, FanDuel) remain fetched together and repriced locally per selection. API coverage still limits available markets; never claim every bet offered directly by a book is exposed by the feed. Auth and overlapping-request protection remain. Labels updated as requested.

Runtime readiness at 2026-09-20 08:01 UTC: fresh NFL production refresh fetched 14/14 games, 3,943 prop rows; DK 3,589 / Caesars 1,554 / FD 3,137 rows with quotes. All 14 game slates matched, oddsMissing false. Fresh slate passed a local run of the exact paper-card builder: $350 core across 10 tickets, $25 fun, noPlay false. This is a PREVIEW, not a locked ledger entry. Authenticated /api/refill?desk=nfl returned HTTP 200, waiting, first kickoff 17:00Z and locksAt 16:00Z (9 a.m. Pacific). Ledger has no September 20 lock yet, correctly before the window. Added Vercel Sunday /api/nfl/lock backup cron calls at 16:00Z and 17:00Z (daylight/standard kickoff coverage), using existing CRON_SECRET Bearer auth and idempotent lock path. No external cron account claim and no real-money bet placed. Do not report allocation as spent until a locked record proves it.

Validation: 190/190 tests across nine suites PASS, including one temporary local production-slate readiness test (removed after evidence captured); 189 committed regression tests in eight suites. Production build including TypeScript PASS. Checks cover negative-EV generation, odds/leg filters, locked picks, fresh combinations, longer-odds value ranking, timing/empty filters, saved mode, repeated/empty-market refresh, Sunday $350 policy and lock idempotency. No rendered Chrome verification; prior browser limitation/intermittent hydration warning remains open. Production SHA and verified archives are recorded in external release-verification.json after deployment.
...
NFL/CFB defense props now canonicalize D/ST / Defense / DEF aliases and attach the verified matchup team, combining equivalent sportsbook quotes into one contract. Defense cards render team logos instead of player portraits. Old cached defense names can resolve against the shared roster catalog. Mixed-sport player cards now use the shared roster portrait lookup and top-left team badges; team sides show team logos and totals show both teams. Football generator and Every Pick Today pass matchup roster scope, including CFB. Cross-sport generated board parlays now include pick imagery. Catalog fetching tolerates individual team failures and retains trusted ESPN alternate portraits; full roster outages still fail instead of being cached as an empty success. Missing provider photos retain visible fallbacks; do not claim every provider image is available.

Validation: 196/196 affected regression tests across ten suites PASS, including defense book-alias merging, team identity, portraits/badges and side/total logos. Production build including TypeScript PASS. No rendered browser verification claimed; prior Chrome-control limitation and intermittent production hydration warning remain unresolved. Exact deployed commit and backup checks are recorded in external handoff release-verification.json after deployment.
...
>
> *(REFRESHED 2026-09-11 by INSTRUCTION 50, and the refresh was NOT optional. The superseded
> 2026-09-08 claim named `a00effb4`, which `git rev-list --count` measured THIS TURN at **exactly
> 10** commits behind `46f68df` — precisely the `K = 10` in `tests/sha-currency.test.ts`, i.e. the
> largest distance that still passes (`d > K` is the failure). The next commit on this branch would
> have turned that guard red, so the claim was refreshed inside the same work rather than left for
...
13. **THE $150 ACTUALLY DEPLOYS (added 2026-08-19, same day — verbatim: "There is only
    ...
    (`decideTopUp` → `generate?topup=1&slot=…`, fresh prices, TOPUP_MAX 6/day registry-capped since INSTRUCTION 49 (was 2, then 4 under INSTRUCTION 48) and fired only on the five PT refill slots or Josh's own Refresh,
    a full generate each — 114-150 Odds credits measured, app/api/generate/route.ts:46). Budget now OUTRANKS the under-bias quota on conflict
...
**INSTRUCTION 18 (2026-09-03, Josh's word, verbatim: "I would say change everything that you think is necessary to optimize this engine/website and get it on track to start making theoretical money. ... Lets make this app an UNSTOPPABLE theoretical money makin' machine" — and, same day: "Could maybe look into taking like. 8-15 leg H+R+RBI etc as one or more of the fun tickets daily as well. Could be other props or longshots as well." / "I dont want to change that $25 fun money hypothetical per day")**
A PAPER-ERA RULE CHANGE dated 2026-09-03, so the record splits cleanly before/after: every core ticket locked from this date carries `coreRules` (= `CORE_RULES` in `src/lib/paper-mode.ts`, since "2026-09-03") and `probRaw`/`czEvRaw`/`bsEvRaw` beside the shrunk numbers. Diagnosis measured over the 19 paper days 2026-08-15..09-02 (129 core tickets): model said 56.0 wins, 46 landed; 1-leg 25-22-6 (−15% ROI), 2-leg 18-32 (−18%), 3-leg 3-23 (−53%); tickets settling above decimal 2.6 went 11-38 (−42%); H+R+RBI OVER legs were 121 of 231 core legs and the calibration store shows the more edge the model claims on HRR the worse it hits (edge 5-10 bucket pred .634 act .571; edge 10-20 pred .671 act .443); leg-level HRR overs 54% vs 61% market-implied; TB unders 70% vs 63%, HRR unders 63% vs 60% (fine); forced pass (caesars_ev) $915 at −27%; stakes ≥$20 −33..−36% ROI, stakes <$10 −2%; the alt ev_gated selection was worse than dk_fd (−35% vs −26%), so the dk_fd basis stays. The rules:
1. **Market shrink** (`src/lib/shrink.ts`, `CORE_RULES.shrinkW = 0.5`): every pool ticket is mapped ONCE through `shrinkTicket` in `buildLockEntry` before the under-bias prune, so primary, alt and forced passes all read the same numbers. legP' = 0.5·prob + 0.5·imp (de-vigged consensus; a leg with no imp is untouched); ticket prob scales by ∏legP'/∏legP (the engine's simJoint number survives); czEv/bsEv/ev recomputed. Motivation: 56.0 predicted vs 46 actual; HRR edge buckets inverted.
2. **Raw numbers recoverable** (`src/lib/server/lock-card.ts` toTicket): `probRaw`, `czEvRaw`, `bsEvRaw` on every core/alt ticket. `validateLedger` is additive (index signature) — no change needed there.
3. **Shape + stake ceiling** (`lock-card.ts` buildModeCard + pool filter; `CORE_RULES.maxLegs 2`, `maxDec 2.6`, `maxStake 25`): both shAllocate calls pass `coreMaxLegs`/`coreMaxDec`/`perParlayCap = min(cfg, 25/amount)`; because the allocator's capG floors at amount/n, the $25 ceiling is ALSO enforced by a clamp at ticket assembly (`capTrim` stamped) and the pool is pre-filtered to ≤2 legs and ≤2.6 at BOTH the basis and the settling price (counted `blockedReasons.core_shape_rules`). The residue top-up is now a per-ticket map, cap-respecting, best-EV-first; what no ticket can absorb is stamped `capResidue` with a note — never a breach. Motivation: 3-leg 3-23; >2.6 dec 11-38; stakes ≥$20 −33..−36%.
4. **Forced pass by true probability** (`CORE_RULES.forcedSelMode = "probability"`, `forcedMaxDec 1.75`): in shAllocate that mode sets evGated=false and disciplined=false, so force=false trips no gate, no nv_tax floor and no Kelly ceiling — exact-sum stays. Forced candidates must price ≤1.75 at both quotes. Motivation: $915 forced at −27%.
5. **HRR overs out of core** (`CORE_RULES.noHrrOver`): any pool ticket with a `batter_hits_runs_rbis` leg whose prop contains " O " leaves the pool before either pass; `blockedReasons.hrr_over_suspended` is always a number (0 included). HRR unders, TB, hits, ML, RL stay. Motivation: 121/231 legs, 54% vs 61% implied.
6. **FUN ladder** (`src/lib/fun-hr.ts` `buildFunLadderTicket`, `FUN_LADDER` $10, 8–12 legs target 10, team cap 2): highest-model-prob H+R+RBI O 0.5 + Hits O 0.5 rows with a Caesars price, one leg per player, leg-disjoint from everything staked (susp ignored for fun, noParlay skipped); type `fun_ladder`. The HR composer takes the other $15 with one fewer seat (≤5 fun tickets); a board that cannot seat 8 sends the whole $25 to HR tickets exactly as before. Fun total is PAPER.fun ($25) every day — unchanged, per Josh.
7. **Tests**: `tests/shrink.test.ts`, `tests/fun-ladder.test.ts`, new INSTRUCTION 18 block in `tests/lock-card.test.ts` (fixture in "probability" mode, non-vacuous). Pins updated with dated notes, none deleted: the 08-22 residue case ($46 → $25 + capResidue 21), the gate-clears-nothing case (the fixture's cheapest ≤2-leg ticket settles at 1.9091, so the ≤1.75 forced pass seats nothing there — measured, not assumed), and the `paper-epoch` source scans (caesars_ev → CORE_RULES.forcedSelMode; residue → capped map).
MEASURED on the armed fixture (probability mode) after the change: 6 core tickets × $25 = $150 (TB singles at 1.91/1.98, ML/RL 2-leg parlays 2.21–2.55), 40 pool tickets dropped by shape, 3 by the HRR-over rule; under dk_fd the fixture pool clears nothing (20 shape drops) — a $0 decision record with `capResidue 150`. Open concern: the 2-leg/2.6/1.75 gates make thin-pool $0 days likelier under dk_fd; the `capResidue` and histogram fields make each one legible, and the deficit still carries to the next fire/top-up sweep.

**INSTRUCTION 19 (2026-09-03, Josh's word, verbatim: "On the stats tab, there should be a button called pitcher vs team where you can select a pitcher as well as a separate MLB team and it shows every active hitter on the roster with their career stats against that pitcher. That way I can see who is hot against each pitcher or if a pitcher is great against a team etc.")**
Shipped same day. Stats page header gains a "Pitcher vs Team" pill (`app/stats/page.tsx`) that mounts `src/components/stats/PitcherVsTeam.tsx`: type-ahead pitcher picker (all 30 active rosters via `/api/pvt?pitchers=1`, revalidate 1800s) + team select; `/api/pvt?pitcher=<id>&team=<teamId>` fans out one statsapi `vsPlayer` call per active hitter and returns rows (G/PA/AB/H/HR/RBI/BB/K/AVG/OBP/SLG/OPS from the `vsPlayerTotal` career split) plus recomputed totals. Pure shaping in `src/lib/pvt.ts`, pins in `tests/pvt.test.ts`. Public route, statsapi only, no ledger data. Known minors (reviewer, unfixed): an unknown pitcher id surfaces as 502 not 400 (statsapi 404s); the 30-roster fan-out has no retry so one rate-limit hit blanks the picker for that request; the pill shows on non-MLB sports too.

**INSTRUCTION 20 (2026-09-03, Josh's word, verbatim: "There should be a tab called "Games" that has every game for the day listed kind of like the mlb app or any other website.")**
Shipped same day. New nav entry Games (`src/components/shell/AppShell.tsx` NAV) → `app/games/page.tsx` (TanStack Query on `/api/games?date=`, 60s refetch while any game is live, 5 min otherwise; date strip today ±2; LIVE / UPCOMING / FINAL / POSTPONED sections with logos, records, probable pitchers with season line, linescore R/H/E, W/L/S decisions, broadcasts). `app/api/games/route.ts` joins the statsapi schedule (revalidate 30s) with the day's stored board (same `BOARD_KEY`/`decodeBoard` helpers as `/api/board`) so each team shows the best-book ML + Caesars price when the board carries it. Pure shaping in `src/lib/games.ts`, pins in `tests/games.test.ts`. Known minors (reviewer, unfixed): the board's `categories.ml` is one side per line, so most upcoming games price only the favored side ("—" on the other); `mlFor` compares `gkey` without the engine's `gm1`/`gm2` doubleheader suffix, so doubleheader games read "—"; nine mobile tabs are tight at 375px; broadcast strings print verbatim.

Josh also gave the ledger sync phrase in chat this day so the private ledger could be READ for the instruction-18 diagnosis. It was used only as an `x-pl-sync` request header and is not written anywhere in this repo; do not write it.

**INSTRUCTION 21 (2026-09-03, Josh's word, verbatim: "On the Games tab, the list should keep going through the last regular season game of the year which is Sunday Sept 27. You should also be able to click on any game to see the box score. I will include examples above. Only games from Sept 1 on need to be included in this tab.")**
Shipped same day. `src/lib/games.ts` gains `SEASON_WINDOW` (2026-09-01..2026-09-27, the regular-season close Josh named); `/api/games` rejects dates outside it (400) and the page clamps. `app/games/page.tsx`: scrollable `DateRail` (`src/components/games/DateRail.tsx`) over every day in the window, "Today" highlighted, `?date=` kept in the URL; every card links to `/games/[gamePk]`. Box score page `app/games/[gamePk]/page.tsx` + `/api/games/[gamePk]` (statsapi boxscore + linescore + schedule hydrate, 30s refetch while live) shaped by `src/lib/boxscore.ts` (pure; fixtures `tests/fixtures/boxscore-{final-822686,live-824796,pregame-823907}.json`, pins `tests/boxscore.test.ts`): header/records/score, linescore with R H E and "x", W/L/S with W-L ERA/saves from the box's own seasonStats, per-club batting box (AB R H RBI BB K AVG OPS, subs indented, Totals), the feed's batting/baserunning/fielding notes verbatim, pitching box (IP H R ER BB K HR ERA with W/L/S/H/BS tags), and the feed's game info block. Doubleheaders: gkey `gm1/gm2` suffix now honored in ML matching and cards carry a "Game N" tag. Postponed games show "Postponed — no box score". Known minor (unfixed): the selected date is read from `?date=` once on mount, so re-tapping the Games tab keeps the rail's date while the URL drops the query.

**INSTRUCTION 22 (2026-09-03, Josh's word, verbatim: "On the Stats tab and any other tab that lists a players name, you should be able to click on that players name & pull up a page that is identical to their Roster Lab profile. I will include examples above and you can also view the website https://roster-lab-phi.vercel.app/players")**
Read-scope addendum, dated 2026-09-03: to honor "identical", the Roster-Lab `src/components/player/PlayerSheet.tsx` and the ui components it imports were READ (read-only, layout parity only). Nothing else in Roster-Lab was read, nothing there was written, no .env was opened. The standing "do not read or write Roster-Lab" rule otherwise holds.
Shipped same day. `src/components/player/PlayerSheet.tsx` (provider mounted in `app/providers.tsx`, `usePlayerSheet`) + `PlayerName.tsx` wrap every rendered player name: Stats table, board rows (name parsed off "Name (TEAM)" via `parseBoardLabel`), Builder legs, Sharp, Pitcher vs Team rows and picker chip, Games list/box score (with MLB ids), Parlay Builder rows. Layout copies the Roster Lab sheet (photo/name/team·pos header, three tiles, split table 2026 / Last 7 / Last 15 / Last 30, by-game bar chart, game log, footer). HONEST DIFFERENCES from Roster Lab, because Parlay Lab has no ESPN fantasy league: tiles are OPS / HR / AVG (hitters) or ERA / K / WHIP (pitchers), not FPTS / rank / % rostered; the chart is TOTAL BASES by game (K's for pitchers), not fantasy points; NO news section (no free news feed keyed by MLB id — nothing fabricated); no draft line. Data: `/api/player?id=` (person + season + byDateRange windows + gameLog, shaped by `src/lib/player-card.ts`, `pickSplit` prefers the combined split so a mid-window trade never shows one club's partial — pinned on the real Arraez 650333 fixture) and `/api/player/resolve?name=&team=` (season player index, revalidate 1 day; exact → team → last-name+initial). Pins: `tests/player-card.test.ts`, `tests/player-wiring.test.ts`. Known minors (reviewer, some fixed in the fix pass): unknown id maps to 404; stamped board picks pass the team; first column sticky on the phone.

**INSTRUCTION 23 (2026-09-03, Josh's word, verbatim: "The order of the tabs on left side of screen should be: Games, Stats, Board, Builder, Parlay Builder, Parlay Calculator (formerly Calc) on Top Left & Ledger, The Sharp, Simulator, Settings on Bottom Left. You can remove Dashboard tab as it does the same thing as pressing the Roster Lab logo in top left of screen which is take you back to main landing page")**
Shipped same day. `src/components/shell/AppShell.tsx` NAV is a typed table with `group: "top" | "bottom"`: top Games, Stats, Board, Builder, Parlay Builder, Parlay Calculator; bottom Ledger, The Sharp, Simulator, Settings (rail uses a flex spacer). Dashboard removed; brand logo still links "/", and the landing page header now reads "Parlay Lab" instead of "Dashboard". Mobile bottom bar keeps 6 tabs (Games, Stats, Board, Builder, Parlays, Ledger); Calculator, Sharp, Simulator, Settings sit in the mobile top bar icons. Pins: `tests/nav.test.ts`; AppShell grid-column pin in `tests/parlay-calc.test.ts` kept.

**INSTRUCTION 24 (2026-09-03, Josh's word, verbatim: "We need a massive UI rebuild on the 'parlay builder' tab. It looks EXTREMELY visually unappealing and I would hate to spend an hour looking at that building parlays. The tabs could be on two scrolls where you press on it still but can scroll the furthest ones like 1st 3 innings & 1st 5 innings from right to left. Actual pick choices on batter props tabs like 'Pete Alonso', 'Gunnar Henderson' when looking at their total bases, HRs etc are WAY TOO BIG.")**
Shipped same day. `app/props/page.tsx` rebuilt on `src/components/props/*`: `props-model.ts` (the old TABS/MARKETS/price/prob/label helpers moved verbatim, zero logic change, now unit-pinned in `tests/props-model.test.ts`), `MarketNav.tsx` (sticky segmented control + horizontally scrolling snap pill rail with a right-edge fade), `PlayerRow.tsx` (28px headshot, 12px name, ≤34px O/U buttons; ~10 rows per 375×812 screen vs ~5 before), `Slip.tsx` (collapsed bottom-sheet handle "N legs · odds · true %", expands ≤45vh, same Odds/True/EV math). Source pins in `tests/props-ui.test.ts`. Nothing tracked/ledgered here — still the sandbox.

**INSTRUCTION 25 (2026-09-04, Josh's word, verbatim: "Move the Ledger tab back up right below Parlay Calc (Rename it from Parlay Calculator)")**
Shipped same day. `src/components/shell/AppShell.tsx` NAV: /calc label is "Parlay Calc" (the 09-03 "Parlay Calculator" lasted one day), Ledger moves into the top group directly after it; bottom group is now The Sharp, Simulator, Settings. Calc page header matches. Pins updated with dated notes in `tests/nav.test.ts` and `tests/parlay-calc.test.ts`.
...
**INSTRUCTION 28 (2026-09-04, Josh's word, verbatim: "It keeps showing Jose Caballero on the board even with a refresh yet he's not in the yankees starting lineup so there's no bets available for him at any book")**
Shipped same day. Diagnosis from the live reads (board `at` 21:45:07 UTC): the stored board's H+R+RBI row "Jose Caballero (NYY)" carried `lu: "projected"` and a generation-time BetMGM −155, `gameInfo` said the Yankees lineup was not posted when it was built, and statsapi's posted nine (hydrate=lineups, pk 823256) had no Caballero. The engine's own rule (legacy @~199372: a POSTED lineup missing the player = off the board, Caesars voids the leg) only runs at generation; the prop tabs render the day's STAMPED picks from `/api/picks`, which are that stored board, so neither Refresh nor a reload could scratch him. Fix is render-time and pure — nothing stored is edited, the graded cohort is untouched: `src/lib/lineup-check.ts` (`shapeLineups` from the statsapi schedule read, `lineupStatus(name, market, pk, lineups)` → in / out / unknown; batter markets only — pitchers never appear in a batting lineup; a game counts as posted only with both nines up), `src/lib/useLineups.ts` (keyless read through `/api/stats`, 2-min refetch), and `app/board/page.tsx` marks every OUT batter on both the live-board DataTable and the stamped-picks table: hidden by default, a red "N picks scratched — not in the posted lineup" note with a show/hide toggle renders them greyed with an OUT tag. Fixture `tests/fixtures/lineups-2026-09-04.json` (the real read, trimmed to NYY@SD posted + DET@CLE gm2 stripped to unposted); `tests/lineup-check.test.ts` pins the Caballero case, the accent/suffix match (feed "Luis García Jr."), pitcher exemption, unposted/unknown → never OUT, and the page wiring. Post-deploy live check found the same Caballero leg on a stored 3-leg H+R+RBI parlay card under the table, so `ParlaysSection` takes a `legOut` callback: the leg is struck through with an OUT tag and the ticket wears a red SCRATCHED LEG pill and is dimmed (stored tickets are not rewritten — the verdict is display-time, same as the tables). Not changed: the engine's projected-lineup logic and `/api/picks` (the picks product stays the stored board — the scratch is a display verdict over live lineups, which is exactly what a book does).

**INSTRUCTION 29 (2026-09-04, Josh's word, verbatim: "I should be able to sort each tab on the 'Board' like H+R+RBI, Hits, etc by clicking on the title of the column ie: 'Tier' or 'Edge Status'")**
Shipped same day. The stamped-picks table on the prop tabs was a hand-rolled `<table>` with no sort; it is now the same `DataTable` the ML/RL tabs use, with `pickColumns` carrying a `sortValue` on every column (#, Grade, Pick, Lock price, Model, Implied, Edge, Status) — click a header once for descending, again for ascending (▲/▼ marks the active column). Status sorts won → in-progress → upcoming → lost → void. Source-scan pin in `tests/lineup-check.test.ts`.

**INSTRUCTION 30 (2026-09-04, Josh's word, verbatim: "Rename 'Tier' to 'Grade'")**
Shipped same day. Both Board headers (the live-board DataTable column and the stamped-picks column) read "Grade"; the chip and `gradeFromEv` were already named grade. The Stats tab's calibration panel keeps its own "Tier" header — that column is a probability bucket, a different thing, and Josh's instruction was about the Board. Pinned: `app/board/page.tsx` contains no "Tier" header (`tests/lineup-check.test.ts`).

**INSTRUCTION 31 (2026-09-04, Josh's word, verbatim: "On 'Board' there should be two tabs next to each other 'Top 50' & 'ALL'; If I click on 'Top 50' then click on one of the categories (ie: hits) then all top 50 picks in that category will show; If I click on 'ALL' then click on one of the categories (ie: hits) then it will show all daily hits props starting with S grade, then A, B, C, etc down ('S' grade will now be the highest possible grade right above 'A' grade)")**
Shipped same day. `app/board/page.tsx`: a Top 50 / All segmented control sits ahead of the category pills. Top 50 is the default and is the previous behaviour exactly (the engine's ranked pool; prop tabs = the day's stamped picks; the cross-market pill is now labelled OVERALL so it no longer collides with the scope name). All builds its rows from the board's uncapped `propBoard` — the same book the Parlay Builder browses — one OVER row per priced line, graded on model − fair (`pO − fO`, the same edge the stamped picks grade on), ordered S → F then by edge; lines the engine did not price carry no grade and sink. Under All the cross-market pill reads EVERY MARKET and mixes markets (the market is named on each row); that view is capped at 400 rendered rows with a note, and search narrows it. ML/RL are unchanged under either scope (never truncated). Scratched batters and the Caesars toggle apply in both scopes. Pins: `tests/board-scope.test.ts`.

**INSTRUCTION 32 (2026-09-04, Josh's word, verbatim: "'S' grade will now be the highest possible grade right above 'A' grade")**
Shipped same day. `src/lib/grade.ts`: `GRADE_CUTS.S = 6` (double the A bar; A ≥ +3 unchanged — S is one more label on the same number, no new model), `gradeRank` S = 6, gold chip with the cutoffs in its title (`GradeChip.tsx`). Every Board grade — live rows on czEv/bsEv, stamped picks and All-scope rows on edge — flows through `gradeFromEv`, so S appears everywhere at once. Pins updated in `tests/grade.test.ts` (S boundary at +6, A at 5.999).

**INSTRUCTION 33 (2026-09-04, Josh's word, verbatim: "There should be a search bar on right side of live tab on board to search for a player within the prop i have highlighted or all of their daily props if i search under 'All' tab")**
Shipped same day. A search box sits at the right of the LIVE pill row on the Board (`aria-label="Search players"`), matching by accent-/punctuation-insensitive name (`normalizeName`) against the current view: the live-board DataTable rows, the stamped picks, or the All-scope rows. All + EVERY MARKET + a name = every one of that player's priced props for the day. Pinned in `tests/board-scope.test.ts`.

**INSTRUCTION 34 (2026-09-04, Josh's word, verbatim: "Prop bets still aren't pulling up on 'Parlay Builder'. Daily odds should generate along with the 'Board' generating")**
Shipped same day. ROOT CAUSE, observed live 2026-09-05 01:05 UTC by pressing Refresh MLB on prod in the browser pane: all 16 per-event props calls through `/api/odds` returned 403 "market shape not allowed", because the engine's props URL has always asked for `SH_PROP_MARKETS` + `SH_PROP_ALT` (`batter_hits_alternate, pitcher_strikeouts_alternate, batter_home_runs_alternate` — the Caesars ladders) and the 2026-08-02 shape allow-list (`src/lib/server/odds-shape.ts`) listed only the six core markets. The server generate sends the same URL to the Odds API directly, so server boards always had props; a DEVICE refresh built a board with every batter/pitcher category at 0 and `propBoard` = 0 games, which then won `bestBoard` on freshness (19 priced ML/RL games vs the server's 15) — the Board's prop tabs still showed the stamped server picks, while the Parlay Builder, which reads the device board's `propBoard`, went empty. That is exactly Josh's report, and it means every device Refresh since 08-02 was props-blind. Fix: the three ladders join the proxy's props shape (still regions=us only — `tests/odds-hardening.test.ts` pins the engine's exact URL passing and the widened-region variant still failing). Belt and braces: `app/props/page.tsx` falls back to today's server-built `propBoard` (via `/api/board`) when the chosen board carries none, with a gold note saying so — the Builder can no longer be emptied by a bad device board. Not changed: `bestBoard`'s preference order.

**INSTRUCTION 35 (2026-09-04, Josh's word, verbatim: "On 'Stats' tab, change Hitting filters from Last 7 Days, Last 15 Days, Last 30 Days, 2026 Season to Last 7 Games, Last 15 Games, Last 30 Games, 2026 Season. For Hitters, this should only reflect games they played in. If a player sits out 3 of their teams last 18 games, that is the sample size for last 15; if he sits out 2 of their last 30, then 2 more games need to be added")**
Shipped same day. The Stats table's MLB timeframe was a calendar `byDateRange` (team days, off days and benchings alike); it is now a per-player GAME window. MLB's `lastXGames` stat type is exactly "the last N games he appeared in", but the league-wide `/api/v1/stats?stats=lastXGames` endpoint pins X at 10 whatever `limit` says (observed 2026-09-05 01:40 UTC: every split gamesPlayed=10 for limit 5/15/30, and `&lastXGames=`/`&x=` variants ignored), while `/api/v1/people?personIds=…&hydrate=stats(group=[hitting],type=[lastXGames],limit=N,season=…)` honours N and accepts the whole league in one call (738 hitter ids in one URL of ~5 KB, 2.8 s, 2 MB upstream). New route `app/api/stats/window/route.ts` (`?group=hitting|pitching&n=&season=`) does season-list → people hydrate (chunked at 400 ids) and returns the league-shaped `{stats:[{splits}]}` the page already parses, slimmed to the table's columns (`KEEP_STAT` in `src/lib/stats-window.ts` — full hydrate was ~1.1 MB for pitchers, ~0.4 MB after). Menu: "Last 7 / 15 / 30 Games" + season (`WINDOW_GAMES.hitting`), values `g7|g15|g30`; a note under the filters reads "Last N games each hitter played in — off days and benchings don't count". Team scope uses MLB's `teams/stats?stats=lastXGames&limit=N`, which does honour N. Not changed: the player card's own "Last 7/15/30" rows (`src/lib/player-card.ts`, still day windows — Josh's instruction named the Stats tab filters).

**INSTRUCTION 36 (2026-09-04, Josh's word, verbatim: "On 'Stats' tab, change Pitching filters from Last 7 Days, Last 15 Days, Last 30 Days, 2026 Season to Last 3 Games, Last 5 Games, Last 10 Games, 2026 Season; For Pitchers, when we do 3, 5, 7 games we are referring to starts for SP. So when it shows Gerrit Cole under 'Last 5 Games' it should show his last 5 Games Started. For a player like Aroldis Chapman, it should show his last 5 RP, not just the last 5 games the Red Sox played, only games he pitched in.")**
Shipped same day with 35. `WINDOW_GAMES.pitching = [3, 5, 10]`; switching HITTING ↔ PITCHING keeps the menu position (Last 15 ↔ Last 5, `siblingWindow`). `lastXGames` is the last N APPEARANCES, which is the spec for a reliever and for any starter whose window is all starts. When a starter (the page's own SP rule: GS ≥ max(1, G/2) on the season line) has a non-start inside his window — 45 of 845 pitchers on the live 2026-09-05 n=5 pull, e.g. an opener day or a rehab relief outing — the route pulls those pitchers' game logs (`hydrate=stats(type=[gameLog])`, same batch call) and `aggregateStarts` sums his last N STARTS, recomputing IP/ERA/WHIP/K-9/AVG from the sums (`windowSource:"starts"` marks the row). Note under the filters: "SP: last N starts · RP/CP: last N appearances". Pins: `tests/stats-window.test.ts` on fixture `tests/fixtures/stats-window-2026-09-05.json` (real statsapi captures, seven players: Cole all-starts window kept, Chapman appearances kept, Houser 2-of-5 starts → last 5 starts from the log, Marte 1 start → 1-start line never padded, Judge/Bae hitting windows) plus page/route source scans (no "Last N Days", no `byDateRange`). Verified on the dev server and on prod after deploy.

**INSTRUCTION 37 (2026-09-04, Josh's word, verbatim, answering the report on 35–36 that the player card still showed day windows: "Yes change everything that shows those stats to the new way")**
Shipped same day. The player profile sheet (`src/lib/player-card.ts`, route `app/api/player/route.ts`) no longer asks statsapi for three `byDateRange` windows; it cuts its split rows from the game log it already fetches: hitters "Last 7 / 15 / 30 games" (games he appeared in, `aggregateHitting` — AVG/OBP/SLG/OPS recomputed from the sums because the feed's per-game rates are season-to-date), starters (the Stats table's own rule, GS ≥ max(1, G/2) on the season line) "Last 3 / 5 / 10 starts", relievers "Last 3 / 5 / 10 games" (`aggregatePitching`, both in `src/lib/stats-window.ts`). The bar chart is the longest window per game (hitters last 30 games; pitchers last 10 starts / appearances) instead of the last 30 days. `windowDates` and the two `*-last30.json` fixtures are gone; the card route is now two upstream calls (season + gameLog) instead of five. Cross-checked on the dev server against MLB's own per-player `lastXGames`: Cole last 5 starts 30.1 IP / 2.97 ERA, Judge last 15 games 57 AB / .193 — identical to what the Stats table's window route returns for them. Pins: `tests/player-card.test.ts` (Acuña's 6-game trimmed log → Last 7 = 6 games with recomputed .455/.538/.636; Skenes' last 3 / 5 starts summed by hand; a reliever relabel; feed order shuffled; no-log → null rows) and the route/sheet scans there. Nothing else in the app rendered day windows (grep for byDateRange / "Last N Days" / "last 30 days" across app/ and src/ is clean apart from the engine's own internals, which are not player-facing stat windows).

**INSTRUCTION 38 (2026-09-05, Josh's word, verbatim: "* Parlay Lab should now be FULLY FUNCTIONING for College Football. Make sure the engine is optimized for each sport individually and make any adjustments you need to * EVERYTHING for College Football (CFB) should be SEPARATE; Tickets for College Football in the Builder should be SEPARATE; Board for CFB should be separate; Ledger & Allotted $ for College Football should be separate (But still $150 Core & $25 Fun money) * Feel free to do a full UI upgrade while keeping the motion background and general color theme. I think the website could be significantly more high level & fun")**
Shipped same day. A second desk, not a mode: `src/lib/sport.ts` holds the global switch (localStorage `pl_sport`, event `pl:sport`, `useSport()`), the `SportSwitch` pill sits under the brand on desktop and in the phone header, and every page early-returns its College Football surface on `cfb` with the MLB surface untouched. Engine per sport: the CFB model (`src/lib/cfb/model.ts`) is a market + FPI margin model — per-book proportional devig, book spreads and totals turned into implied margins through a normal curve (σ 16.5 margin, 18 total, HFA 2.6, 0 at neutral sites), Pinnacle-weighted median consensus across `us,eu` books, blend {market .60, spread-implied .25, FPI .15} for win probability and {market .75, FPI .25} for the margin, every book quoted at its own line with EV re-evaluated there, Caesars (`williamhill_us`) the settle book, letter grades from the shared `gradeFromEv`, quarter-Kelly capped at 2 % only on playable rows; constants and the paper rules live in `src/lib/cfb/rules.ts` (`CFB_MODEL`, `CFB_RULES`, `CFB_PAPER`: $150 core + $25 fun per slate day since 2026-09-05, 3–7 core tickets, EV ≥ 2 % at Caesars, decimal ≤ 2.60, stakes $5–$25, one side per game, forced top-ups at decimal ≤ 1.75 and EV ≥ 0). Separate everywhere: localStorage `pl_cfb_ledger` / `pl_cfb_bank2`, Redis `pl:cfb:ledger:v1` / `pl:cfb:bank:v1`, routes `/api/cfb` (ESPN scoreboard for the date and the next date + ESPN FPI + Odds API `americanfootball_ncaaf`, odds cached 240 s, quota forwarded, `oddsMissing: true` when the key or the feed is absent) and `/api/cfb/ledger` (same `x-pl-sync` gate as MLB sync, every entry must carry `sport: "cfb"`), query keys starting `"cfb"`, ticket ids `cfb-<date>-core-<n>` / `cfb-<date>-fun-1`, a $2,500 CFB bank with its own append-only adjustment log in Settings ("COLLEGE FOOTBALL BANK"), a CFB ledger graded from ESPN finals at the Caesars line (48 h ungradable window, NO-PLAY days skipped), its own sync beacon and chip. `tests/cfb-separation.test.ts` pins that no CFB module references an MLB key or route, that `/api/cfb/ledger` never names the MLB Redis keys, that every page imports `useSport`, and that `AppShell` mounts the CFB beacon. Team matching in `src/lib/cfb/names.ts` (normalised names, a 10-entry alias table, three match tiers, 3 h kickoff window). Fun-money decision (mine, recorded so it can be reversed): the first fixture card's fun parlay, ranked by EV, was the same three underdogs as its core singles at +631 (fixture data), so the fun picker now takes the likeliest sides at Caesars' line — grade D or better, never an F — across distinct games, moneylines and spreads before totals, until the decimal is ≥ 4 (3–5 legs), named FAVORITES PARLAY when at least half the legs are ≥ 50 % to hit. UI upgrade with the motion background and palette kept: page-enter transition (`app/template.tsx`), rail active pill with `layoutId`, `SportSwitch`, `StatTile` / `Segmented` / `EdgeMeter` / `Sparkline` primitives, `.press` / `.card-lift` / `.ticket` / `.shine` tokens in `app/globals.css`, `PageHeader` eyebrow + chip, landing copy and desk chooser, amber `--color-cfb` accent. Reference doc: `docs/cfb-desk.md` (constants, pipeline, alias table, keys, the fun decision). Tests: eight `tests/cfb-*.test.ts` files (normal, names, model, card, grade, store, route, separation) on captured fixtures in `tests/fixtures/cfb/` (12 games, 14 odds events, 138 FPI teams — fixture captures). Gate after the doc write: tsc clean, 143 test files / 1317 tests green in 188 s (run alone, no dev server). Deployed from the gated tree and read on prod 2026-09-05 (`/api/cfb?date=2026-09-05&bankroll=2500`): 68 games, 68 matched, 0 unmatched, 380 rows, 344 priced and playable at Caesars, 0 NaN, `oddsMissing false`, FPI stamped 2026-09-04T08:00Z, quota 17578 → 17572 (one 6-credit pull). Phone-width (375) walk on prod in CFB mode: Games (68 upcoming, priced cards — the local-dev Suspense stall on `/games` does not reproduce on prod), Board (68 games, 67 priced at Caesars, 20 +EV of 380, best edge Coastal ML +950 at +13.3 %), Builder (six $25 core singles — Missouri St +40.5, North Texas +40.5, California +2.5, East Carolina +28, Coastal +21, W Michigan +27.5 — $150 of $150 deployed, avg EV +3.9 %; FAVORITES PARLAY +622, 4 legs: Florida ML −3500 plus the three likeliest underdog spreads, 15.4 % to hit — Caesars posted no moneyline on most big favourites that day, so the likeliest priced sides were underdog spreads), Ledger (empty, $2,500 base), Parlay Builder (sandbox, Caesars / Best toggle), The Sharp (constants panel), Stats (NCAAF pill + FPI panel), Settings (both banks); the switch back to MLB restored the MLB Board (15 games · 291 rows). Known and accepted: local dev has no `ODDS_API_KEY`, so `/api/cfb` returns `oddsMissing: true` locally and price checks are prod-only; the MLB hooks still mount behind the CFB desk (cached, cheap). Nothing from the ask was left out.


**INSTRUCTION 39 (2026-09-05, Josh's word, verbatim: "* Need to add props for College Football: like anytime TD, pass TDs, passing yards, receptions, rush yards, receiving yards, * Football side is freezing A TON & only way to get out of freeze is to swipe back a page. Same as last time this happened I don't want swiping left to right to go backwards like Apple has their stuff set. • ⁃ The games list doesn't need to be on the 'Board' when it's already on the 'Games' tab; what should be on the 'Board' tab is all of the prop parlay options like MLB has with live, mixed, safe. Longshots, etc")**
Shipped same day. (a) Props: six CFB player markets (anytime TD, pass TDs, pass yds, receptions, rush yds, rec yds; contract `src/lib/cfb/props-types.ts`, pricing `src/lib/cfb/props.ts`, ESPN season context `src/lib/cfb/props-context.ts`, route `app/api/cfb/props/route.ts`, shared slate helpers `src/lib/cfb/slate-server.ts`); O/U pairs devigged per book at the same point, consensus line = weighted median of posted lines, fair = true median of the devigged probs (null under `CFB_PROPS.minBooks`), anytime-TD yes-only priced against a 1.08 overround; the Parlay Builder gained a market nav (SIDES · ANYTIME TD · PASS TDS · PASS YDS · RECEPTIONS · RUSH YDS · REC YDS) and prop legs go on the slip only when Caesars' line equals the consensus line. (b) Freeze: the Scramble Lab root cause again — history-push navigation arms iOS's edge-swipe-back recognizer and wedges taps, and per-item backdrop blur over the video freezes the page. Every internal `<Link>` is now `replace` (nothing pushes history, so there is nothing to swipe back to), `html, body { touch-action: manipulation }`, backdrop blur removed from every per-item surface (glass, liquid-glass, ticket, stat-tile, table header, tooltip, player-sheet overlay) and kept only on the shell chrome; guard `tests/nav-flat.test.ts`, observed red first. (c) Board: the CFB Board no longer lists games (that is the Games tab); it is the Picks board (`src/components/cfb/CfbPicksBoard.tsx`, `src/lib/cfb/picks.ts`) — ranked picks by category (all / ML / spread / total / the six prop markets, TOP 50 or ALL, search, sortable table) and the parlay section with PARLAYS / MIXED / LIVE views and SAFER / LONGSHOTS / MIX tiers (`CFB_PARLAYS` in `src/lib/cfb/rules.ts`). SAFER draws from sides AND props at ≥ 58 % because Caesars posts no moneyline on big favourites and a sides-only SAFER was empty on the opening slate. Cost lesson, read from the Odds API headers on the first prod pull 2026-09-05: a per-event player-prop pull costs about 31 credits (24 events = 753 credits, `x-requests-used` 2428 → 3187), not the 6 a slate pull costs. So `CFB_PROPS` caps a slate at 12 priced events, caches the priced board in Redis for `revalidateSec` (2 h) and refuses fresh pulls past `dailyBudget` (1200 credits a day, estimated at `measuredCreditsPerEvent` = 31); the board reports `source`, `budgeted` and `spentToday`. Prod's `/api/odds?u=` proxy refuses player-prop market shapes (403), so the props route fetches with the server key itself. Full reference in `docs/cfb-desk.md` ("Props, picks and parlays"; "Freeze fix"; "Quota").

**INSTRUCTION 40 (2026-09-05, Josh's word, verbatim, with Caesars Sportsbook app screenshots attached as the reference: "* It's not showing any current props for CFB. In game prop lines should still populate just like they do on the MLB side * The green 'Refresh Board' button is gone from the Board screen * We need to make the UI more appealing and more exciting to use. I have included examples from Caesar's Sportsbook app. Think along the lines of making this functional for the every day bettor. * The 'FPI' board on stats tab can be put in a button that pops out into an overlay that covers 60% of the screen and disappears when you click the 'x' in top right or click outside the borders of the overlay; it takes up too much space * Since we are separating sports betting boards by using 'MLB', 'CFB' etc tabs on main dashboard, only that specific sports' stats need to be in the stats tab for that sport * Along with the UI changes you're making that should already cover this, the logo sizes on the parlay builder page are so disproportionate to the boxes. The boxes should be smaller vertically and the logos should be slightly bigger * Make the UI on the Parlay Calc signifcantly more intriguing to use. If i was a high level CEO right now, I would click away immediately when I saw what that looks like and not use it. I want a billionaire CEO to look at this website & first thought is they immediately want to purchase it or invest in it")**
Shipped same day. (a) Live props: `selectPropEvents` (`src/lib/cfb/props.ts`) admits upcoming AND in-play games (never final / postponed), live first; whenever a priced event is in play the props board runs on `CFB_PROPS.liveRevalidateSec` (10 min) instead of the 2 h window, a live pull re-prices only the in-play events (at most `liveMaxEvents` = 6) and carries the upcoming games' rows from the stored board, the last good board is retained for `boardRetainSec` (36 h) so the daily budget cap degrades to "lines as priced at H:MM" instead of an empty board (`stale: true`). Cost of a live refresh is about 31 credits × in-play games (≤ 6), so the 1,200-credit day covers roughly six live refreshes — the daily budget was NOT raised because the Odds API plan is Josh's to confirm. (b) The green "Refresh Board" primary pill is back in the CFB Board header (`CfbRefreshPill`, invalidates both the slate and the props queries; the small ↻ pill is gone). (c) Caesars-grammar UI: shared `Overlay` (60 % sheet, × / backdrop / Escape close, portal, no blur) and `OddsGrid` (Spread · Money · Total tappable cells ≥ 44 px) in `src/components/ui/`, plus `.chip-row`, `.carousel`, `.hero-price`, `.sheet-60`, `.hit-44` tokens; Games cards and the Builder's SIDES cards rebuilt on the grid with `TeamMark` at "md" and shorter boxes; prop rows as Over / Under cells (anytime TD a single YES); Board gained a TOP EDGES carousel and a parlay carousel with "$10 wins" / "$25 pays" to the cent; Builder tickets as boost-style carousel cards. (d) Stats is scoped per desk (`src/lib/stats-scope.ts`: `sportsFor("cfb")` = NCAAF only, `sportsFor("mlb")` = MLB only, query gated until the scope is settled so MLB rows never flash on the CFB desk) and ESPN FPI moved behind an amber "ESPN FPI" button that opens the 60 % overlay. (e) Parlay Calc rebuilt (`app/calc/page.tsx`, `src/components/calc/`, pure math in `src/lib/calc-math.ts`: American ⇄ decimal, parlay decimal, implied and per-leg confidence, payout / profit / EV, a payout ladder, quick-add prices, stake presets, copy summary). Review pass (four lenses, adversarially verified) confirmed 16 findings, all fixed before the gate — the two that mattered: a pre-kick board cached for 2 h was still being served after kickoff (now re-priced 10 min after any game in it goes live), and the live cadence would have burned the budget into an empty board (now carried rows + stale note). Same-day follow-up, read on prod after the deploy: with one sorted list capped at 12, nine in-play afternoon games took every slot and the twenty-plus evening kickoffs had no props at all, and the footnote blamed the budget (92 credits spent) for a board that was merely stale; `selectPropEvents` now keeps two pools (live ≤ `liveMaxEvents`, then pre-kick ≤ `maxEvents`), carried rows adopt the current slate's status (kicked-off → `live`, not playable), and the footnote names the in-play games and blames the budget only when `budgeted`. Reference: `docs/cfb-desk.md` (Props "Live window", Quota, UI). Answered the same day, read from the live MLB board: the H+R+RBI "O 0.5" rows Josh saw under ALL (Butler, Duran) are lines Bovada / BetOnline post that Caesars does not (Caesars price null, tagged watch, H+R+RBI suspended from tickets); a hide-non-Caesars-lines option was offered, not shipped.
**INSTRUCTION 41 (2026-09-05, Josh's word, verbatim, answering the H+R+RBI question under instruction 40: "1. Yes, run everything based on the current caesars lines; hide the non-Caesars lines from ALL 2. If i go to another state like Arizona & I'm using Draft Kings or FanDuel then I will let you know and we will switch it while I'm there")**
Shipped same day. The MLB Board's ALL scope (`app/board/page.tsx`, the `allRows` memo) now keeps only the prop-board lines Caesars posts (`cz.o` priced); a line only another book posts at a different number — the "O 0.5 H+R+RBI" rows on Butler and Duran that Bovada / BetOnline posted while Caesars had 1.5 — is skipped, never graded, and the ALL footnote says "N lines Caesars posts on today's board … M lines only other books post hidden" (read on the 2026-09-05 MLB board before the change: 2,296 prop rows, 428 without a Caesars price — 195 of the 608 HR rows, 138 of 385 TB, 50 of 310 H+R+RBI, 24 of 763 hits, 21 of 51 outs, 0 of 179 Ks). Top 50 is untouched (the engine's ranked pool already grades EV at Caesars; the ⓘ Caesars toggle from 2026-08-09 still works on every tab). Point 2 is a standing rule, not a change: the settle book stays Caesars (`williamhill_us`) on both desks until Josh says he is in another state on DraftKings or FanDuel, and the switch is made only for that trip. Pinned in `tests/board-scope.test.ts`.
**INSTRUCTION 42 (2026-09-05, Josh's word, verbatim: "* Its only showing ANYTIME TD picks for 3 games under 'ALL' button on 'Board'. There are a ton of games live and a ton of games the rest of the day. It should be grading every possible pick available on the board that falls under those props and displaying them. If they aren't top 50 that's fine but they should but under the 'ALL' tab. * Under the 'generated parlays' on board tab, there needs to be A TON more. There should be 50 parlay options under each category (ML, spread, Anytime TD, Pass TD, Pass Yards, Receiving Yards, Combos, etc) The live parlay section and combo section (that has live & pregame picks on the same ticket) should still be generating picks as well")**
Shipped same day (workflow: 3 builders, integrator, 3 review lenses, 11 verifiers, 8 confirmed findings — 6 fixed in code, the `.gitignore` stray left as found, the budget capacity finding reported to Josh). (a) Props: every eligible game is priced — `CFB_PROPS.maxEvents` 60 (was 12), `liveMaxEvents` 24 (was 6), `dailyBudget` 2500 (was 1200); the carry decision is per game (`pricedAt` on the stored board), a live game whose last pull had zero rows is not re-pulled inside its 2 h window (the empty-event rule), and the stored board is chunked gzip + base64 under an index key because Upstash REST refuses requests over 1 MB. Cost: ≤ 24 × 31 = 744 credits per 10-min live pull, so a busy Saturday spends the 2,500-credit day in about three live pulls and then serves stale-flagged lines — raising the day cap or the Odds API plan is Josh's call, not made here. (b) Picks: live sides and live props are graded and shown under ALL (LIVE tag, no Kelly); `CfbPicks.liveRows`. (c) Parlays: twelve category sets (ML, SPREAD, TOTAL, ANYTIME TD, PASS TDS, PASS YDS, RECEPTIONS, RUSH YDS, REC YDS, COMBOS, MIXED live+pregame, LIVE), up to `CFB_PARLAYS.perCategory` = 50 distinct tickets each, leg counts spread 2–6 round-robin then EV order; the Board's Generated parlays section is a chip-row of the twelve categories with counts, phones mount tickets in chunks with "Show more" up to 50, desktop grid up to 50, a MIXED ticket badges only its live legs. Reference: `docs/cfb-desk.md` (Props INSTRUCTION 42 bullet, Picks bullet).
**INSTRUCTION 43 (2026-09-05, Josh's word, verbatim: "* It's still only showing Anytime TD picks for ARST @ MEM, WYO @ CSU, FIU @ USF, WMU @ MICH, SHSU @ TROY, BOISE @ ORE; They are still 12 games today that haven't started w/ current Anytime TD odds * It's also only showing 4 Anytime TD parlays in the generated parlays. It should be showing 50+ Anytime TD parlays * 'The Sharp' & 'Simulator' tabs can go back up right above Parlay Builder * 'Ledger' tab can be moved down to bottom of page right above Settings * Add color to the Tab titles (ie: Board, The Sharp, Simulator, etc)")**
Shipped same day (workflow: 3 builders, integrator, 3 review lenses, 17 confirmed findings — 15 fixed in code, 2 kept as behaviour and documented). Read on prod before the build: 46 priced games, 17 with rows; the 29 empties are FBS-vs-FCS games no book posts props on (legitimately empty); 11 games carried DK/FD Anytime TD rows with no Caesars quote because each was priced once and then rode its 2 h carry before Caesars posted. (a) Props — the CAESARS-MISSING RULE: an upcoming game with rows where some market has no Caesars quote on any row is re-priced every `CFB_PROPS.czMissingRevalidateSec` (30 min) inside `czMissingWindowSec` (4 h) of kickoff; zero-row games stay on the 2 h empty-event hold; live games re-price only when their own `pricedAt` is older than 10 min; re-pulls send `cache: "no-store"` (the Next data cache is stale-while-revalidate, so a shorter `revalidate` alone returns the old payload); a failed re-pull keeps the game's stored rows and old `pricedAt`; the answer carries `czMissing` / `noProps` and `ttlSec` drops to 30 min while any game is Caesars-missing; footnotes say "priced N of M games", "K games post player props at other books but no Caesars line yet — re-checked every 30 min inside 4 h of kickoff", "J games have no player props posted at the books we price". Aggregate cost stated honestly in rules.ts: 12 such games × 8 pulls × 31 = 2,976 credits > the 2,500 day rail — the rail binds, live games are served first. (b) Parlays — TIERED SETS: each single-market category fills to 50 with tier 1 legs (EV ≥ −3 at Caesars) first, then tier 2 legs down to `CFB_PARLAYS.setFloorEvPct` (−12); tickets carry `gated` (every leg cleared −3) and ungated tickets wear an "edge −" tag with their red EV, the PARLAYS tile counts them "below gate"; Anytime TD has its own band `setBands.anytime_td` = 2–4 legs, dec 4–250 (Caesars shades ATD, and the old 60× cap forbade a third leg). Tier 2 applies to ML/SPREAD/TOTAL too, on purpose (INSTRUCTION 42 asked for 50 under every category). (c) Nav: top rail is Games, Stats, Board, Builder, The Sharp, Simulator, Parlay Builder, Parlay Calc; bottom is Ledger, Settings; every tab has its own tone colour on label, icon and active pill (Board keeps the lime brand); phone bottom bar unchanged in membership. Reference: `docs/cfb-desk.md` (INSTRUCTION 43 bullet); pins in `tests/cfb-props-route.test.ts`, `tests/cfb-picks.test.ts`, `tests/cfb-picks-ui.test.ts`, `tests/cfb-props-ui.test.ts`, `tests/nav.test.ts`.

**INSTRUCTION 44 (2026-09-05, Josh's word, verbatim: "* no they should be using in game lines as well; make it also use in game prop lines for all of the same props as they are available; which is through 3rd quarter in most games * Lets also fix the iOS simulator helper so we can use it")**
Shipped same day (workflow: builder, 2 review lenses, 4 confirmed findings, all 4 fixed). Why the ANYTIME TD set read 0 on prod at 16:55 PT: Caesars carried Anytime TD only on two LIVE games and every single-market set and COMBO built from pregame legs alone (live legs fed only MIXED and LIVE). Now `buildCfbPicks` (`src/lib/cfb/picks.ts`) draws every single-market set (ml, spread, total, anytime_td, pass_tds, pass_yds, receptions, rush_yds, rec_yds) and COMBO from pregame AND in-game Caesars legs — tier 1 at EV ≥ −3, tier 2 (single-market sets only) down to `setFloorEvPct` −12 — one leg per game, bands and gated-first fill order unchanged, tickets ranked by EV with no bias for or against a live leg. MIXED stays live + pregame, LIVE stays live-only, the legacy SAFER / LONGSHOT / MIX view stays pregame-only. Each `CfbParlay` carries `liveLegs`; a ticket with any live leg is never a paper stake (no Kelly). Review fix: the twelve sets are kept disjoint by an `emit()`-level cross-set `taken` key set with precedence in `CFB_PARLAY_CATEGORIES` order (single-market > combo > mixed > live) — the first build emitted 24 / 32 / 22 duplicate leg sets between COMBO and MIXED / LIVE at 2 / 3 / 6 live games, which the PARLAYS tile would have double-counted. Board (`CfbPicksBoard.tsx`): a ticket in one of the ten pregame categories with `liveLegs` > 0 wears an "N in-game" chip (`InGameTag`, `data-testid="cfb-parlay-ingame"`; MIXED and LIVE tickets do not, their names already say it), the PARLAYS tile appends "· N with in-game legs" over those ten sets only, and the ten blurbs say the lines are Caesars' pregame and in-game. Pinned in `tests/cfb-picks.test.ts` (the "INSTRUCTION 44" describes: an ATD slate whose only Caesars rows sit on two live games fills the set to 50 two-leggers, every leg live, `liveLegs` exact; disjoint-sets pin at 2 / 3 / 6 live games; the INSTRUCTION 42 / 43 pins that encoded "pregame only" for the sets were rewritten) and `tests/cfb-picks-ui.test.ts`. Doc bullet in `docs/cfb-desk.md`. The "through 3rd quarter" clause is the books' behaviour, not a rule here: a live leg is admitted while Caesars still posts it; nothing in the engine cuts at a quarter. Simulator helper: Claude desktop's `claude-ios-sim` crashed once (18:10 PT, first screenshot after a fresh boot, Core Image / Metal binary-archive nil crash) and has worked since — attach, screenshot, tap and HOME verified 18:15 PT on the iPhone 17 Pro Max (iOS 27.0 runtime, Xcode 26.6); root cause unproven, nothing in the repo changed for it.
**INSTRUCTION 45 (2026-09-06, Josh's word, verbatim: "Parlay Lab CFB should've been running the same $150 per day theoretical Core money and $25 Fun money per day")**
Shipped 2026-09-06 after 14 review rounds (the last deliberately REDUCTIVE — delete / correct / pin only, no new machinery, because eleven consecutive rounds had each introduced a fresh defect inside the fix). ROOT CAUSE: the CFB paper constants had existed since the desk was built — `CFB_PAPER = { since: "2026-09-05", daily: 150, fun: 25 }` (`src/lib/cfb/rules.ts:29`, read 2026-09-06) — but NOTHING EVER LOCKED THEM SERVER-SIDE. A CFB card existed only if a browser built one, so prod `GET /api/cfb/ledger` held 0 entries while MLB's `/api/ledger` held 22. The money was configured correctly and never deployed. NOW SHIPPED: a server lock rail mirroring MLB's — `app/api/cfb/lock/route.ts` (GET, `force-dynamic`; gate order 503 if `CRON_SECRET` unset → 401 unless `cronHeaderAuthed(req)` → 503 unless `storeEnv()`; reads only `?date` and `?dry=1`, NEVER a key from the query string; store keys pinned as literals `pl:cfb:ledger:v1` / `pl:cfb:bank:v1`), `src/lib/cfb/lock-server.ts` (`decideCfbLock`, `buildCfbLockEntry`, `buildCfbSweepEntry`, `decideCfbTopUp`/`planCfbTopUp`/`applyCfbTopUp`, `overlayCfbGrading`, `cfbSettleCandidate`/`cfbSettleReady`, `assertCfbEntryMoney`/`assertCfbCardMoney`), and `src/lib/server/cfb-lock-forward.ts`, which `app/api/scheduler/route.ts` calls AFTER the untouched MLB tick — `mlbTick` was proven byte-identical to HEAD's GET body (brace-matched extraction, 174 lines each, sha1 `6f4e5ca4abf2fcf7a5ec7f235aaded9343069478` on both; numstat on that file `74 0`, zero deletions), so the MLB rail is wrapped, never edited. New constants in `rules.ts`: `CFB_LOCK` (`leadMs` 60 min — the lock window is [first kickoff − 1h, first kickoff]), `CFB_SWEEP_DAYS` 3, `CFB_TOPUP_MAX` 2, `CFB_TOPUP_RETRY_MS` 45 min, `CFB_SETTLE`, `CFB_UNGRADABLE_MS` 48 h, `CFB_VOID_RECHECK_MS` 7 d. THE MERGE KERNEL IS NOW SHARED BY BOTH DESKS: `src/lib/ledger-merge.ts` grew `unionCore`/`unionFun`/`allotmentCap`/`funCap` plus the `coreDropped`/`coreDroppedPL`/`funDropped`/`funDroppedPL`/`stakeConflict`/`betConflict`/`capBreach` receipt channels, and MLB rides the same code — MLB behaviour was re-pinned, not assumed. TWO REAL MONEY DEFECTS were found and fixed in the last round and independently re-verified by probe in both merge orders: (1) two copies of one day EACH holding exactly the allotment merged to $170 (over) or $130 (under) — both directions now land on exactly $150 and are idempotent across repeated sync cycles; (2) a RIVAL lock's receipted stake could cross onto this card unmarked whenever it fit under the allotment — it is now refused and named on `stakeConflict {kept, refused}` with `betConflict` naming the disputed id. Also closed across the rounds: the core desk ceiling (`allotmentCap` clamps like `funCap`), the `$25 phantom loss` on ungraded drops, the unconditional verdict withdrawal that deleted honest grades (MLB was $45 wrong per sync into `computeBankroll`/`ticketKelly`), a stale 48 h VOID destroying a corroborated corrected grade, and a `NoIndex` closure on `CfbLedgerEntry` so it no longer inherits `SyncEntry`'s index signature. `placed` is still never written on the CFB rail; there is still no `lockedBy` field anywhere and none was introduced. WHAT THE MONEY ACTUALLY DOES: on the repo's own 2026-09-05 fixture the lock seats core $75 of $150 (three tickets at the `maxStake` $25 cap) and fun $25 of $25 — NOT a bug and NOT money lost: `CFB_RULES` (`minEvPct` 2, `maxStake` 25, `oneLegPerGame`, `forcedMaxDec` 1.75, `tickets` 3–7) is unchanged and only three games on that slate cleared the bar; the card discloses it on its face ("$75 of the $150 stayed undeployed — every ticket is at the $25 max and no other game offers a side >= 0% EV under 1.75") and the top-up decider fires but plans $0 because the same board offers nothing else. WIDENING THOSE RULES IS AN OWNER DECISION NOT YET MADE. **THE OPEN ITEM THAT DECIDES WHETHER ANY OF THIS DEPLOYS:** `vercel.json` declares exactly two crons — `45 21 * * *` and `0 0 * * *` (21:45Z / 00:00Z = 14:45 / 17:00 PT), `grep -ic cfb vercel.json` = 0, both verified 2026-09-06 — and BOTH LAND AFTER the fixture Saturday's window (12 games kicking 16:00Z/16:30Z ⇒ window 15:00Z–16:00Z), so that Saturday would be written down as a $0 no-play PERMANENTLY, because a date locks once. The rail depends on the EXTERNAL cron-job.org "scheduler" job (`docs/cron-jobs.md:17`, every 15 min, hours 15–23 and 0–2 UTC, header `x-cron-key`) which THIS REPO CANNOT VERIFY — and that doc's own red banner records four earlier entries carried as live for ten days that were never created. `tests/vercel-cron-pokes.test.ts` pins the Hobby two-cron cap, so a third cron is not free. Nothing about the schedule was changed: it is Josh's call and the options are written down in `docs/cfb-desk.md`. KNOWN LIMITS SHIPPED WITH IT (all recorded in `docs/cfb-desk.md`, none blocking): two independently locked copies naming ONE game under different ids both seat ($50 on one game, violating `CFB_RULES.oneLegPerGame`) because the shared kernel dedupes by ticket id only — the single server lock path does not produce it; `stakeConflict` carries no reason code, so a receiptless refusal, an allotment refusal and a rival refusal are indistinguishable to the UI; `pickBase` still breaks ties on serialised JSON byte length; 16 tracked `" N."` duplicate byte-copies sit in the tree. THE CITATION GUARD ADDED IN ROUND 13 WAS DELETED, NOT REPAIRED: its comment scanner popped the template-literal frame on the first `${...}` close, so any single-line template with ≥2 substitutions desynced it and it read nothing further in that file — measured blind on ~44% of tracked code files while `docs/cfb-desk.md` claimed it "reads every tracked source comment". A guard that overstates its own coverage is worse than none; the class is now guarded by REVIEW ONLY and the doc says so.
TWO FALSE CITATIONS were found by the final verification pass, reported rather than shipped, and then FIXED before the commit (the kernel's stake-warning comment justified itself by naming three files as verbatim carriers of a string two of them carry zero times; `src/lib/cfb/store.ts` quoted that warning in its pre-deletion form beside a stale test count, re-measured to "Tests 20 passed (20)"); `docs/cfb-desk.md` limit (xv) records both as CLOSED. That is the first round in four where a self-reported citation defect did not ship. GATE, measured 2026-09-06 on a hash-bracketed quiescent tree (23 paths, IDENTICAL open to close): `npx tsc --noEmit` exit 0 and `npx vitest run --no-file-parallelism` **156 test files, 2151 tests, all passed**, duration 175.77s; the two post-gate comment corrections were re-verified with tsc exit 0 and 4 files / 306 tests green.
**INSTRUCTION 46 (2026-09-08, Josh's word, verbatim: "Parlay Lab Baseball 1. Core Money should be calibrating itself more often. It is doing horrible. Should consider doing some higher $ 2 team parlays. Hypothetically could be 2 $60 2 leg parlays one day w/ 3 $10 3-4 leg parlays one day, 5 $30 2 leg parlays the next, 3 $40 2 leg parlays w/ $20 3 leg parlay & $10 4-5 leg parlay the next, 4 $30 2 leg parlays and a $30 3-4 leg parlay the next, $90 2 leg parlay w/ $40 2 leg and $20 4 leg parlay the next, $75 2 leg parlay w/ $50 2 leg, $15 3 leg and $10 5 leg parlay the next etc / Parlay Lab College Football 1. Board & Builder should have player headshot as well as team logo 2. Don't like how Parlay Calc is centered in screen and is the only page that is centered in screen on desktop. Would rather have it be aligned left side of screen and be larger extending right. Just looks weird. 3. On all pages, I like the logos with all of the picks but it should be the team logo the player plays for not both team logos so it's easier to separate the players & teams. Keep the matchup below the player name on anytime TD etc keep everything the same besides changing the two team logo to just the logo of the team the player is on 4. Builder UI on phone app version is atrocious (screenshot 9/7/26 4:11pm) 5. On 'Board' tab, The engine description below 'Refresh MLB' button should be expandable/collapsible to reduce space it takes up initially 6. On 'Games' tab on phone app version, game boxes can be significantly smaller to fit more on one screen. They can also be expandable/collapsible. I would start with them collapsed how they are and allow them to be clicked to expand down to show box score preview. To go to full box score, just click 'box score' button in top right of each box 7. 'Preview' should be named 'Game Preview' and has avg, ops stats but no AB, R, H, RBI, etc. it should also have batter vs pitcher matchup data on that page 8. On 'Parlay Builder' when looking at a game's prop bets say Giants/Rockies H+R+RBI there should be 3 buttons: All, Giants & Rockies. If I click Giants or Rockies it only shows that teams available picks for that prop etc 9. On Ledger, when expanding/collapsing an individual parlay or pick it should collapse no matter where you click inside that singular box; besides clicking the players name in the bet which should take you to that bet if it is currently available pregame or live; even if the line has changed 10. Should be evaluating season long props and season long prop parlays so I can mess around and have fun with a bunch of season long tickets; win totals, receiving yards overs, Pass Yards/TDs overs, rushing yards overs, rush/receiving TDs overs etc")**
Shipped same day (workflow: 7 builders A–G on one tree with disjoint file ownership + 14 verifiers, then a fix round of 7 fixers + 7 verifiers; the last two defects — stranded-money double count, Season Lab scroll target — fixed by hand). Items 5/6/7/8 are MLB surfaces despite the CFB heading (Refresh MLB, box scores, Giants/Rockies); 9 is both ledgers; the 9/7 4:11pm screenshot never reached the Mac, so item 4 was diagnosed from the pre-change source and checked at 375px. GATE: tsc clean, 165 files / 2358 tests, the 2 reds were this doc's stale STATE-CLAIM sha and a split `@/lib/sport` import pin, both fixed and re-run green.
- MLB core is a SHAPE now (`src/lib/core-shapes.ts`, `CORE_SHAPES_SINCE 2026-09-08`): Josh's six $150 examples verbatim (A 2×$60 2L + 3×$10 3-4L · B 5×$30 2L · C 3×$40 2L + $20 3L + $10 4-5L · D 4×$30 2L + $30 3-4L · E $90 2L + $40 2L + $20 4L · F $75 2L + $50 2L + $15 3L + $10 5L), rotated by UTC day index; `lock-card.ts` fills one ticket per slot (slot stake, slot leg range, dec ceiling by legs 2.6/5.36/9.38/16.41, forced 1.75). ONE TICKET RIDES TO THE FULL SLOT STAKE ($90 on a Kelly-$12 pick) — my reading, REVERSED by Josh the same day (INSTRUCTION 46a below: cap at Kelly). INSTRUCTION 18's $25 cap / 2-leg max / 3-7 tickets are superseded; `PAPER_TICKETS {3,5}`.
- Self-calibration: `readShapeCalibration(date)` (wired at generate + scheduler backfill, fail-safe null) reads the trailing-30-day ledger into 2-leg vs 3+-leg buckets (singles NOT bucketed); once both hold ≥ 20 graded tickets and the ROI gap ≥ 10 points the rotation walks only the three shapes friendliest to the winning bucket. Below that: full rotation. Shape fields (`coreShape`, `shapeLine`, `slotsOpen`, `slotsUnfilled`, per-ticket `shapeSlot`) ride the public card view and survive `mergeDay` (adopt-first, never overwrite).
- Legacy carry: unstamped tickets seat best-fit by stake; a ticket bigger than every free slot is STRANDED (money counts, holds no slot) and the displaced slots read `cannot fill further`, which the scheduler's sweep treats as terminal (no more ~120-credit top-ups). Stranded money is netted ONCE — a block fire's `dailyOverride` already excludes it (the verifier caught the double count that would have frozen a legacy day at $0).
- Grading cadence: `GRADE_HOURS = [15, 18, 22, 2]` UTC (was 15/2) — four passes, all inside the cron-job.org poke window (15–23, 0–2 UTC). Hours outside the window can never fire; widening is Josh's cron-job.org call. SUPERSEDED the same day by INSTRUCTION 46b (five Pacific slots).
- CFB identity: ESPN `byathlete` `headshot.href` + `teamId` joined onto prop rows (`props-context.ts` → `props.ts`, gated to the two teams in the game, name-only match no longer trusted); `PlayerMark` (headshot over own-team logo) on board, builder, slip, ticket cards, ledger; `PairMark` only for game-level picks or when no team resolves; `card.ts legOf` copies player/headshot/pos/teamId onto ticket legs.
- Parlay Calc: left-aligned, two columns from `lg` (md leaves ~504px, too narrow). Board: engine overview collapsed by default (`pl:board:overview-open`). Games: compact collapsible cards, top-right Box score/Preview link ≥ 32px. Game Preview: AVG/OPS only pregame + batter-vs-pitcher table from statsapi `vsPlayer` (roster-filtered; "matchup data unavailable" vs "no probable" distinguished). Parlay Builder: All/Away/Home per game group. Ledger (both desks): tap anywhere toggles; player-name link deep-links `/props?tab=&mkt=&game=<gkey>&player=` (CFB: `/props?cfb=1&…` flips the sport) with `replace`.
- CFB Builder phone: money strip + stacked full-width tickets, notes collapsed, sticky ≥ 48px lock pill, headline `$coreSum` with `/ $150` as a sub-line.
- Season Lab (`/season`, CFB-only nav entry, `IconSeason`): win totals + season yardage/TD overs on HAND-TYPED lines (the Odds API lists no NCAAF season markets — verified 2026-09-08); fair numbers from ESPN byathlete pace + FPI; ¼-Kelly on a separate `CFB_SEASON.kellyBank 250`, `ticketMax 25`, `sameTeamHaircut 0.9`; own store `pl_cfb_season`; `paceOf` says on pace / behind / cleared / dead. Zero Odds credits.
- Still open for Josh: (a) enable a wider cron-job.org window if he wants grading more than 4×/day; (b) the INSTRUCTION 45 items (cron-job.org scheduler check, CFB allocation widening) remain his call.

**INSTRUCTION 46a (2026-09-08, Josh's word, verbatim: "Cap at Kelly, don't ride the full slot; I was just suggesting that if there was a reason for a ticket to be much larger than another one day that's okay")**
Shipped same day. The slot's stake is now a CEILING, not a target: a ticket carries the allocator's own sizing (Kelly under the disciplined modes, exact-sum on the forced pass), capped at the slot; `topUp` is always 0 from the slot pass (`withTopUp` kept for the merge contract and legacy stamps). The gap Kelly declined is RETIRED for the day, stamped `slotUnderSum` on the entry (accrued per fire; the merge keeps the larger copy) — a seated slot is a consumed slot. Every budget read goes through `dayConsumed(entry) = allocSum + slotUnderSum` (`blocks.ts`): block shares in generate, `decideTopUp`'s owed, the scheduler's terminal short-circuit — otherwise the sweep would spend ~120 credits per top-up re-filling slots that are already seated. `capResidue`, the shortfall note and the budget-over-bias yield all key off UNFILLED slots, never off Kelly sizing under a seated one. Consequence Josh should expect: most days will deploy well UNDER $150 (four Kelly-$12 picks in $60/$60/$10/$10 slots = $44), which is the point — the shapes say how big a ticket MAY be. Gate: tsc clean, 165 files / 2359 tests.

**INSTRUCTION 46b (2026-09-08, Josh's word, verbatim: "Widen the cron-job.org window to run grading @ 8am, 9:30am, 12pm, 3pm & 4:45pm; How do we increase the size of kelly? this is all hypothetical so cash flow can be much higher")**
- Grading: `GRADE_HOURS` (UTC) is replaced by `GRADE_SLOTS_PT = ["08:00","09:30","12:00","15:00","16:45"]` in `src/lib/server/grading-progress.ts` (since INSTRUCTION 49 the same five slots are also the refill slots — `REFILL_SLOTS_PT` is the same array); `decideGradePass` fires on the first scheduler tick inside [slot, slot+15 min) America/Los_Angeles (`ptMinutesOfDay`, Intl, DST-correct). **No cron-job.org edit was needed** — the existing ticker (every 15 min, UTC 15–23 and 0–2) already covers all five slots in both PDT (15:00/16:30/19:00/22:00/23:45Z) and PST (16:00/17:30/20:00/23:00/00:45Z); `tests/daily-grading.test.ts` pins every slot inside the window at both offsets. The 02:00Z night pass is gone; the 08:00 PT pass finishes the evening block. `tests/scheduler-route.test.ts` MLB_BODY reason re-pinned (18:27Z = 11:27 PT stays a non-grading minute). `docs/cron-jobs.md` has the slot table.
- Kelly size: the server lock priced Kelly off the legacy **$750** default — `app/api/generate/route.ts` boots the engine on an empty in-memory storage, so `SH.bankroll` fell to legacy L1065's fallback, and `lock-card.ts` L170 fell to a literal 750. The per-ticket ceiling is `kellyStakeMult 4 × min(¼ f*, 2%) × bankroll` = at most 8% of bankroll → **$60 at $750**, so the $75/$90 shape slots could never fill. The browser's managed bankroll already initialises at `BANK_BASE` **$2,500** (`src/lib/bankroll.ts`). Fix: `PAPER.bankroll = 2500` (`src/lib/paper-mode.ts`, pinned equal to `BANK_BASE` in `tests/paper-epoch.test.ts`), seeded into the generate route's engine storage as `pl_bankroll` (JSON, the way legacy `LS.get` reads it) and used as the lock's fallback. Ceiling is now **$200/ticket**; a typical 2-leg at ~+260 with f* ≈ 4.5% sizes ≈ $112, so Kelly still binds on the $90 slot when the edge is thin and the slot binds otherwise — INSTRUCTION 46a unchanged. Changing the number is one edit to `PAPER.bankroll`; the other levers (`kellyStakeMult` 4, the ¼ fraction, the 2% cap — all in `legacy/index.html`) were left alone because they change the discipline, not the cash.
- The Kelly figure ($2,500) was my choice to match the app's own bank; Josh the same day, verbatim: **"Bump the bankroll to $10,000"** — `PAPER.bankroll = 10000`, ceiling $800/ticket, so every shape slot is reachable and the slot is the binding cap on most tickets; Kelly still trims thin edges (46a). Pin in `tests/paper-epoch.test.ts` now `bankroll: 10000`, no longer tied to `BANK_BASE`.

**INSTRUCTION 47 (2026-09-08, Josh's word, verbatim: "1. Widen the CFB allocation to $250  2. NFL needs to be built NOW  3. Allocation should be set to $350")**
Shipped same day (workflow: 6 readers → 3 designers → 2 judges, then a spec writer, foundation builder A alone, builders B–H in parallel on one tree with disjoint file ownership, 3 verifiers, a fix round and an integrator; the first build run lost builder D, the verifiers and the integrator to the session limit and was resumed from cache). Item 3 read as the NFL allotment (it follows item 2); MLB stays $150. My decisions, recorded so they can be reversed: CFB $250 needed the ceilings to move too — `CFB_RULES.maxStake` 25 → 50 and `tickets` 3–7 → 3–10 (7 × $25 = $175 could never hold $250; kellyCap 0.02 × $2,500 = $50 = the new max); NFL `NFL_RULES` identical ceilings so 7 × $50 = $350; NFL Season Lab is CUT (the `/season` nav entry stays CFB-only); `vercel.json` untouched — NFL locks ride the existing cron-job.org ticker.
- ONE ENGINE, THREE DESKS: `src/lib/football/league.ts` (`LeagueConfig`, `DeskHandles`, `assertLeagueConfig`); `CFB_LEAGUE` is assembled at the bottom of `src/lib/cfb/rules.ts` from the existing `CFB_*` objects (every literal pin on that file still matches); `src/lib/nfl/rules.ts` is a pure constant module (`NFL_PAPER` {since 2026-09-10, daily 350, fun 25}, `NFL_MODEL` σ 13.5 / σ-total 13.5 / HFA 2.0, `NFL_BANK_BASE` 2500, `NFL_LOCK` 60 min / 25 s, `NFL_SWEEP_DAYS` 4, `NFL_SETTLE` 5 h, keys `pl_nfl_ledger` / `pl_nfl_bank2` / `pl:nfl:ledger:v1` / `pl:nfl:bank:v1` / `pl:nfl:oddsgap:v1` / `pl:nfl:props:*`, ids `nfl-<date>-…`, feeds `americanfootball_nfl` + ESPN `football/nfl` scoreboard (no `groups=80`) + powerindex `limit=40`). `src/lib/cfb/*` engine modules take `cfg: LeagueConfig` (default CFB at the client layer; REQUIRED at the server money seams `src/lib/server/football-lock.ts` / `football-props.ts` / `football-routes.ts`); `src/lib/football/store.ts` `makeDeviceStore(cfg)` with the CFB instance re-exported under today's names; `src/components/cfb/*` ARE the shared surfaces reading `useLeague()` (`src/components/football/LeagueContext.tsx`, default `CFB_DESK`); `src/components/nfl/Nfl*.tsx` are `<LeagueProvider desk={NFL_DESK}>` wrappers; `app/api/nfl/{,ledger,lock,props}/route.ts` are thin shells keeping their own literal keys and the CFB gate order. FPI columns are now read BY NAME (`fpi`, `fpirank`) — the old positional read would have taken `epaoffense` as the NFL rank. Merge kernel: `DESK_PAPER` keyed on the entry's sport (mlb 150 / cfb 250 / nfl 350). Scheduler: CFB and NFL forwards run under `Promise.allSettled`, the NFL forward imported on its own line from `src/lib/server/nfl-lock-forward.ts`; body gains `nfl`. Shell: `Sport` = mlb | cfb | nfl, three-pill `SportSwitch` (phone pills 29.5 px wide × 44 px tap height — narrow, accepted), `--color-nfl` blue, `useNflSyncBeacon();` beside the pinned CFB beacon, `?nfl=1` props deep link, NFL early-return under the pinned CFB line on every page.
- Fixtures: real ESPN captures in `tests/fixtures/nfl/` (scoreboard 2026-09-13 with 13 games, 2026-09-10 SF@LAR, a final-scores day, powerindex 32 teams, byathlete); odds and per-event props SYNTHESIZED in the CFB fixture shape (the Odds API was never called). Known: the synthetic moneylines are model-identical so no side clears +2 % on the fixture — the lock tests prove ids/band/cap on a sweetened in-test board.
- Tests: `tests/nfl-config.test.ts` (config invariants both leagues), `tests/nfl-separation.test.ts` (bidirectional: no `pl_cfb` / `pl:cfb` / `americanfootball_ncaaf` / `college-football` in the NFL tree, no `pl_nfl` / `pl:nfl` in the CFB tree, MLB keys in neither), plus nfl-engine / nfl-store / nfl-routes / nfl-lock-route / nfl-props-route / nfl-pages / nfl-money-ui / nfl-board-ui. CFB $250 re-pins across `tests/cfb-separation` / `cfb-lock-route` / `cfb-card` / `cfb-grade` / `ledger-merge` / `stats-scope` were re-derived by running.
- OPEN FOR JOSH: (a) London / Germany 13:30Z NFL kickoffs sit outside the cron-job.org ticker hours (15–23Z, 0–2Z; the lock window would be 12:30–13:30Z) — such a date is swept as a missed-window NO-PLAY claim row unless the ticker's hours are widened (his account); (b) `NFL_PROPS.measuredCreditsPerEvent` 31 is the CFB measurement — read `x-requests-used` on the first real Sunday (2026-09-13) and reset it; 13 games × 31 ≈ 400 credits per Sunday props pull under a 1,000/day NFL budget shared with CFB's 2,500 on the one Odds API account.

**INSTRUCTION 48 (2026-09-09, Josh's word, verbatim: "The Card for today is 'locked' which is fine, but it only played $25 today. I understand thats all it had meeting the criteria at this time which is completely fine. Throughout the rest of the day refresh, if it analyzes more picks/parlays that meet the betting criteria, it can continue to add to the card up to the daily allotted amount. It can lock multiple times per day, but it can never remove a pick it can only add to it")**
Read as: LOCKED means the first lock happened, not that the day is closed. The card keeps growing across the day's refreshes up to the daily allotment, and nothing already on it is ever removed or resized. The engine already appended on every desk — MLB rebuilds every fire off `carry = await getLockEntry(date)` (`app/api/generate/route.ts:437`) and returns `core = [...carried, ...newCore]` with carried copied verbatim (`src/lib/server/lock-card.ts:625-626`, `lockedAt` preserved :764, OVER THE DAY guard :749-751); football `applyTopUp` appends (`src/lib/cfb/lock-server.ts`) and `topUpDate` re-reads then replaces the day in place (`src/lib/server/football-lock.ts:583-588`). What stopped the card growing was only cadence, so the build is constants + two free refusals + an enforced invariant.
- Constants (line numbers as of INSTRUCTION 48; shifted by 49): `TOPUP_MAX` 2 → **4** and NEW `TOPUP_EMPTY_RETRY_MS` = **90 min** (`src/lib/paper-mode.ts:68` and `:72`); `CFB_TOPUP_MAX` 2 → **6** per arm (`src/lib/cfb/rules.ts:216`, `CFB_TOPUP_RETRY_MS` unchanged); `NFL_TOPUP` `{ max: 2 → **6**, retryMs: 45 min }` (`src/lib/nfl/rules.ts:116-119`). `MAX_RUNS_PER_DATE` stays 4, so the MLB hard ceiling is 4 + 4 = 8 spending runs a date (`app/api/generate/route.ts:57`, `:208-213`).
- THE INVARIANT: new pure module `src/lib/append-only.ts` — `appendOnlyViolation(prev, next)` returns null when every ticket id in prev.core ∪ prev.funT is present in next with a numerically equal stake, else the first violation as a sentence naming the id and both stakes; `assertAppendOnly(prev, next, where)` throws `APPEND ONLY (where): …`. A raised stake is a violation too (a resize is neither "add" nor "keep"). Four throw sites, each before any write: (1) `lock-card.ts buildLockEntry` right after the OVER THE DAY guard (`assertAppendOnly(carry, { core, funT }, "buildLockEntry")`); (2) `lock-card.ts writeLock` after `mergeLedgers` and before the SET — both `writeLock/stored` (the stored day vs the merged day) and `writeLock/fire` (the fired entry vs the merged day), so a racing second writer makes the write throw, generate reports `lock: { error }`, the registry key is NOT written and the next poke retries on a fresh carry; (3) `lock-server.ts applyTopUp` before `assertEntryMoney`; (4) `football-lock.ts topUpDate` before the SET (its catch reports action "error").
- MLB `decideTopUp` (`src/lib/server/blocks.ts:234-297`) gains two FREE refusals after the existing five (no lock / fully deployed / pending block / every game started / `used >= max`), return shape `{ fire, reason, owed, used }` unchanged: (6) SLOT-FIT — via the new pure `openSlotStakes(entry)` (seats `entry.core` on `entry.coreShape` exactly as `seatCarried` does, returns the still-free slot stakes in shape order; null on a pre-09-08 day without `coreShape` → skipped): if no open slot is ≤ owed → `no open slot fits the day's remaining $<owed> — the smallest open slot is $<min>; the shortfall is Kelly sizing inside seated slots, not an unfilled seat`; if no open slot at all → `every slot of the day's shape is seated — nothing more to fill`. Exact because a sweep's budget is `effectiveBlockBudget("")` = owed once no block is pending and `ownSlots` owns any open slot with stake ≤ rem — otherwise the run would deploy $0 for 114–150 credits. (7) EMPTY-SWEEP COOLDOWN — the latest `topup-*` registry row with `tickets === 0` and `firedAt` inside `TOPUP_EMPTY_RETRY_MS` → `top-up <key> priced a board <m> min ago and seated nothing — the next sweep waits <w> more min rather than re-buy the same prices`; a sweep that seated anything never arms it (the board is moving). Scheduler passes `emptyRetryMs: TOPUP_EMPTY_RETRY_MS` (`app/api/scheduler/route.ts`).
- WHY `pending` (`blocks.ts:252-255`) AND `cannot fill further` (`scheduler/route.ts:248-273`) STAY, with a CORRECTION on the record: a block fire is a FULL-SLATE generate — `eng.collectSlate()` takes no scope (`app/api/generate/route.ts:306`, `src/engine/index.ts:162`), so the old "~50-91 credits a fire" comment is RETIRED; the measured figure is 114–150 per run (`generate/route.ts:46`). The hold is kept not because a block fire is cheaper but because the pending block's own fire already carries the whole deficit (`effectiveBlockBudget` = daily − reserved − dayConsumed, `blocks.ts:138-165`) — sweeping first buys a second full board for the same money and could seat a late-block game before that block's own gate has judged it; `canStillFire` already releases burned-down blocks. `cannot fill further` trips only when EVERY open slot carries that phrase, minted solely for stranded legacy tickets; tickets locked since 09-08 carry `shapeSlot`, so on a current day the list is empty and the sweep proceeds (`tests/scheduler-route.test.ts:369-421` pins the return shape).
- `dayConsumed` counts `slotUnderSum` — a day can read full under $150 by Josh's own 09-08 cap-at-Kelly rule ($25 Kelly-sized into a $60 slot consumes $60 of the $150; the day's open money is its OPEN SLOTS, never a seated slot's remainder).
- ODDS CREDITS (real money; the paper is hypothetical). MLB run 114–150 credits; football lines pull 6; ticker 48 pokes/day (every 15 min, UTC 15–23 and 0–2 — `docs/cron-jobs.md`, UNCHANGED, Josh's cron-job.org account; the last MLB sweep chance of a date WAS the 02:45Z poke — SUPERSEDED by INSTRUCTION 49 below: top-ups fire only on the five PT slots, so the last automatic refill is the 16:45 PT slot).
...
- UI (Josh reads "LOCKED" as final): `app/builder/page.tsx` renders `mlb-locked-filling` — "Still filling — $x of $150 core placed. The desk keeps adding while pregame games remain; a locked ticket never changes." — when core sum < daily and a game is still ahead; `app/ledger/page.tsx` `DayCard` header reads "n tickets · $deployed of $175"; `CfbBuilder LockedSummary` renders `cfb-locked-filling` under the same condition (NFL reuses the component).
- Tests: `tests/append-only.test.ts` (new); re-pins in `tests/paper-deficit.test.ts` (TOPUP_MAX 4, cooldown, slot-fit, `openSlotStakes`), `tests/scheduler-route.test.ts` (fourth sweep forwarded, fifth refused `/cap/`), `tests/lock-card.test.ts` (fourth top-up appends, never drops; buildLockEntry and writeLock throw APPEND ONLY), `tests/cfb-lock-route.test.ts` (cap now `CFB_TOPUP_MAX` rounds; applyTopUp throws), `tests/nfl-config.test.ts` (`NFL_TOPUP` `{ max: 6 }`, `CFB_LEAGUE.topUp`).
- FIX ROUND (2026-09-09, after two verifiers): (1) THE ENFORCEMENT POINT THAT MATTERS IS THE DEVICE SYNC ROUTE, not the server rails — `applyTopUp` builds `next.core = [...entry.core, ...plan.tickets]`, so the football asserts (sites 3 and 4) are contracts that cannot fail on their own inputs; the one writer that could shrink a locked day was a phone: a Builder lock minted before the server card reached it (same positional id namespace) wins `pickBase` on a graded early game and `mergeLedgers` drops every server ticket the phone lacks (the measured 2026-09-05 outcome). `restoreShrunkDays` (`src/lib/append-only.ts`) now runs in all three ledger PUTs (`app/api/ledger`, `app/api/cfb/ledger`, `app/api/nfl/ledger`) after `mergeLedgers` and before the SET: a locked stored day (football: `source === "server-lock"`; MLB: any locked day) that the merge shrank or resized is put back byte-for-byte and the reply carries it — both sync loops adopt the server's reply, so the phone converges on the server card. (2) `/api/clv` re-reads the store immediately before its merge and asserts `clv` append-only — its earlier stale read + whole-blob SET could overwrite a sweep's freshly seated tickets out of the store invisibly. (3) The merge kernel now unions football `topUps` by ordinal, appends `alt.core`, and carries `slotsOpen`/`slotsUnfilled` from the copy with the larger `allocSum`; `decideTopUp` also derives the next ordinal from `-topup<n>-` ids already seated, so a lost row can never re-mint a seated id. (4) A top-up sweep whose lock THROWS now writes its `topup-N` registry row as `{ tickets: 0, reason: "lock failed: …" }` in generate's catch, so it counts against `TOPUP_MAX` and arms `TOPUP_EMPTY_RETRY_MS` (the credits were spent at `collectSlate()`); the scheduler's orphan-block write is additive (re-read + overlay) so it cannot drop a concurrent `topup-N` row. (5) The CFB/NFL "Still filling" line renders only for a `server-lock` day with a game still ahead. (6) The credit table above is corrected: arms only close, so a football date buys at most `CFB_TOPUP_MAX` top-up boards (42 lines credits with the lock, not 78), a fixed-rails row is added, and the quota reading is dated and flagged stale.
- STATUS: built on the shared tree 2026-09-09, tests green as the tests item reports, **NOT committed, NOT deployed**. `.gitignore` carries an unowned `+.env*` line in the working tree that no builder made — do not stage it with this instruction without Josh's word. Open call: on the arithmetic above the month is ALREADY short; the first props-rail lever (`CFB_PROPS.dailyBudget` 2500 → 1500) was proposed pending Josh's word — ANSWERED the same day by INSTRUCTION 49 ("Don't lower any budgets"; Josh buys credits instead), so no lever is pulled. The every-tick top-up cadence of this instruction is superseded by the five-slot calendar below.

**INSTRUCTION 49 (2026-09-09, Josh's word, verbatim: "I can purchase more credits. Don't lower any budgets. I need high stakes days to really test the engine over time. It shouldn't be refreshing every 15 minutes. It should be 8am, 9:30am, 12pm, 3pm & 4:45pm. Other than that I can manually do it and it can function the same way whether I manually refresh it or it refreshes itself automatically")**
- VERDICT (design B manual-parity, with four grafts from design A): the slot gate lives INSIDE each desk's own server path from that path's own clock; Josh's manual click runs the IDENTICAL server pass with slot `"manual"`; the cooldowns are unwired rather than re-tuned; MLB top-up bypasses the 45-min generate limiter so a click right after a slot is honoured. Grafted from A: (1) the slot is RECORDED on every top-up row (MLB registry row `slot`, football claim row `slot`) so a second poke inside the same slot window is refused free; (2) `TOPUP_MAX` 4 → 6 (five slots + one manual); (3) a dedicated `POST /api/refill?desk=mlb|cfb|nfl` behind the sync phrase (`x-pl-sync`) — the click spends credits, so it is a POST, never a GET; (4) grading is no longer starved when a refill fires — the calibrate forward and the generate forward run together under `Promise.allSettled`. Rejected: a second "Refill card" pill (the existing Refresh pills become the manual path — one control per desk); the scheduler passing the slot to football (the lock routes compute it themselves so `?manual=1` and the ticker share one code path); `TOPUP_MAX` 5 (no manual headroom after a full slot day); keeping the football retry cooldown on the slot path (08:00 → 09:30 is 90 min, so any cooldown of ≥ 90 min measured from the 08:00 attempt could swallow the 09:30 slot whenever the 09:30 tick landed earlier in its window than the 08:00 tick did).
- WHAT CHANGES: the cron-job.org ticker still pokes `/api/scheduler` every 15 min (48/day) and `vercel.json`'s two crons (21:45Z, 00:00Z) still run — NOTHING changes on cron-job.org or in `vercel.json`. First locks are unchanged (MLB lineup/first-pitch blocks, football 60-min lead). What changes is the TOP-UP/REFILL pass: it fires only on the first tick inside [slot, slot + 15 min) for the five slots, or on Josh's own Refresh with his sync phrase stored, in which case the SAME server pass runs with slot `"manual"`. A refill refused BEFORE the pull (capped, same slot already ran, no paper lock, every game started, manual headroom) costs zero credits; an attempt that prices a board and seats nothing still costs that board — 6 credits on football, 114–150 on MLB — and, with the cooldowns unwired, may recur on the next slot. Budgets: none lowered.
- FIX ROUND (2026-09-09, the critic's pass): (1) a BLOCK FIRE on a slot tick no longer consumes the grading pass — `mlbTick`'s block branch runs the generate forward and `gradeForward` together under `Promise.allSettled` and the body carries `grading`; a block fire still pre-empts the refill on that tick by design (the block's generate seeds the card the refill would have topped up). (2) The scheduler forwards football CONCURRENTLY with `mlbTick` (`Promise.allSettled` started before the MLB tick, worst case max(25 s, 25 s, MLB) inside `maxDuration` 90) and carries its own `decideRefillTick(Date.now()).slot` as `?slot=` so all three desks decide the same slot; the lock routes accept only a slot named in `REFILL_SLOTS_PT` and otherwise fall back to their own clock. (3) generate's `topup` is honoured only for `scheduled` callers (cron key), and the unauthenticated vercel-cron user-agent fallback is deleted (401 unconditionally). (4) MLB claims its `topup-N` row (`reason: "in flight"`, stamped `slot`) BEFORE the spend and a second poke inside 5 min answers `skipped: "topup-claimed"`; the claim is not NX-atomic (accepted: the `K_RUNS` INCR still bounds the day). (5) MANUAL HEADROOM: the five slots and Josh's clicks draw from one pool of `TOPUP_MAX` attempts, so a click is refused free when `used + unstampedSlotsAhead >= max` — exact string `manual refill would spend a slot's attempt — <n> attempt(s) left, <m> automatic slot(s) still ahead today` (`manualHeadroomRefusal`, `slotsAheadPT`, `unstampedSlotsAhead` in `grading-progress.ts`); after 16:45 PT a click is honoured up to the cap. `TOPUP_MAX` still COUNTS manual attempts — a click before the last slot spends one of the six (flagged for Josh: raise `TOPUP_MAX` to 7 if he wants a click AND all five slots on the same date). (6) `POST /api/refill` wraps the desk forward in try/catch → 502 JSON `{ ok:false, error }` with the secret scrubbed; `refillReason` falls back to the generate body / football not-locked shapes so every click prints a line; the MLB pill re-runs the plain generate when the refill is refused for `no paper lock` / `every game started`. (7) Because a date's last slot is 16:45 PT and MLB blocks fire through the afternoon, MLB sees realistically 0–1 AUTOMATIC refills a date (the 16:45 slot, sometimes 15:00) — the rest are Josh's clicks.
- THE FIVE SLOTS (`REFILL_SLOTS_PT` is the SAME array object as `GRADE_SLOTS_PT`, `src/lib/server/grading-progress.ts` — one calendar for grading and refills; `decideSlotTick` / `decideRefillTick` beside `decideGradePass`):

| slot (PT) | PDT UTC | PST UTC (from Nov 1) |
|---|---|---|
| 08:00 | 15:00 | 16:00 |
| 09:30 | 16:30 | 17:30 |
| 12:00 | 19:00 | 20:00 |
...
| 16:45 | 23:45 | 00:45 |
  ...
  The vercel crons at 21:45Z / 00:00Z fall outside every 15-minute window in both regimes, so they never refill. The last automatic refill of a date is the 16:45 PT slot (not the 02:45Z poke of INSTRUCTION 48).
- Constants: `TOPUP_MAX` 4 → **6** (`src/lib/paper-mode.ts`; hard MLB ceiling `MAX_RUNS_PER_DATE` 4 + 6 = 10 runs a date); `TOPUP_EMPTY_RETRY_MS` keeps its 90-min value for the pure `decideTopUp` helper but is NO LONGER PASSED by the scheduler; `CFB_TOPUP_RETRY_MS` → 0 (identifier kept, inert); `NFL_TOPUP` → `{ max: 6, retryMs: 0 }`; `CFB_TOPUP_MAX` stays 6.
- MLB path: `app/api/scheduler/route.ts` `mlbTick` → `decideRefillTick(now)` + `decideMlbRefill` (new `src/lib/server/refill.ts`, which lifts the day reads and the cannot-fill terminal out of the scheduler) → `forwardMlbRefill` → `/api/generate?topup=1&slot=<slot>`; generate accepts `topup` above the 45-min limiter, rejects an unknown slot with 400, and stamps `slot` on the `topup-N` registry row. `decideTopUp` (`src/lib/server/blocks.ts`) gains the same-slot refusal after the cap: order is paper → owed → pending → every-started → cap → same-slot → slot-fit → (empty cooldown only if `emptyRetryMs` is passed) → fire.
- Football path: `/api/cfb/lock` and `/api/nfl/lock`'s already-locked branch gate on `decideRefillTick(now)` (or `?manual=1`) BEFORE any ESPN/props feed is touched; `topUpDate` carries `slot` into `decideTopUp`/`claimTopUp`, the claim row and the filled row keep it, and a same-slot claim is refused free. Body gains `refill: { trigger: "slot" | "manual", slot }`.
- Manual route: `POST /api/refill?desk=mlb|cfb|nfl`, header `x-pl-sync: <phrase>` (`syncAuthed`); 503 fail-closed without `CRON_SECRET`/store, 401 without the phrase, 400 on a bad desk; forwards server-side with `x-cron-key` (the secret never appears in a body or query; GET is not exported). The app: the MLB "Refresh MLB" pill and the football "Refresh Board" pill call it when the sync phrase is stored (`src/lib/refill-client.ts`, `useRefillDesk`), showing the server's reason as a muted inline note.
- Exact reason strings (shared by every desk): NOT-A-SLOT `not a refill slot (automatic refills run on the first tick after 08:00/09:30/12:00/15:00/16:45 PT; Josh's own Refresh runs the same pass any time)`; SLOT `refill slot <slot> PT — the scheduler re-prices and appends on the first tick after each of 08:00/09:30/12:00/15:00/16:45 PT`; MANUAL `manual refill — Josh's Refresh runs the same pass the slots run`; SAME-SLOT `refill slot <slot> PT already ran today — the next automatic refill is the next slot; Josh's own Refresh still runs any time`.
- Credits: MLB hard ceiling 10 runs × 114–150 = 1,140–1,500 a date, realistic 3–4 runs 342–600; CFB/NFL: slot-only ≤ 5 attempts = 30 lines credits; 6 with one manual = 36; 42 including the lock's own 6-credit pull. MLB: 9 runs / 1,026–1,350 without a click, 10 runs / 1,140–1,500 with one. INSTRUCTION 48's "first lever `CFB_PROPS.dailyBudget` 2500 → 1500" is COUNTERMANDED by this order ("Don't lower any budgets") — see `docs/credit-budget.md`. The last quota reading in the tree is still 16,480 remaining on 2026-09-05; no new reading was taken.
- STATUS: built on the shared tree 2026-09-09, **NOT committed, NOT deployed**. Fix-round gate (2026-09-09, run alone, after the docs were re-grepped clean of the guard-wiring plants): tsc clean; `npx vitest run` 181 files / 2724 tests, all green — tests/sha-references.test.ts included; no sha was added by this instruction. `.gitignore` still carries the unowned `+.env*` working-tree line that no builder made — do not stage it with this instruction without Josh's word.

**INSTRUCTION 50 (2026-09-11, Josh's word, verbatim: "1. Refresh button not working on 'Board' tab; works if I refresh on 'The Sharp' tab … 5. Need to be able to collapse list of picks for each individual game/prop by clicking/pressing in the top box that shows the team matchup" — all five items are reproduced BYTE-VERBATIM in the block immediately below, the worked example included, because that example is the evidence which settles item 3)**

```
1. Refresh button not working on 'Board' tab; works if I refresh on 'The Sharp' tab
2. It's not updating with live odds; it will show the player is top 4th w/ 3 H+R+RBI, but show them as an 'S' grade for over .5 H+R+RBI when their live over/under is 3.5 H+R+RBI
3. Parlay builder should have a generator that I can select # of legs, prop category, min & max odds then it will generate a parlay for me within those parameters; if I hit regenerate then it regenerates a new parlay; each slot is clickable to keep that player(s) in any round and spin the other slots
...
4. Need player headshots for Parlay Builder etc or need team logo next to name
5. Need to be able to collapse list of picks for each individual game/prop by clicking/pressing in the top box that shows the team matchup
```

- ITEM 2 — THE MECHANISM, AND IT IS **NOT** ITEM 1 (every line below re-read from disk this turn): `currentValue` (`src/engine2/grade.ts:142-177`) computes the live tally — the batter branch is `H + R + BI` at `:176` — and its return type at `:146` is `{ txt: string } | null`, so **the number is thrown away at the point it is computed**; only the display string survives. `src/lib/liveNow.ts:27` carries that same `{ txt; inning }` shape onward to the page. `gradeFromEv` (`src/lib/grade.ts:19-29`) is a pure threshold on ONE number against `GRADE_CUTS` (`:17`, `S: 6`) — it takes no line, no clock, no game state, and cannot know a leg has already won. `app/board/page.tsx` then renders the live tally in the Pick cell and `GradeChip grade={gradeFromEv(…)}` in the Grade cell of the SAME table (grade column at `:325-335`). The app holds "3 H+R+RBI" and "line 0.5" on one render and never compares them. That is the whole proximate cause, and fixing it costs nothing: the boxscore is already polled on the 60-second live loop, so no new fetch and no credit is involved.
- ITEM 2 — VARIANT A, what Josh most likely saw: a board generated pregame, `czEv` frozen at the value `legacy/index.html:2531` computed then (`shEV(r.p, czDec)`, emitted as a PERCENT at `:2544` via `Math.round(czEv*1000)/10`, which is why the `S: 6` cut means +6%), and never re-read afterwards — `useBoard` is `staleTime`/`gcTime` `Infinity` (`src/lib/useBoard.ts:15-16`), `MAX_AUTO_RUNS_PER_DAY = 0` (`src/lib/board-stale.ts:64`), and the last automatic refill slot of a date is 16:45 PT. An S minted at 11am is still on screen in the 4th inning, because nothing re-priced it.
- ITEM 2 — VARIANT B, latent today and the decisive finding: the live-resume sim seeds each leg with the player's REAL tally so far (`legacy/index.html:2134-2138`), stamps `liveInit` (`:2150`), and `:2402` then OVERRIDES the model probability with `pO = shClamp(spL, 0.001, 0.999, "2402")`. For a player already past his line that clamp is certainty, and the grade it produces is arithmetically correct: `shEV` (`:1705`) is `p*(dec−1) − (1−p)`, so at `p = 0.999` the percent EV is `(0.999·dec − 1)·100`, which clears the +6 S bar at `dec ≥ 1.061061` — **american −1638** by the repo's own `decToAm` (`src/lib/ticket-math.ts:27`). In plain terms: at ANY price softer than about −1640, a leg that has already cleared its line mints a genuine, correctly-computed S. Variant B is rare TODAY only because the Board's refresh is broken.
- ITEM 2 — **BINDING SEQUENCING:** the settled-suppression ships in the SAME change as the refresh fix, never the refresh alone. Fixing item 1 first would turn every mid-game tap into an S-minting machine — the refresh would re-run the live sim, variant B would fire on every settled leg, and the bug Josh reported would get worse before it got better.
- ITEM 1 — FOUR CAUSES, all four fixed, all four verified this turn: (1) `app/board/page.tsx:594` falls back to the browser regenerate ONLY when `fired === false` AND the reason matches `/no paper lock|every game started/` — every other refusal (capped, same slot already ran, manual headroom, not a refill slot, …) resolves with NOTHING re-priced; (2) `refillDesk` (`src/lib/refill-client.ts:16-18`) returns `{ status, body }` for ANY http status and throws only when no sync phrase is stored, so 401 / 502 / 503 all arrive as mutation SUCCESS carrying no `fired` field, and the tap dies in silence; (3) the pill label at `:601` reads `regen.isPending` only, so a refill in flight shows the idle text and the button looks dead under the finger; (4) **the actual reason "refreshing on The Sharp works"** — the Board's pick rows come from a MOUNT-ONLY effect, `fetch("/api/picks")` at `:212` with `}, []);` at `:221`, so navigating to /sharp unmounts the page and coming back REMOUNTS and refetches. `app/sharp/page.tsx:192` is an unconditional `regen.mutate()`, but that is not what Josh was observing on the Board: NAVIGATION refreshed his rows, the button never did. The fix: the picks read becomes a `["picks"]` query invalidated by both the refill and the regenerate, and a tap falls back to `regen.mutate()` on ANY `fired === false`, ANY non-2xx status, and on `onError` — so a tap can never resolve with nothing re-priced.
- ITEM 1 — CREDIT CONSEQUENCE, stated plainly and NOT gated: the unconditional fallback means a tap the server refuses now runs a browser generate, **~114–150 Odds credits, `GEN_CREDITS_EST` 140** (`src/lib/engine-client.ts:171-172`). That spend is COUNTED FOR VISIBILITY and printed under the pill on every click; it is deliberately NOT blocked, per the counter's own standing note at `src/lib/engine-client.ts:158` — "to make the spend VISIBLE, never to block it: the price-age lock guard depends on being able to regenerate, and nothing should stop a bet". **No budget, cap, allotment or slot is lowered by this instruction and no cooldown is added:** `TOPUP_MAX` 6, `MAX_RUNS_PER_DATE` 4, the five PT slots and every football budget are untouched, as is `docs/cron-jobs.md`.
- ITEM 3 — THE ODDS BAND IS PER-LEG, AND JOSH'S OWN EXAMPLE PROVES IT (computed this turn with the repo's own `amToDec`/`decToAm`, `src/lib/ticket-math.ts:25,27`): −145 → 1.689655, −124 → 1.806452, −137 → 1.729927, −130 → 1.769231; the product is **9.341931, i.e. +834 combined**. His stated range converts to −152 → 1.657895 and +110 → 2.100000. **+834 is nowhere near that range, and all four of his legs sit inside it individually** (1.657895 ≤ each of the four ≤ 2.100000). So "-152 -> +110" is only self-consistent as a PER-LEG band, and the earlier brief's reading of it as a combined-parlay band is wrong. SHIPPED: per-leg is the default and primary control; a combined-payout target is an explicit optional second mode on the same predicate, off by default. **OPEN — one line for Josh, the only thing this instruction asks him:** "your four legs multiply out to +834, so I read '-152 → +110' as the range each SINGLE leg has to sit in — right?"
- ITEM 3 — the generator itself is pure and seeded (`src/lib/parlay-gen.ts`): exact leg count, the prop-category picker IS the market rail's own setter (one state, so the sheet and the list can never disagree), a canonical sort by leg id so the same settings and the same spin give the same ticket on any device, typed failure codes that each print one honest sentence, pinned slots that survive a respin, one leg per player NEVER relaxable, one leg per game on by default and never silently relaxed. Prices come only from sides the book actually posted; a side that is not posted is skipped, never invented. Sandbox only — no ledger write, no credits, and it never touches `/api/refill` or `/api/generate`.
- ITEM 4: an MLB `PlayerMark` — headshot disc with the player's OWN team logo badged in the corner, initials on error — the MLB twin of the CFB mark Josh already approved, on every generator slot and on `src/components/mlb/ParlaysSection.tsx:212`, which prints a bare label today. Trap recorded for whoever touches it next: `TEAM_TAG_ALIAS` (`src/components/props/props-model.ts:97`, `OAK` → `ATH`, `CHW` → `CWS`) and `espnLogoCode`'s reverse specials must be folded through ONE path, or Athletics and White Sox rows silently lose their logo.
- ITEM 5 — THE COLLAPSE ALREADY EXISTED, AND WAS A 36px TARGET: `src/components/props/GameCard.tsx:34-38` is already a real `<button>` carrying `aria-expanded={open}`, and both Builder cards already use it (`GameCard.tsx:145` with `useState(true)` at `:138`; `src/components/props/PlayerRow.tsx:213` with `useState(true)` at `:201`). Josh's report is real but the defect is smaller and more specific than "it does not collapse": the header is `h-9` — **36px**, under this repo's own 44px bar (`app/board/page.tsx:640` is `min-h-[44px]`, pinned by `tests/board-overview-toggle.test.ts`) — it has no `press` feedback, so a tap that DOES work gives nothing back, and the open/closed state resets on every remount (a deep link, a market change, a revisit). Fixed to `min-h-[44px]` + `press`, `aria-controls` emitted only while open, a line count on the market card header so a collapsed card still says what is inside it, and a localStorage-backed collapsed set so a collapse survives a remount and a reload. Two things NOT done, both flagged for Josh: the Board tab has no per-game grouping at all (nothing to collapse there — building it is a new feature, not this fix), and the default stays OPEN for a game key never seen before.
...
**INSTRUCTION 57 (2026-09-13):** Josh requested arbitrary-depth undo/forward through generated parlays, substantial Board longshot diversity (Zonovan Knight must not anchor every ticket), and headshot/team-logo marks that are not blank. Implemented session snapshot navigation in the shared generator, with separate slip undo and a saved-quote notice. Regenerating after going back discards the forward branch; changing date/sport or leaving the page ends this session history. Football Board composer now selects against shared player exposure, caps repeat counts at one third of requested ticket capacity rounded up, and applies a separate selection pass to each category's longshot subset. Thin candidate pools may yield fewer tickets, with actual repeat counts visible on the Board. Exact 50-ticket expectations now test bounded size and exposure instead; price, gate, leg-count and deterministic ordering contracts remain tested. Roster lookup supplies verified team/headshot identity whenever the generator opens. Image fallback is headshot → team logo → initials when neither image is available. No player/team is guessed. Sandbox navigation never writes a ledger or places a bet; automatic paper budgets and legacy model remain unchanged. Browser fixture verified eight backwards/eight forwards, branch behavior, loaded logos and no overflow at 390px. Full gate/deployment recorded in the dated release artifact.

**INSTRUCTIONS 58–59 (2026-09-13):** Josh requested player headshots from Roster Lab on every prop-pick surface, with the player's club logo in the top-left corner, and then requested a less bland, more exciting generator with lower perceived effort. Roster Lab's public imagery implementation was read from `/Users/josh/Documents/Roster-Lab/src/sports/football/headshot.ts` and `src/components/ui/Avatar.tsx`; no Roster Lab account data or code was modified. Parlay Lab now resolves public ESPN roster IDs, uses ESPN portraits first and a name/team/position-matched Sleeper NFL fallback, then retains the existing team-logo/initials fallback. The small normalized catalog is cached hourly; per-mark callers share requests. CFB requires a bounded team scope. Shared marks cover football Board, prop rows, slips and tickets; MLB Board labels, prop rows, slips and ledger legs gain the same portraits. The generator has a studio hero, stronger dice action, portrait cards, combined-odds hierarchy and reduced-motion-aware ticket reveal. Existing styles/filters/pins/history remain. Browser-tested on a removable illustrative fixture at 390px and 1280px; real public catalogs tested locally. The fixture is removed before release. Financial logic, budgets, probability model and account integrations are unchanged. Final gates and deployment are recorded in the dated review artifact.

**INSTRUCTION 64 (2026-09-14):** Persistent sportsbook selection and quote-specific value across odds views. Preserve original locked paper records; missing selected quotes never borrow another book. Full implementation and validation details: docs/progress.md and dated review-2026-09-14.

**INSTRUCTION 60 (2026-09-13):** Josh requests full $350 NFL Sunday core and a retroactive Week 1 card. Sunday paper policy begins 2026-09-20; ranks verified pregame game-side/total singles by EV across up to ten distinct games, full-budget equal weighting, including negative EV, with explicit cohort markers. Thin slates can exceed normal per-ticket stake cap. No fabricated/unavailable/started prices. Week 1 is an isolated retrospective replay, using actual 15:55:36Z historical odds and archived September 8 ESPN ratings; ten $35 singles, not a rewritten pregame lock. The original live ledger/fun card remains intact. NFL Ledger exposes prospective P/L/Brier and the separately graded replay. NFL probabilities are not automatically refit by this change. Release proof in review-2026-09-13.

**INSTRUCTIONS 61–63 (2026-09-13):** Josh extends core paper action to MLB/CFB, reports Simulator showing baseball under NFL, asks for a cross-app sport audit, and wants readable NFL text. MLB experimental primary now uses full-sized probability-ranked slots from September 13 (new days: three $50 two-leg slots); preserves carried tickets/shapes and the disciplined alternate. CFB full $250 core begins September 14. Other scope: sport-aware Simulator/model explorer, home bankroll/results, Ledger hook isolation, Season Lab gate, MLB board-fetch/regeneration gate, and bright neutral NFL text with blue accents. Core action does not imply profitable edge; missing real candidates remain explicit. Full scope and proof recorded in dated progress/review.

**INSTRUCTION 65 (2026-09-16):** Josh requested missing MLB Board prop categories, live suggestions and Pregame/Live/Mixed generator modes. Implemented real RBI/Runs browse quotes, stored-board fallback and active/fresh live projection; Mixed requires both phases. No pregame model probability is reused at a live line. Engine selection and locked ledger remain isolated. Josh explicitly approved the obsolete regression updates and deployment on September 16. Full gate: 3,278 tests in 211 files pass; see progress and dated handoff proof.

**INSTRUCTION 66 (2026-09-16):** Josh: "in parlay generator, need to be able to exclude a player when they pop up if i've already taken them on other tickets etc. should be able to check a box that excludes until filter is changed or something". Shared MLB/NFL/CFB checkbox removes the player across lines and future spins, clears their pins, and exposes a checked list for individual restore or Clear all. Exclusions reset when generator filters, board or sport change and are never saved into reusable setups. History cannot bypass exclusions when adding a ticket; regeneration releases excluded pins restored by recall. Mobile QA verified these behaviors with 44px targets and no overflow/errors. Temporary QA route removed.

**INSTRUCTION 67 (2026-09-17):** Josh: "See all updates done by GPT-6 Astra on website and in Parlay Lab Handoff before proceeding. I do not want you to change anything that is currently on the website besides exactly what I explain. I want all parlays etc calculated based on DraftKings odds instead of Caesars. So everything the exact same with the engine/website except whenever it uses Caesars to grade, create an edge %, kelly stake etc replace Caesars with DraftKings and calculate every single thing based off of DraftKings. That includes Builder, Board, Parlay Generator, etc". Shipped: one constant, `SETTLE_BOOK = "draftkings"` in `src/lib/sportsbook/books.ts`, is the book every grade, edge %, EV and Kelly stake is computed at. The legacy MLB engine still reads its `CAESARS_KEY` variable, but every `createEngine` caller (engine-client, /api/generate, the scheduler) now passes `settlementBook: SETTLE_BOOK` and the facade rebinds that variable, so the `cz*` fields on rows, parlays, tickets and the lock card are DraftKings numbers; `attachBookQuotes` stamps `settlementBook` on the board and its rows, lock-card tickets and CLV pending legs carry it, and the client repricers key their "already priced at this book" short-circuit on that stamp instead of the default book, so a Caesars-priced board cached before this release is repriced at DraftKings from the quote index until the next refresh slot rebuilds it. Football (CFB + NFL model, props, live props, season lab, sharp board, UFC helper) flipped the `settleBook` constants; the parlay generator's "book-only" filter follows the selected sportsbook (DraftKings by default) and no longer hard-codes CZ; the sportsbook selector's storage key changed so every phone opens on DraftKings. Selection was NOT changed: the legacy engine's `shBasisPick` (DK/FD selection basis) is untouched, no budget, slot, cap or tier moved, football's cached board rows are still served as stored. The 2026-09-05 CFB capture, the NFL synthesised odds and the synthetic props/live fixtures were hand-checked with Caesars as the settle book, so `tests/helpers/settle-book.ts` loads them with the two books' keys exchanged — same prices, same consensus, every hand-computed number unchanged. Astra's uncommitted 30-file DraftKings pass (found in the working tree, never committed) is archived verbatim in the handoff's `review-2026-09-17-draftkings/`; its selection-basis override, double repricing and comment rewrites were not adopted, its stamping ideas were.

**INSTRUCTION 68 (2026-09-17):** Josh: "There should be a tab titled 'Ballpark Factor' that shows daily ballpark factor for every stadium that is being used in the engine to calculate bets. The engine should obviously know but it should take into account temperature, elevation, wind mph, wind in/out/left/right, etc". Shipped: `src/lib/mlb/ballpark.ts` — 30 parks (elevation, roof, orientation, the engine's own R/L park priors), `parkEnv` = temperature curve × wind mph × direction (in/out/left/right/cross, quadratic in mph, dome or closed roof = calm) × elevation, clamped 0.8–1.25, split into HR / hits / total-bases factors; `parkDailyForGame` builds the per-game read from the schedule's weather. The legacy engine's `windNote(g)` (L2024) consults a `shParkDaily` hook when `SH_CFG.parkDaily` is armed and its hits/TB paths read the hook's `h`/`tb` (`(wind.h||1)`, `wind.tb`); unarmed the blob is byte-identical (every baseline stands). `applyParkDaily(cfg)` + `bindParkDaily(engine)` are armed by all three generators (engine-client, /api/generate, the scheduler). New tab **Ballpark Factor** (`/ballpark`, MLB desk only, `app/api/mlb/ballpark/route.ts` reads the stored board's gameInfo + the schedule's weather): every park on today's slate with temp, wind mph + direction, elevation, roof, the HR/H/TB multipliers and the markets they lean. Cached boards reprice at the next refresh slot; nothing is invented when a game has no weather (factor 1, "no weather posted"). Engine string changed → `SERVED_ENGINE_SHA_VERIFIED` refreshed, `tests/served-verification.json` pending until the post-deploy re-grep — CLEARED the same day: the a3de0d5 production chunk matched byte-for-byte (287,367 chars, sha 52510d67); the verifier's facade anchor now accepts any shim parameter name (`)(s,'` this build).

**INSTRUCTION 69 (2026-09-17):** Josh: "When sorting by column on board or any other page, it needs to function correctly. On board tab, when switching from 'Top 50' to 'ALL' then selecting a prop, it does not sort from highest grade to lowest grade when you press on the grade column enough times to have it going top to bottom." Root cause (measured on prod): the Grade column keyed on `gradeRank` alone, so every unlettered row (live-market and unpriced) keyed to 0 and ties within a letter kept insertion order, and `DataTable` kept its sort state across view changes, so a prop tab opened in whatever direction the previous tab was left in. Fix: `gradeSortKey(rank, ev)` in `src/lib/grade.ts` (letter band × 1000 + clamped EV; settled rows sink below everything via `SETTLED_SINK`), used by the Board's two Grade columns and the CFB picks board; `DataTable` gained `defaultSort` and `resetKey` (Board resets on `scope|cat|live|selectedBook`, opens on Grade ▼) and its comparator pins NaN to the bottom. Tests: `tests/board-sort.test.ts`.

**INSTRUCTION 70 (2026-09-17):** Josh: "It also ALWAYS needs to show player headshot and/or team logo next to every single pick. If its a team ml.rl then it only needs a team logo ie: Tigers ML. If its a player on the team it needs a player headshot and team logo ie: Kevin McGonigle over 1.5 H+R+RBI (needs Kevins headshot and Tigers logo next to pick)". Shipped: `clubFromLabel` (`src/lib/mlb-visuals.ts`) resolves a club from an ML/RL label (full name, nickname, or leading tag — OAK/ATH, CHW/CWS, ARI/AZ); `BoardLabel` draws the headshot + team badge for a "Name (TM)" label and the club logo disc for a team label; the Board's stamped/ALL table, the generated parlays, the parlay generator sheet and the ledger all draw through it. Tests: `tests/pick-marks.test.ts`.

**INSTRUCTION 71 (2026-09-17):** Josh: "Some of the parlays showing up on 'Board' under generated parlays have 3 picks from the same game (ie: royals vs astros; 2 astros to hit HR & 1 royal to hit HR on 4 leg parlay w/ 1 Tiger to hit HR). Two picks from same game is fine if its needed because there aren't a ton of games or the picks are very high %/edge but 3 should be avoided. Also there should be an option there or parlay generator to see the edge % on any parlay that i personally generate/create using the metrics used by the engine to generate its own parlays. There needs to be as many parlays generated on the bottom of the board page as possible for variety. Even if its -95% (joking) i need more ability just to see more parlays even tho I wont take them so maybe it sways me away from a player i was going to take on my own because I see how negative they make HR or H+R+RBI parlays etc". Shipped: three dormant `SH_CFG` knobs in `buildParlaySet` (legacy L2654) — `parlayGameCap` (a ticket takes at most N legs from one game; the HR plan used to stack a game freely), `parlayMore` (the per-type and mixed plans repeat N extra times), `parlayCap` (per-player cap across a set, was 3) — armed by `applyParlayVariety(cfg)` (`PARLAY_VARIETY = {parlayGameCap: 2, parlayMore: 2, parlayCap: 5}`) in all three generators; unarmed the blob is byte-identical. Board: the generated-parlays section pages ("Show 48 more" / "Show all") instead of stopping at 24; a **My parlay** bar — a "+" on every board row and every generated-ticket leg, "→ mine" on a ticket copies all its legs — prices the tapped legs with the engine's own ticket arithmetic (selected book's odds multiplied out, engine true % multiplied out, EV = true × dec − 1, fair price), names legs the book doesn't price or the model can't rate and leaves them out, and says out loud that same-game correlation is not modelled (the engine's sim-joint repricing is server-side only) and when a third leg from one game exceeds the engine's own cap. Nothing is stored or ledgered. Tests: `tests/parlay-variety.test.ts`, `tests/my-parlay.test.ts`.

**One leg per TEAM in the parlay generator — R2b (2026-09-18, late evening):** Josh, verbatim: "Add filter on parlay generator alongside 'Two legs from one game' that says 'Two legs from one team' so i can prevent a 3 teamer from having 2 players from same team". `GenSpec.onePerTeam?: boolean` (src/lib/parlay-gen.ts) is the rule: at most one leg per folded `team` tag (a leg with no tag is never blocked). UNDEFINED = OFF so every saved recipe and every pinned fixture mints its exact ticket and seed; `specSeed` adds `t1` only when it is on; both desks (`GEN_SPEC_DEFAULT` on app/props/page.tsx and CfbProps.tsx) default it ON; `decodeSetup` loads a recipe without the field as ON. Enforced everywhere R2 is: `fits`/`seat` (a `usedT` set on Slots), the price lookahead, the mixed-phase and spread-across-categories checks (through one `blocks(rules, p, l)` helper), the payout repair, the pinned-slot conflict (`why: "same-team"`, after same-player and same-game), and `capacity`, which is now exact under both rules — under R2b the node that owns a seat is the CLUB (union of its players' games), so the doubleheader matching still answers the true number, and the lookahead arms when a club (not only a player) has legs in two games. `relaxHint` names `same-game` first whenever the game switch alone OR together with the team switch would fit (on an ordinary slate one-per-game implies one-per-team, so the game switch is the first step and the next spin names the team switch); `same-team` on its own only when it is the one binding. The ticket carries `sameTeam` beside `sameGame` and the sheet says "N team(s) carr(ies|y) more than one leg". Sheet: a `Two legs from one team` Toggle directly under `Two legs from one game` in Advanced; RELAX_PATCH/BUTTON/HINT `same-team` ("Allow two legs from one team"). Tests: `tests/parlay-gen-team.test.ts` (new, 17 — fixture: the Phillies game carries the in-band H+R+RBI overs, so 4 legs with the game switch on is short-pool have 3 relax same-team, 3 legs fill PHI/CHC/MIA, off → three Phillies flagged; undefined ≡ off byte-for-byte; a hand-built doubleheader where a greedy seat would strand; pins; the sheet), football-gen (+4 — the synthetic fixture resolves no roster, so its untagged legs are never blocked; then the test tags two clubs itself and pins one-per-club, the same-team failure and the stacked-club flag), parlay-gen-setup (+1, round-trip + legacy default). Unchanged: the server's own parlay sets (`buildParlaySet` / parlayGameCap) — this is the generator's rule only.

**Live HR parlays + HR line rule + rail-driven ranked list + compact generator (2026-09-18, evening):** Josh, with a desktop screenshot of the MLB Parlay Builder (Timing Mixed, HR, 3 legs, "34 lines · 1 game", "Attempt 5: no matching parlay… Mixed needs at least one pregame leg and one live leg"): "Why won't it generate parlays right now for HRs? There are a ton of HR live props on the board and just starting. I can access them on the Draftkings, william hill & caesars apps. I refreshed the board as well from 5:00pm last refresh to 7:03pm. Its also not sorting by filter type beneath in the 'every pick today S-F'; when I click a filter like 'H+R+RBI' it still shows washington nationals ML, anytime HR props etc" and "Also no HR bets shown EVER should be over 1.5 HR unless its a live bet in which the player already has 1 HR live OR it is a manual filter by myself to just look at grades on over 1.5 HRs pre game for funsies. Every bet should be over .5 HR or 1+HR. Either one. Lets also compact the UI on the parlay generator. If we have to do dropdowns etc in order to reduce space wasted/taken up then so be it." **Root cause (read off the 7:03pm production board, `/api/board?date=2026-09-18`):** nine of ten games were `live:true` with 81 HR rows priced at the 7:02pm refresh, and NONE reached the builder — `marketPhaseBoard` dropped every started game from "upcoming" and the pool builder rejected started rows without `quoteAt`, which only the authenticated live-props overlay ever stamped (fetched only with a stored sync key, ≤12 events a pass, no polling). So Mixed saw one upcoming game (SF@LAD, 34 HR lines, 16 of them O1.5) and no live leg → `phase-empty`. **Shipped: (1)** `liveMarketBoard` (`src/lib/mlb/market-board.ts`) has a second source — a `live:true` game's own rows, when statsapi says the game is priceable and the row's settle-book (else freshest) `bookQuotes[*].at` is within the same 30-minute gate (`rowQuoteAt`); the row is projected with `quoteAt` stamped and `pO` nulled (the engine's number on an in-play row is the pregame model; only `fO` is carried, so the leg is a "market" leg like every overlay leg). The overlay still wins when it has the row; a started game the board lists as `live:false` (began after the refresh) gets nothing — its prices are pregame prices. Board LIVE (`LiveOpportunities`) gets the same rows through the same call. `phase-empty` now carries `{pregame, live}` counts and `genFailLine` names the missing side and the last board refresh time ("in-play prices count for 30 minutes after the board refresh (last board refresh 7:03 PM). Refresh MLB, or switch to Pregame"). **(2)** the ranked chips WORK when hydrated (DOM-only CDP check on the production alias: NFL "Spread" → 28 rows, header "Ranked S → F · Spread"; MLB "RL" → 33 rows) — Josh was tapping the market rail / the generator's category chip, which carry the same labels and left the list alone. `RankedPicks` takes an optional controlled `filter`/`onFilter`; the MLB desk drives it from the rail (`rankedKeyOf`: rail tap, tab switch, and the generator's category tap via `moveRailTo`); "All" is still the default. The ranked pool is now built over phase "mixed" (upcoming + fresh in-play), so once every game has started the list no longer reads "HR 0 · Hits 0". Football desks untouched (uncontrolled). **(3)** the HR line rule: `hrLineAllowed(market, ln, {live, tally, altHr})` — an HR line above 0.5 is admitted only live with the homers already in the boxscore (`tally ≥ ln − 0.5`, via `liveNow.legNow`) or under the manual filter; `pruneHrLines` applies it to the upcoming games inside `marketPhaseBoard` (`opts.altHr`), the live projection applies it on both sources, the Board's ALL scope applies it to its pregame prop rows. The manual filter is a "Show O1.5 HR" chip (`data-testid="hr-alt-filter"`) on the MLB Props browse view for the Anytime HR market only; the generator and the ranked list never receive it. Server categories/parlays were already 0.5-only. **(4)** the generator sheet compacted: one-line hero (no eyebrow, tagline or emblem; counts inline), Legs / Sides / Timing as native selects on one row, hit-rate floor + window as two selects on one row, the explanatory paragraphs moved to `title` tooltips, Save / Load under Advanced, tighter spacing (`.gen-select { color-scheme: dark }`). Nothing about what the generator does changed. Tests: `tests/hr-line-rule.test.ts` (new), `mlb-market-phases` (+7: fallback, overlay precedence, stale/future/missing, post-refresh starts, decided lines, mixed from board rows, phase-empty counts), `parlay-gen-ui` (+5 compaction pins, re-pinned chip count), `ranked-picks` (+2 controlled filter, wiring pins). Unchanged and Josh's call: `MLB_LIVE_CLIENT.liveMaxEvents` 12 / budgets — the board's own rows need no credit.

**Cerulean on graphite + compact Board + prop lean (2026-09-18, later the same day):** Josh, three items verbatim: (1) "Add bet %/money % to the player props too somehow" (2) "On 'Board' tab, shrink the grade column horizontally so everything in the box fits on one screen. There is no reason for it to be that long. If theres explanations, make them individually expandable" (3) "Lets change the theme to Cerulean Blue. I think the neon type of color with the green & pink is what makes it too much to look at with the black background. Maybe we gray the background more than black as well to dilute it and make it look less like a cosmic bowling screen". Shipped: **(1)** no sportsbook or public page publishes ticket/handle splits on player props (re-checked; the consensus page carries sides only), so props carry a **price-implied market lean** instead — `src/lib/prop-lean.ts` (`leanFromPair` de-vigs the two-way price at the settle book, DK first; `mlbPropLean(row)`, `footballPropLean(sides, mode)`, `footballLeanIndex(rows, mode)` and `mlbLeanIndex(games)` key the lean to the same row keys the boards use) and `src/components/ui/LeanChip.tsx` (`data-lean-chip`; "56% O · 44% U lean" on browse rows, "56% lean" / "44% mkt lean" on a sided pick; the tooltip starts "Price-implied lean, not a bet count" and names the book). Wired on MLB Props `PlayerRow`, MLB Board (live board + picks table), football Props `PropRow`, football Board `CfbPicksBoard`, and the ranked prop picks on both desks through the existing `RankedPick.splits` slot; `GenLeg.lean` stamped by both pool builders. The lean is never labelled bet % or money %. **(2)** `DataTable` `Column.fit` (`w-px` shrink-to-content) + `headerTitle`; cell padding px-2; the grade / fair / book / best / EV / Kelly columns fit on both boards; the Pick cell is capped (176px, 280px from md); the real culprit — `SettledGrade` rendering its whole nowrap explanation inline — is now a per-row "why ▾" toggle (`data-testid="settled-why-toggle"` / `settled-why`, the sentence stays in `title`); `LiveGrade`'s market branch is "+2.7% vs market" with the explanation in `title`. **(3)** `app/globals.css` rethemed end to end without renaming a token: bg `hsl(216 9% 13%)` graphite (surfaces 15/18/21%, lines 26/32%), `--color-pos` `#3ab0e8` cerulean, brand tokens `#1565a8 / #1f8fd4 / #5cc4f2`, gradient text cerulean→sky, live lavender `#b794f6`, gold and the NFL `#4f8cff` / CFB `#f5a524` accents unchanged; every pink rgba, glass tint, aurora, glow and the whole `.gen-studio` family retinted and dimmed; `theme-color` / manifest `#1e2126`; the llama footage is recoloured by a CSS filter (`hue-rotate(230deg) saturate(0.8)`) under a `bg-bg/62` scrim; the Board tab tone is periwinkle `#A5B4FC` (never the brand colour). Tests: `tests/prop-lean.test.ts`, `tests/board-compact.test.ts`, `tests/theme-cerulean.test.ts` (new, 33 tests — the theme test forbids any pink literal and any new green); `nav`, `prop-hit-rate` re-pinned. No local headless screenshot run this time (Josh declined it); proof is the test pins + the full serial gate.

**Eight-item UI pass (2026-09-18):** Josh, eight items verbatim with 15 desktop screenshots (1 Parlay Lab landing, 9 Caesars mobile dark + 1 light, 5 Parlay Lab desktop): (1) "Re-order the sports from MLB/CFB/NFL to NFL/CFB/MLB" (2) "Clicking Parlay Lab logo in top left brings up this page for some reason … It should never bring this up. It should either pull up games tab, board tab or stats tab" (3) "Each pick anywhere on the website should also tell me the bet % and Money %" (bet % = share of tickets on each side; money % = share of the handle) (4) Caesars screenshots — "the team logo is not visible with the black circle background encompassing the logos"; "no headshots on stats page only logos showing"; desktop first, mobile too (5) "It should show grades next to every pick on the games page ie: spreads, MLs, overs" (6) "The boxes for the picks on 'Board' and 'Builder' screens are way too big. They could easily be shrunk by 50% vertically" (7) generator "header/filters on top & the picks/generate button below … stacked top/bottom", "numbers next to the picks generated", "ability to reorder/drag the picks so if im keeping the bottom pick i can drag it to top, hit the 'lock it in' button on the pick then regenerate the ones below it" (8) "Below the Parlay Generator, the default view should be every pick available for the day ranked from S down … filters above it to sort by each option (ex: ML, RL, HR, Hits, H+R+RBI, etc)". Shipped: **(1)** `SPORTS = ["nfl", "cfb", "mlb"]` in `src/lib/sport.ts` (the switch maps it verbatim; `tests/nav` re-pinned). **(2)** the `Brand` wordmark in `AppShell.tsx` is `<Link replace href="/games">` — the landing is unreachable from the logo. **(3)** `src/lib/splits.ts` parses the public scoresandodds `/{nfl|mlb|ncaaf}/consensus-picks` page (left pennant = AWAY, verified against statsapi; `findGameSplits` by abbr/name aliases + kickoff, `sideSplit(g, "ml"|"spread"|"total", side)`, `mlbLegSplit(feed, {label, sub, gkey})` for board/builder rows); `app/api/splits/route.ts` (10-min server cache, always HTTP 200 — `games: []` when the page is down); `src/lib/use-splits.ts` (`useSplits(league)`, react-query 10 min, falls back to its own idle QueryClient outside a provider so bare card renders never throw); `SplitsChip` (`data-splits-chip`, "78% bets · 62% $" / compact "78%·62%$", renders NOTHING when there is no match). Wired on football Games (`OddsGrid` cell split line), Board (`CfbPicksBoard` + FeaturedPick), Builder/ledger tickets (`CfbTicketCard`), and MLB Games (`TeamRow`), Board (`app/board`), Props (`GameCard`), Builder (`TicketCard`). **No player-prop splits exist publicly** — prop rows carry no chip, by design. **(4)** `TeamMark`/`PlayerMark`: the disc has NO dark background or ring when a logo draws (logo at full size with a drop shadow, `LOGO_BADGE_CLASS` light badge for the headshot's club); Stats page rows get a 24px headshot (`headshotFor`: MLB img.mlbstatic, NFL/CFB ESPN combiner; initials fallback `data-stat-headshot`). **(5)** every `OddsGrid` cell carries `grade` (GradeChip badge top-right, `.odds-cell-grade`, `.is-S` gold … `.is-F` red) on both football Games cards; MLB Games ML rows get a `GradeChip` (`MlPrice.grade`) — RL/total are not on the MLB games payload, so only ML is graded there. **(6)** `DataTable` cells px-2.5 py-1.5; `TicketCard` (MLB builder), `CfbTicketCard`, `FeaturedPick` legs compacted (single-line legs, 9.5px subs, tighter paddings). **(7)** `GenSheet`: settings stack ABOVE the ticket (no side-by-side grid, no sticky column); every slot wears `.gen-slot-no` (1..n) and `aria-label="Lock in slot N: name"` / "Unlock"; slots are HTML5-draggable with ▲▼ buttons (`Move slot N up/down`, ends disabled); `useParlayGen.reorder(from, to)` re-seats the ticket (`applyOrder`, keyed to the ticket so a new spin is never mis-ordered) and moves any pin with its leg (spec untouched when nothing is locked, so a drag never re-rolls). Pinned by `tests/parlay-gen-reorder.test.ts` — the same locked leg at slot 1 vs slot 4 gives the same ticket key, seated where the pin is. **(8)** `src/components/props/RankedPicks.tsx` (`RankedPicks`, `RankedViewTabs`, `RANKED_PAGE` 60): every priced pick on the day graded by `gradeFromEv` and sorted S→A→B→C→D→F then EV; "All" + per-category chips with counts (MLB: ML, RL, RBI, Runs, Hits, TB, HR, H+R+RBI, K's, Outs; football: ML, Spread, Total + every football prop market); tier counts in the header; price button toggles the slip; "Show 60 more". Default view on `/props` for MLB (`view` starts "ranked", a deep link still opens "games") and on CFB/NFL (`CfbProps`; the props query now stays enabled on the ranked view). Tests: `tests/splits.test.ts` (fixtures `tests/fixtures/consensus-{nfl,mlb,ncaaf}.html`), `tests/parlay-gen-reorder.test.ts`, `tests/ranked-picks.test.ts` (new); `nav`, `parlay-gen-ui`, `cfb-props-ui` re-pinned. Verified locally by driving headless Chrome over the DevTools protocol (the desktop app's hidden browser pane no longer hydrates pages): every page on all three desks renders with zero console errors; the local checkout has no board data, so ranked rows / generator slots were exercised by the fixture tests only.

**UI overhaul (2026-09-18):** Josh, five items verbatim: (1) "The UI needs a significant upgrade … The parlay generator needs significantly more customization. It doesn't need 'Safer Mix'/'Balanced Mix' or 'Favorites/Even/Longshots' because majority of the time those lines aren't consistent across props" (2) "The exclude player button is way too big and visible it looks atrocious" (3) "Everything should always be graded and priced based on DK so that shouldn't need to be an option in advanced options for parlay generator" (4) "incorporate stats on the parlay generator/picks pages … analytics on how often players are achieving the selected prop we are viewing over the last 7, 15, 30 60 & 120 games" (5) "I want it to feel more like I'm playing a game … Look at the screenshots provided of caesars sportsbook app … how little space there is between filters". NO screenshots came through; the chip grammar follows the known Caesars/William Hill layout (tight ~28–30px pill rows, minimal gaps). Shipped: **GenSheet rewritten** (`src/components/props/GenSheet.tsx`) — build styles, odds presets, "Your mix" and the DraftKings-only toggle are gone (DK is the settlement book everywhere via `SETTLE_BOOK`; `czOnly` stays in the spec type, default false, no UI); the sheet is chip rows: Legs · **Categories (multi-select)** · Odds per leg (Min/Max + a two-thumb slider that stops on prices the board really posts) · **Hit rate floor** (Any/50/60/70/80%) + window L7…L120 · Sides · Games · Positions (football) · Timing; Advanced keeps two-legs-one-game, started games, **"Every category on the ticket at least once"** (spread rule, default on) and Model-priced-only; Save/Load setup is codec **v2** (`src/lib/parlay-gen-setup.ts`: markets/spread/minHit round-trip, games never saved, v1 recipes still load with the style dropped). Exclude is a 24px ghost ✕ (`aria-label="Exclude {name} from generated parlays"`, ↺ restore). **Generator core** (`src/lib/parlay-gen.ts`): `GenSpec.markets/spread/minHit/games`, `specMarkets()`, `spreadTargets()` + the spread rule in `fillSlots`, relax hints `hit`/`games`, seed parts `markets:/spread:off/hit:/games:`; both adapters build a union pool with `market/line/gameLabel` on every leg (`mlb-gen-pool.ts` takes an optional `MlbHitSource` and stamps `hit`; `football/gen-pool.ts` filters on the set). Hook `useParlayGen`: the rail follows the last-tapped category; a rail tap outside the set collapses to that one category; pins blank when the set changes. **Hit rates**: `src/lib/prop-hit-rate.ts` (pure counting — a hitter's game needs an AB, a pitcher's a start; over = strictly more, under = strictly less; windows 7/15/30/60/120; tone pink ≥60% gold ≥45% red), `app/api/mlb/hit-rates/route.ts` (POST names → per-player 2026 game logs from the free MLB statsapi, `people?personIds&hydrate=stats(gameLog)`, 10-min server cache, 400 players cap, no key, no Odds credit), `src/lib/mlb/player-index.ts` (shared with `/api/player/resolve`), `src/lib/mlb/useHitRates.ts` (react-query, 10 min stale; `useHitWindow` at localStorage `pl:props:hit-window`), `HitChip`/`HitDots` (`src/components/props/HitChip.tsx`) on every Props player row, every generator slot and every Board pick with a side+line; the MarketNav search box carries a tap-to-cycle L7→L120 button. Football gets no hit data (`showHitRate` off). **Readability**: `--color-muted` 65→72%, `--color-faint` 45→55%, `.glass` 78→88% surface, backdrop scrim 40→55%, rail chips 28px, `.gen-studio` retinted pink. Tests: `tests/prop-hit-rate.test.ts` (new), `tests/parlay-gen-multi.test.ts` (new, on the gen-pool fixture + the CFB synthetic props), `parlay-gen-ui`/`parlay-gen-setup`/`football-gen` re-pinned. Hit rates are a count of games already played — the footer says so — never a forecast.

**Brand colour (2026-09-18):** Josh: "I was saying the whole website should match the new color of the background. it was green with the green llama." The identity is hot pink now: surfaces hue 330, `--color-pos` #ff5fb8, brand gradient #a855f7 → #ec4899 → #ff5fb8, every lime glow in globals.css retinted, Board/Ballpark tab tones pink, PWA theme colour #0b0408, app icons recoloured. `--color-acc-teal/green/lime` keep their NAMES (classes + `tests/picks-surface` key on them) — they are names, not colours. −EV red-orange, gold, live ice-blue, CFB amber, NFL blue unchanged.

**Backdrop colour (2026-09-18):** Josh: "Bring parlay lab background back to the original colors in this mp4. Pink instead of green for the most part." `VideoBackdrop` no longer applies `hue-rotate(120deg)`; the footage plays in its own pink/magenta glow with red neon goggles. The `.aurora` fallback gradients (globals.css) are pink/rose to match. Cosmetic only.

**INSTRUCTION 72 (2026-09-17):** Josh: "I like how CFB threw in one massive ticket today for CFB with very limited game chances and hit big. MLB needs to have more variety. Almost every day its just 2 team hits prop parlays. There needs to be more H+R+RBI, ML/RL, straight bets etc. You can also increase daily money to be spent every single day no matter what by builder/ledger for MLB to $350". Root cause (measured on the 09-17 pool: 88 parlays + 73 mixed, one single each): since `PAPER_ACTION_SINCE` (2026-09-13) the MLB card ran the fixed shape P — three $50 two-leg slots filled purely by probability, both ceilings 2.6 — and the highest-probability two-leg ticket on any MLB slate is a hits parlay, so the card was hits parlays every day; rule 5 (`noHrrOver`) dropped every H+R+RBI-over leg before allocation, and ML/RL tickets only exist as 2-leg `rl`/`ml` SAFER parlays. Shipped, from **2026-09-18** (`VARIETY_SINCE`): `PAPER.daily` = **$350** with `dailyBefore` = 150 and `paperDaily(date)` the one date-aware reader — the lock card (`dayCeiling`), the refill (`decideMlbRefill`), `/api/generate`'s `effectiveBlockBudget`, the ledger merge kernel (`deskPaperOf`, MLB by the day's own date) and the banner all read it, so every day from 08-15 to 09-17 stays a $150 record. Shape **V** (`VARIETY_SHAPE`, $350, nine market-typed slots in fill order): $40 H+R+RBI 2-leg (`kind: "hrr"` — the ONLY place an H+R+RBI-over ticket is admitted; rule 5 holds everywhere else), $40 ML/RL 2-leg (`kind: "team"`, every leg ml/rl), 2×$50 2-leg, 2×$40 straight (1-leg), $40 3-leg, $30 4–5 leg, $20 5–6 leg. `buildStraightPool(data)` (`src/lib/server/lock-card.ts`) composes a 1-leg ticket from every pregame board row that carries a settlement price, a best price, an engine % and a leg key (ML/RL, Hits, TB, H+R+RBI, K's, Outs; `straight: true`, EV via `evAt`, `src "s"`), deduped against engine ids; the allocator's `fitsSlot` now includes `slotKindAdmits(kind, legs)` and, on untyped slots of a variety day, prefers a ticket whose `type` is not yet seated (falls back rather than leave money unallocated). `paperPolicy` = `variety-action-v1` on variety days (`probability-action-v1` before); shape pick reason names INSTRUCTION 72; card note names the straight count. Pre-variety days are unchanged (09-17 re-fires as P / $150). Tests: `tests/variety-core.test.ts` (paperDaily pins, source scans of every daily consumer, shape V slot names/kinds, `legMarket`/`slotKindAdmits`, `buildStraightPool` arithmetic, a mock-allocator fill, the real fixture engine on 2026-09-18 ≤ $350 with ≥ 3 market types and straights present, append-only re-fire); `core-shapes`, `paper-epoch`, `paper-deficit`, `ledger-merge` re-pinned to `PAPER.dailyBefore` for historic days. First $350 variety card: the 8am PT 2026-09-18 slot.

**September 19, 17-item phased request — Phase 1 implementation:** Josh requested probability visibility, restored Stats splits, hourly start windows, compact player-first generator controls, flexible mixed/same-game permission, live progress, common multi-select timing/market/style/sport filters, 50% vertical density, signed mobile odds, 0–80% hit floors, and expanded ballpark analysis. See `september-19-phases.md` for all four phases and precise completion boundaries. Phase 1 updates the shared generator and ranked list, not the auto paper engine or budgets; deployed as 7ee44f2 / parlay-yzb99bcwj; /api/version and populated NFL mobile generator verified; 3598 tests, TypeScript and production build passed. Mixed timing now permits all-live/all-pregame tickets, superseding the older mandatory-both instruction above. New filters bind pins and saved recipes. The follow-up implementation and its validation boundaries are recorded below.

**September 19 follow-up — finish the remaining 17-item request:** Josh: "Finish the rest of it. Just wanted it split into at least two sessions." Shared timing/market/style/sport discovery, cross-sport stored-quote candidates/slips, market-relative strategies, verified live prop freshness and official stat meters, restored MLB statistical splits, ballpark grades/order/evidence/game picks, Builder start-window display filters and signed-price audit are implemented. See `september-19-phases.md` for exact behavior and release evidence. Density offers readable Compact by default plus the requested literal 50% height and 100% comparison; widths are unchanged. Same-game math is still an explicitly labeled independent-product estimate, not a new joint-probability model. No paid quota, paper budget, ledger or parity-locked simulation changes. The implementation is deployed: 3,621 full-suite tests and the production build passed; post-rebase audit checks and production mobile verification passed. The final follow-up corrects stale help text and carries this release record.

**September 19 compaction checkpoint — standing owner instruction:** Josh: "/compact and make sure everything is written in parlay lab handoff now as well as every single time we update & especially make sure when we compact incase you accidentally forgot during an update session which should never happen". Every update must be mirrored; requested compaction additionally requires a forced, completed, verified sync. Canonical instructions are in CLAUDE.md and the generated START-HERE. The exporter now fingerprints actual source contents, includes new nonignored files in dirty snapshots, refreshes history when HEAD changes and fails rather than silently retaining a failed archive. Current application release: `510c64a019764293743650820f5627577de4d540`, verified Ready at parlay-47cfywyys; all remaining September 19 feature phases are deployed. This checkpoint changes handoff tooling/documentation only. Secret files remain excluded. No remaining feature work is being deferred by compaction.

**INSTRUCTION 73 (2026-09-25, five iPhone screenshots):** Josh requests significantly shorter Games cards; Top Edges initially 10 with Undo/Clear; compact generator control columns and generated Board parlays; Every Pick Today category rail directly below Customize your picks and above Timing; much stronger individual pick colors across the app; compact Ledger; restore recognizable, attractive ticket cards on The Card; solid camera-area background with all header text and controls below the safe area. This is Parlay Lab (the screenshots), not the companion Roster Lab project. Implemented as presentation/navigation changes only. NFL/CFB share the updated football surfaces; MLB shares generator/discovery/ranked controls and gains compact generated-ticket/ledger styling. No pricing, ranking, allocation, budget, settlement, lock or API-provider changes. Local populated fixture preview exercised Games, generator, Every Pick Today, Top Edges, The Card, generated parlays and Ledger. Temporary fixtures and simulated safe-area styles are removed before release. Exact validation and deployment evidence are in the current progress entry and external release-verification.json.
...
- **THE SYSTEM IS AUTONOMOUS AND PROVEN OVER FOUR LIVE DAYS.** Scheduler pokes every 15
  min (hours 15–23, 0–2 UTC, cron-job.org "Parlay Scheduler"; since INSTRUCTION 49 a poke only TOPS UP on the five PT refill slots — first locks are unchanged); **per-block locking live**
...
| doc | what it holds |
|---|---|
| `september-19-phases.md` | September 19 seventeen-request phase map, implemented generator changes, validation limits and remaining work |
| `cfb-desk.md` | the College Football desk reference: model constants, pricing pipeline, team-alias table, card and fun-parlay rules, player props and the picks / parlay tiers (live window, carried rows), the iOS freeze fix, the props quota cap, the Caesars-grammar UI primitives, every separate key/route/store, tests and fixtures, known dev-only behaviours |
| `nfl-desk.md` | the NFL desk reference (2026-09-08): `NFL_PAPER` $350 core / $25 fun since 2026-09-10, `NFL_RULES` $50 max and 3–10 tickets, `NFL_MODEL` σ 13.5 / HFA 2.0, the shared football engine behind `LeagueConfig` + `LeagueContext`, every `pl_nfl_*` / `pl:nfl:*` key, the four `/api/nfl*` routes, the scheduler forward, cron coverage (London 13:30Z gap), the Season Lab cut, the unmeasured props credit figure |
...
| `first-sunday-six.md` | Captured Caesars promotion prices, historical timing evidence, coverage and limitations. |

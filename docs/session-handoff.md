...
>
> - **STATE-CLAIM 2026-09-19:** `origin/frontend-rebuild` = `9c048d8` (verified by `git rev-parse refs/remotes/origin/frontend-rebuild` after the 1H bets release, commit 9c048d8 deployed as parlay-2e1olviq3).
...
**September 19, 17-item phased request — Phase 1 implementation:** Josh requested probability visibility, restored Stats splits, hourly start windows, compact player-first generator controls, flexible mixed/same-game permission, live progress, common multi-select timing/market/style/sport filters, 50% vertical density, signed mobile odds, 0–80% hit floors, and expanded ballpark analysis. See `september-19-phases.md` for all four phases and precise completion boundaries. Phase 1 updates the shared generator and ranked list, not the auto paper engine or budgets; full deployment evidence is pending validation. Mixed timing now permits all-live/all-pregame tickets, superseding the older mandatory-both instruction above. New filters bind pins and saved recipes. Do not mark the remaining phases complete.

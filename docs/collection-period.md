...
| **`coreNoHR`** | `true` | **SCOPE CORRECTED 2026-08-01 (owner's item 2).** It governs **CORE TICKET ELIGIBILITY ONLY** (`legacy/index.html` L2975–76: rejects a ticket whose type is `batter_home_runs` or any of whose legs carries `\|batter_home_runs\|`). **It does NOT control HR's absence from the MIXED set — that is enforced independently by a BARE LITERAL at L2785** (`if(k==="batter_home_runs")return` in `mixPool`), verified on the archived 2026-07-26 board: **49 HR legs in `parlays`, 0 in `parlaysMixed`**. **So setting this key to `false` would restore HR to CORE and NOT to MIXED.** Its old comment — *"HR volatility lives in the FUN bucket only"* — overstates the key's reach: the key does not put HR in FUN, it keeps HR out of CORE, and a separate unnamed literal keeps it out of MIXED. Measured basis added 2026-08-01: CV 1.97 at HR's median implied vs 0.70–1.03 for every other market |

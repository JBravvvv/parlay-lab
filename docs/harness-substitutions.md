...
| `CAESARS_KEY` → `deps.settlementBook` | whenever the caller passes `settlementBook` — production always passes `SETTLE_BOOK` (`draftkings`, INSTRUCTION 67, 2026-09-17); a test that omits it runs the blob's own `var CAESARS_KEY = "williamhill_us"` | nothing when the test passes `SETTLE_BOOK`; a test that omits it grades, edges and stakes at Caesars | a fixture hand-checked at Caesars passing against an engine that settles at DraftKings in production — every grade, edge %, EV and Kelly stake keys off this one binding |
| `shParkDaily` → `parkDailyForGame` (`src/lib/mlb/ballpark.ts`) | only when the caller arms `SH_CFG.parkDaily` AND binds the hook (`applyParkDaily` + `bindParkDaily`) — production always does (engine-client, /api/generate, the scheduler); fixtures never do | nothing: unarmed, `windNote` runs the legacy 10-mph rule and the digests are byte-identical (`tests/ballpark-engine.test.ts`) | a baseline passing against an engine whose HR/hits/TB probabilities move with temperature, wind mph, direction and elevation in production (INSTRUCTION 68) |
| `SH_CFG.parlayGameCap / parlayMore / parlayCap` (`buildParlaySet`) | only when the caller arms them (`applyParlayVariety`, production always: 2 / 2 / 5); fixtures never do | nothing: unarmed the ticket sets are the legacy plan (three per player, a game may be stacked) | a parlay baseline passing against an engine that in production caps two legs per game, triples the plan and allows five tickets per player (INSTRUCTION 71) |

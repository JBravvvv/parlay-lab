...
| **M7+M9** | ⚠️ **INTERLOCKED PAIR — ship together or not at all.** M7: `shPOver` uses Poisson where the process is binomial. M9: λ is inflated ≈ **+13.9%**, exactly cancelling it | **derivation stands, net output is DORMANT.** Rung signature **−4.9 / +0.5 / +2.8 pp** at O0.5/1.5/2.5 — a **− → +** flip. But on the real board `batter_hits` O0.5 reads **+0.3 pp** median with **47%** under-has-edge: no level bias, no flip, no skew. ⚠️ **The −4.3 pp that motivated it was a FIXTURE artifact** | **each other.** Fixing either alone moves **617 rows ~5 pp the wrong way**. M9 cannot be localised until graded hits accrue — the market gives a probability, not a mean | **interlocked, held** |

...
| **M14** | **the allocator is NON-MONOTONE IN PRICE** — a uniform one-book price improvement of every pool ticket LOWERS card E[ln]: +126.6 → +129.0 → **+117.3** → **+110.4** → **+70.5** → +74.4 bp at 0/+0.25/+0.5/+1.0/+1.5/+2.0 pp (parlay card; singles fall at +1.5/+2.0 too — 07-26 board, allocator in loop). Mechanism traced: admission is `czEv ≥ coreEvMin` (L3002; `coreEvMin: 2`, L1075 — does NOT expire at the reopens), ranking is greedy `eff = prob/(1+0.5·sharedGames)` (L3055, L3076–95) — **price enters admission and never ranking**, so floor-admitted high-prob/low-edge newcomers displace higher-growth incumbents. Discovered via the two-book collapse (3.013 → 0.513), which is hereby re-attributed | the objective of record (log-growth; "¼-Kelly sizing" comments L1070/L3103) and the ranking quantity (probability) are different quantities — the same base=prob decision A1 documents (`2292b85` deliberate), now shown to violate monotonicity, not just growth level. **Any fixed-card PRICE-response measurement inherits this failure mode** (audit table below); the multi-book adoption decision is downstream of M14 | **NO ALLOCATOR CHANGE SHIPS — freeze.** n=1 board; A1 (price-aware ranking) + A2 (scale-stable admission) are the existing amendment pair addressing its two halves — evidence joins THEIR exit sign-off | **NEW 2026-07-28 — documented defect with magnitude (−56 bp from +1.5 pp on this board)** |

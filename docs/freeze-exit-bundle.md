...
| **M14** | **the allocator is NON-MONOTONE IN PRICE, and the mechanism is LOCALIZED TO COMPOSITION (2026-07-28, owner's three controls, all free)**. In-loop sweep: +126.6 → +129.0 → +117.3 → +110.4 → **+70.5** → +74.4 bp at 0…+2.0 pp. **Control A (composition frozen, prices bumped): strictly monotone increasing** +126.6 → +131.4 → +136.1 → +140.9 → +145.6 — pricing and sizing are CLEAN; the defect is admission + ranking and nothing more. **Control B (negative bumps, in loop): +121.2 / +117.8 / +124.9 at −0.5/−1.0/−1.5 — "Worse prices produce better cards."** (verbatim, the strongest statement this defect admits — E[ln] RISES from −1.0 to −1.5). **Control C (census)**: newcomer stake $0 → $38 → $84 → $133 across +0.25…+1.5 while card legs collapse 24 → 16; the +1.0→+1.5 cliff (−40 bp) = two 2-leg newcomers entering (incl. `Hits O0.5 + Hits O0.5`, the predicted high-prob/low-edge class) displacing the deGrom/Freeland outs parlay + a mixed. **`prob` in `eff` is the MODEL probability, not price-implied** (ticket `prob` built from leg `est` products + sim adjustments, L2641–2712; alloc reads `w.pl.prob` L3053, `base=prob` L3055) — no feedback loop; the admission story takes precedence and holds. Card capped at `maxCoreTickets: 6` — displacement, not expansion | **`coreEvMin = 2` is an UNMEASURED parameter in the defect's admission path**: percent units, gate `selEv >= evMin` (L3002/L3009); provenance = RAISED to 2 by `06d3cbc` ("Raise the EV gate to +2%") — chosen, never fitted; no doc records a measurement behind the value. Second-axis sweep (in loop, this board): E[ln] **monotone INCREASING in coreEvMin** — 1 → +113.0 (25/67 clear), 2 → +126.6 (21), 3 → +126.6 (19), 4 → **+139.1** (18) — the loose floor IS the admission valve (and at 4 the card ≈ the leg-equivalent card's +139.1). **The measured two-book gain, +1.07 pp/leg, sits INSIDE the (+1.0, +1.5) interval containing the cliff** — this is not a large-bump artifact | **NO ALLOCATOR CHANGE SHIPS — freeze.** n=1 board; the sweep does not generalize until it reruns on a second board, and board count is gated on credits (the burn plan). A1 (price-aware ranking) + A2 (scale-stable admission) are the two halves — evidence joins THEIR exit sign-off | **NEW 2026-07-28, mechanism LOCALIZED same day — composition only; magnitude −56 bp at +1.5 pp on this board** |
...
| M14 (2026-07-28) | allocation-axis, orthogonal to the rate estimators — but it CONTAMINATES any fixed-card price-response measurement | **frozen defect, no ship.** Dependency: the multi-book adoption decision and every future price-improvement claim must run allocator-in-loop; A1/A2's exit sign-off inherits the M14 evidence |

...
|---|---|---|---|---|
| **M1** | **`shParkF` → the closed-form factors** | hit rate **1.5% median / 3.5% max** error today; **HR rate 4.5% median / 11.0% p90 / 14.5% max (PNC)**. 92% of venues populated, and the closed form uses **1.000** at 29 parks in 30 | nothing. Self-contained | **ready to spec** |
...
| **A3** | **`consMinEv` multiplicative structure** | the per-leg bar **tightens** with leg count — **−1.000% / −0.501% / −0.334%** at 1/2/3 legs — driven by the **1.071** Caesars overround. It is a **structure filter wearing a quality filter's name**, and it runs opposite to `coreEvMin`, which loosens (+2.00 / +1.00 / +0.66%) | nothing. **Josh's call: do not change it** — recorded here so the bundle is complete, not because it is proposed |
...
|---|---|
| `batter_hits` closed-form **−4.3 pp undershoot vs market** | found today by the external check. No mechanism traced yet, and the sim is not the answer |

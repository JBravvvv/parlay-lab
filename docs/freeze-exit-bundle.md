...
| **M3** | **H+R+RBI λ conditioning** (mass-weighted blend of `hF`/`tbF`, not a product) | closed-form λ is `rate × power` on a non-Coors slate — **zero site variation** against a market rung-drift of **+0.479**. Recovers ~0.12 of spread on its own; more with M1 | M1 for the park term. Weights need fitting | scoped **[src: ARCHIVE — the 2026-07-26 board; n=5 closed-form pairs, n=15 sim (hrr-recalibration L8/L47)]** |
...
| **M20** | **every market suspension is UNENFORCED in two device-reachable selection modes** — the `hrrAltMax` bar (and, by the same construction, Thursday's outs flag) lives inside `buildParlaySet`'s DISCIPLINED branch, so it applies only at `selMode ∈ {ev_gated, dk_fd}`. `probability` and `caesars_ev` are each ~2 taps from the device (Settings → mode, persisted to `pl_selmode`), and the suspension guards had only ever extracted `{CRON_SEL_MODE, getSelectionMode default}` = `{ev_gated}` — one quarter of the domain, reporting success. **The exposure is BY DESIGN (the parity stance — the legacy modes reproduce the historical engine) but its premise, that those modes are not production-reachable, is false on this device.** | magnitude, measured 2026-07-30 on the armed fixture slate: **11 H+R+RBI legs into the pool and 4 into the FUN pick** in EACH legacy mode (the exact 2026-07-27 eleven-leg-scare numbers), pool sizes 48; **10 `pitcher_outs` legs** in each, today, pre-flag. Disciplined modes measured ZERO in pool and FUN, with AND without the override (`force` strips the Kelly ceiling in `shAllocate` but never reaches `buildParlaySet`'s bar) | coverage CLOSED the same day, test-only (no engine string, no hash move, no vintage event): `tests/helpers/modes.ts` extracts the whole reachable domain from source (the `SelectionMode` union + the cron pin + the client default + override reachability read from the Builder's `shSetOverride(true)` control), both coupling guards extended — disciplined × {force off, on} asserted ZERO, the legacy pair pinned as a COUNT census so a silent change in either direction fails CI | **NEW 2026-07-30 — measured, guarded, NOT fixed: making the bars unconditional would end the parity stance, so it is an owner decision (spec-only). Live confirmation is chain step 15 (reading 28): the probability-mode device read on today's board** |
> **M20 — THE BANKROLL NUMBER, MEASURED 2026-07-30 BEFORE THE BOARD (owner's item 1)**: in `caesars_ev` the live card's **rank-#1 ticket is an H+R+RBI parlay at $62 = 8.27% of a $750 bankroll** — the SAME market as the 46.3/59.2 finding, both legs **O 1.5** (the 32%-hitting subset). Its **Kelly ceiling is computed at $13 and discarded** (`disciplined` false in the ceiling-free modes) — a **4.8×** overstake, bound instead by structural `capG` $62.5. Susp-row census by mode: ev_gated 14 · dk_fd 14 · probability 0 · caesars_ev 0 (14 HRR rows on the board in every mode) — so **the suspension reduces exposure in the pinned mode and inverts to a top-of-card position in a two-tap mode**. The $50 operator rule catches THIS ticket; it will NOT catch the post-flag `probability`-mode **outs ticket at $17 (2.27%)** — the rule bounds size, not suspended-market participation. Full block: collection-period, BANKROLL EXPOSURE.
| **M26** | **an oversizing warning suppressed by the same condition that makes oversizing certain** — `app/builder/page.tsx` L885 gates the Kelly banner on `card.kellyDaily > 0`, and `kellyDaily` is exactly 0 in `probability` mode because every ticket's Kelly fraction floors at 0. The warning is off precisely where every ticket is infinitely overstaked. Provenance SILENT: added in `06d3cbc` (2026-07-19) whose only comment explains the advisory posture and never mentions the zero case | measured: `caesars_ev` banner **FIRES** ("Entered $250 is 83.3x the Kelly-consistent $3") — so for that mode the display is NOT the failure; `probability` banner **SUPPRESSED** at `kellyDaily = $0` with six tickets staked $17–$56. The per-ticket badge DOES render in both — the screen has been showing **"Kelly $0" beside a $62 stake** all along (`TicketCard` L143, L167–172), a number with no sentence saying what it meant | **CLASS, not instance — ZERO-SUPPRESSES-EXPLANATION**: second site at L857, where `RebuildNote` (which markets are still under `consMinN`) is hidden when `picks.length === 0`, i.e. on exactly the NO-PLAY day the consensus gate most likely caused. Spec-only fix: surface the ratio with a breach label, drop both positivity gates, guard red first | **NEW 2026-07-31 — display tier; the only instrument that would have caught M24 by eye, disabled in the worst case** |
| **M27** | **a signed-off guard was WEAKENED IN PLACE, and its original is sitting beside it unrun** — `tests/sha-references.test.ts`'s `originRefs()` returned only `refs/remotes/origin`; `ea7445a` (2026-07-29T14:10:29-07:00) appended `"HEAD"`, so a doc-cited sha reachable only from a LOCAL, unpushed commit now passes. The widening is documented in place and has a real justification (the hold rhythm), and it changes nothing while the tree is clean — but it landed inside a commit whose subject is *"Session handoff before compaction"*, five lines in a large docs window, and was never reviewed as a guard change. The pre-widening file survives as an UNTRACKED orphan (`tests/sha-references.test 2.ts`, 07-29 12:32) that vitest's `tests/**/*.test.ts` glob does not run — a strict assertion preserved next to its relaxed replacement, invisible to the runner | residual risk: a doc cites a sha that exists only on a local commit; a later reset or rebase dangles the citation and the guard has already passed. Zero instances today (everything pushed) | nothing — the two other orphans (`bot-path-whitelist`, `min-gap`) are byte-identical copies and carry no assertion difference | **OWNER'S DECISION, not restored unilaterally**: (a) keep the widening and accept the reset/rebase hole, or (b) narrow it to shas that are ancestors of the current branch AND listed in the handoff's held-stack record. A runner-glob check forcing file-count = run-count is spec'd, not shipped **[src: REPO/GIT — a diff between the live guard and an untracked orphan, plus `git log -S` on the widening commit. No board, no fixture, no magnitude beyond the file counts (86 .ts under tests/, 81 run, 5 non-matching of which 2 are legitimate helpers).]** |
| **M28** | **a credential-collection UI has been INERT SINCE THE DAY IT SHIPPED, and it tells the operator it is protecting him** — `app/settings/page.tsx` collects a device passcode, writes it to `localStorage.pl_pass` (L348), and **no client code reads it into any request**. `6a28eef` (2026-07-11) shipped the working path — `app/sharp/page.tsx` read the value and sent `x-pl-pass` on the `/api/sharp` POST with a 401 branch — and **`e9f4bc7` the SAME DAY (2026-07-11T12:16:10-07:00, *"The Sharp = the built-in quant engine's daily read (no key needed)"*) deleted all three lines**, leaving the collection UI behind. Twenty days inert | the DISPLAY half is the sharper part: the panel reads *"Passcode for spend-money actions … **this stops strangers from burning your API credits.** Entered once per device."* — an assertion that a protection exists where none does. **SIXTH instance of the computed-and-discarded class** (after `shPenQF`, `shUmpKf`, the Kelly ceiling in legacy modes, `cfSel`'s counterfactual, and M26's positivity-gated warning) **and the only one that claims a protection.** It is also the reason `APP_PASSCODE` cannot simply be switched on: `/api/sharp` L69-70 401s the moment it is set, on every device | M27 only in kind, not in dependency. Blocks §39 step 4 | **SPEC'D, NOT SHIPPED** (owner's instruction, 2026-07-31): one new `src/lib/pass.ts` exporting `passHeader()` plus six one-line spreads at the call sites — no engine string, no hash move, no vintage event **[src: REPO/GIT — `git log -S` on `x-pl-pass` across all branches (two commits, both 2026-07-11), plus a repo-wide grep of `src/` and `app/` returning zero current senders. No board, no fixture, no magnitude.]** |
| **M29** | **the direction-preference feature is HALF-WIRED, and the LIVE half is the engine half** — `legacy/index.html` L2465 reads `var pref=(SH_CFG.dirPref&&SH_CFG.dirPref[mkt])&#124;&#124;"both"` inside the `dscp5` branch (`selMode==="dk_fd"&#124;&#124;selMode==="ev_gated"` — **the server default**) and **OVERRIDES the model-chosen side**; `src/lib/engine-client.ts` L117 pushes `cfg.dirPref = getDirPref()` from `localStorage.pl_dirpref` at boot. But `DIR_MARKETS` (L73) has **ZERO importers** and `setDirPref` has **ZERO callers** across `src/`, `tools/` and `tests/` — **nothing in the app can set the key**, so the only route in is a hand-edited localStorage value | **MEASURED EFFECT ON EVERY ARCHIVED BOARD: EXACTLY ZERO, and that is a structural result, not a small number** — boards are built server-side where there is no `localStorage` and `SH_CFG.dirPref` is `{}` (L1098), so no archived measurement can restate. The exposure is **device-side only**. The sharper half is that `BoardEcho` carries **no `dirPref` field**, so a device that had it set would produce a card whose SIDES differ from the server board **with nothing in the echo to say so** — the same silent-divergence shape as M20. Found by the mirrored-constant sweep, not by looking for it: `DIR_MARKETS` names **5 of the 6** modelled markets, omitting `batter_home_runs` | M28 and M20 in kind (collected-or-declared but never wired; device-reachable divergence the echo cannot see), not in dependency. Blocks nothing | **SWEEP COMPLETE 2026-08-01: M29 IS ISOLATED.** SH_CFG declares **36 keys**, BoardEcho carries **13** of them, and **23 sit outside it** — but only **TWO** SH_CFG keys are written from `localStorage` at all (`selMode`, echoed; `dirPref`, not), and the engine writes **no** `SH_CFG.<key>` anywhere, so there is no second device vector. The other 22 are echo-incompleteness, not exposure — though **`umpKFrozen`/`penQFrozen`/`coreNoHR` being outside the echo means no board testifies to its own brake state** (§12I). **AFTER-THE-FACT TRACE FOUND AND SHIPPED:** the emitted row carries `p`/`imp` (L2494) and the model's arithmetic leaves **`p >= imp` on every row**, so a forced side is the one thing it cannot produce — `sideConsistency()` in `tools/board-report.mjs`, 3 cases in `chain-tools`. **UNEVALUATED: no board archive is on local disk.** **UNFIXED, NOT YET SPEC'D.** Two independent halves: (1) add `dirPref` to `BoardEcho` so a divergence is visible — additive, no engine string; (2) decide whether to WIRE the Settings control or DELETE the dead export — wiring it is what expires the `batter_home_runs` waiver in `tests/mirrored-constants.test.ts` **[src: REPO — engine L1098/L2465, `src/lib/engine-client.ts` L73/L80/L88/L117, and a repo-wide grep of `DIR_MARKETS`/`setDirPref`/`pl_dirpref` returning the definitions and no consumers. No board, no fixture, no magnitude.]** |
| **M25** | **one boolean gates ELEVEN protections, and it is two taps from the device** — `disciplined`/`evGated`/`dscpM`/`selMP` are four spellings of one predicate (`selMode ∈ {ev_gated, dk_fd}` AND `!force`), and behind it sit: `coreEvMin` (the EV gate, L3008) · `coreCzEvMin` (settlement floor, L3011–18) · `booksInd` consensus (L3032–34) · `consMinN`/`consMinEv` (L3037–48) · NO-PLAY as a first-class result (L3056) · the Kelly ceiling (L3112–15) · the disciplined target (L3121–22) · fallback ceiling-breach prevention (L3156) · the legacy-only exact-sum dump (L3158) · the HRR bar's display half (L2513–14) · the HRR bar's pool half (L2651–59) — plus the outs flag, which mirrors the last two BY DESIGN. 13 sites, 11 protections, one flag | **$500 staked against $13 of computed ceiling across the 11 legacy tickets = 38.5×**; negative-czEv ML/RL parlays (−4.5 to −12.5) reach the card because protections 1, 2 and 6 are off together — **legacy modes place negative-EV bets BY CONSTRUCTION**. What stays ON in every mode: the structure caps (`coreNoHR`, `coreMaxLegs`, `coreMaxDec`), the leg/K caps, `perParlayCap`+the 1/n relax, ticket-count bounds, dedup+damping, and the 10% `dailyBankrollCap` lock roof — so the split is clean and the impossible branch (a protection outside the flag that still fails in legacy) is SILENT. `disciplined` is mode **plus `!force`**, so an override in a disciplined mode opens the same eleven holes | M24 restates from a SIZING defect to a DISCIPLINE-BRANCH defect. Two global doc claims corrected and dated (CLAUDE.md L38; collection-period L3163); two engine COMMENTS recorded-not-edited (L1070 "zero edge = zero stake", L2991–92 "stakes only flow after an explicit override") because editing a comment would move the served engine hash. Operator rule #3 is the live mitigation | **NEW 2026-07-30 — the single point of failure for the entire discipline set** **🔴 [src: WAS FIXTURE — RE-MEASURED ON THE ARCHIVE 2026-07-31. ~~$500 vs $13 = 38.5× pooled across 11 legacy tickets~~ came from the ARMED FIXTURE (collection-period L7323-24) and is RETIRED. On the ARCHIVED 2026-07-26 production board (bankroll $750, daily $250, 67-ticket pool, `tests/m25-archive.test.ts`): **`probability` 6 tickets, $250 staked against a $10 computed ceiling = 25.0×, 6 of 6 over ceiling, 5 with a $0 ceiling, 5 negative-czEv**; **`caesars_ev` 6 tickets, $250 vs $265 = 0.9×, 2 over, ZERO negative-czEv, ZERO $0-ceilings**; `ev_gated` $225 vs $225 = **1.00×, zero over**. → THE CLAIM REFINES: "legacy modes place negative-EV bets by construction" is MEASURED TRUE for `probability` and NOT DEMONSTRATED for `caesars_ev` on this board — the fixture pooled both into one number. The 13-sites/11-protections count is a CODE READ and stands unchanged.]** |
| **M24** | **the belief-sized Kelly ceiling is COMPUTED IN EVERY MODE, ATTACHED TO EVERY TICKET, AND APPLIED IN ONLY TWO** — `shAllocate` L3113–14 computes and applies it inside `if(disciplined&&kMult>0)` (L3108: `ev_gated`/`dk_fd`, `!force`), but `shCardCalc` **L3345** calls `shKellyFrac` for EVERY pick unconditionally, stamps `p.kelly` on the ticket and sums it into the displayed `kellyDaily` — in all four modes. The engine knows the belief-sized stake, carries it on the object, shows it, and does not size with it | **11 of 11 legacy tickets exceed their own computed ceiling** (armed fixture, DAILY $250, bankroll $750): `probability` 6/6, `caesars_ev` 5/5; rank-1 `caesars_ev` = **$62 vs a $13 ceiling = 4.77×** on H+R+RBI O1.5 (the 32%-hitting subset). **Nine of the eleven carry a computed ceiling of exactly $0** — `shKellyFrac` floors at 0 for non-positive Kelly edge, i.e. "stake nothing" — and are staked **$2–$62** (the negative-czEv ML/RL parlays). Disciplined modes: **6 of 6 tickets at exactly 1.00×** — the ceiling binds precisely where believed (impossible branch SILENT). The ceiling is computed on the MODEL's probability, so the disciplined modes are sized by a quantity measurably wrong on HRR and the legacy modes discard even that | RECONSTRUCTIBLE ON THE LEDGER without `selMode`: `shKellyFrac` is a pure function of (`prob`, `czDec`), both present since 2026-07-11, and `kellyStakeMult`=4 is frozen — so all 38 placed tickets can be checked (added to reading 15; a ratio > 1 is only reachable in a legacy mode or under `force`, so a realized overstake dates the row's discipline). Operator rule #3 dated the same day: no legacy mode is opened at all until M24 resolves | **NEW 2026-07-30 — fourth computed-and-discarded instance, and the first landing on STAKE rather than price** |
| **M22** | **the ledger's locked-row guard permits three mutable subfields, and one is rewritten IN PLACE with no prior-state retention** — `shLedgerSave` L3304–3315 merges only `grading`/`gradedAt`/`clv` onto a locked snapshot (every money field is immutable, correctly), but `shGradeOrientFix` L3628–3643 overwrites `grading.legs[].detail` on `ml_home`/`rl_home` legs (flipping "3-5" → "5-3") and stamps `v: 2`, keeping no copy of what it replaced | **scope is the finding's limit**: it never assigns `result` — a graded OUTCOME cannot change (impossible branch SILENT) — and it can only touch ML/RL legs, so **the 07-17→07-22 HRR funding population is structurally untouchable by it**; 46.3/59.2 therefore stays PROVENANCE-UNVERIFIED and does NOT become unrecoverable. Prior values are unrecoverable from the store but re-derivable from public boxscores | v-field census added to the export's reading (per-field presence counts by date). Unfixed: the rewrite could retain the prior string at ~zero cost — spec-only | **NEW 2026-07-30 — provenance-retention defect, scoped to a display string, no path to results or money** |
| **M23** | **`shPenQF` is computed and discarded — the archetypal shape, third instance** — the factor is pinned by `SH_CFG.penQFrozen: true` (L1690 returns 1), while its rolling DB `data/pen_quality.json` **has not been written by the bot ONCE**: last commit `4cd1c5d`, 2026-07-20 (the owner's own), because `origin/main`'s `context.yml` **L30** add list is `git add data/ump_k.json` — the DB is rebuilt in the runner and discarded, so accumulation restarts from a ten-day-old base every run | REPLAY (armed fixture, `ev_gated`, frozen false vs true): **16 of 173 rows move, max 15.1 pp, mean 1.44 pp — and the emitted CARD is IDENTICAL** (unlike `shUmpKf`, whose replay changed the card). So: it reaches prices, it does not change what gets bet on this board (n=1). **Nothing reports it as active** — `pred-serialize` L83 says "never applied", the `u`-tag needs `uf !== 1`, the shadow is named shadow (third branch does NOT fire) | pipeline costs **zero Odds credits**. **Spec-only, and load-bearing before any unfreeze**: `shPenQFShadow` returns a NaN for at least one of the 30 teams — inert under the pin, a NaN multiplier in pricing if lifted; guard finite-ness first | **NEW 2026-07-30 — computed-and-discarded confirmed with magnitude; the pin and the uncommitted DB are two independent blockages** |
| **M21** | **the vintage convention cannot stamp a COUNT-ARMED parameter, and one crossed inside the window unstamped** — the convention's class line ("vintage EVENTS are code, config, gate-crossings, cadence") was extended for the DATE-armed `consMinEv` expiries; nothing in it distinguishes a crossing whose fire moment is set by accumulating data, and nothing requires a stamp AT fire time. **`shUmpKf` is count-armed** — `tools/build_context.py` L232 emits a real `kFactor` only at `db.umps[hp].g >= 5` — and on **2026-07-30 the first umpire ever (Lance Barrett, g 4→5) crossed it, FIVE DAYS ahead of the ~08-04 projection**, with no parameter change, no deploy, and no record | magnitude: the crossing reached NO board and NO series restates, because of a DOUBLE BRAKE — the carrier `context.json` is frozen by the pause (last write 2026-07-29 20:32Z) and the factor is pinned by `SH_CFG.umpKFrozen: true`. But it does not start from zero if either brake lifts: 1 umpire armed today, 8 more at g=4. **And the counted quantity's dependence on our own cadence is the general finding**: `shUmpKf` counts real MLB games (cadence-independent), while the consensus gate counts OUR graded rows against `consMinN=100` — **so rationing boards changes when markets reopen, i.e. the ration decision is itself an engine-vintage decision** | ENCODED same day (test-only): `tests/self-arm-stamp.test.ts` pins the armed count and goes RED when accruing data moves it, naming what to record and where — the stamp is written AT fire time by the person who must judge the boundary; it also asserts the double brake is still in place, so a silent release shows up as a failure rather than as a live factor. OBSERVED RED with the pin planted at 0 against the live count of 1 | **NEW 2026-07-30 — convention gap + one unstamped crossing, both recorded; no series restates (double-braked); the encoded rule is the fix, and `umpKFrozen`'s own replay magnitude sits in the frozen table beside it** **[src: ARCHIVE — data/ump_k.json. ⚠️ FIGURES STALE: 2 armed and 11 at g=4 as of 2026-07-31, not 1 and 8; the ~08-04 projection was an ESTIMATE, now superseded by simulation]** |
...
| **M6** | `pitcher_strikeouts` → sim (sixth PA outcome) | K's are **169 rows**, core-eligible, priced with **no simulation of the quantity they are about** | the second-RNG-stream design (below) | **sized, see next section** **[src: ARCHIVE — the 2026-07-26 board row census; the finding itself is a CODE READ (no sim path exists for the quantity), which needs no magnitude]** |
...
| M20 (2026-07-30) | selection-MODE axis — orthogonal to every rate estimator (pricing is identical in all four modes; only pool admission differs) and to the archived-board sweeps (all ev_gated) | **guarded same day, unfixed**: any suspension claim states its mode scope; the 07-26 board family and every ledger card carry `selMode`, so no archived measurement is contaminated — the exposure is forward-looking (a device read in a legacy mode) |
| M21 (2026-07-30) | the STAMPING axis — orthogonal to every measured effect (the crossing reached no board); it governs whether future crossings are recorded at all | **encoded same day, unfixed by design**: the convention still does not name count-armed events in prose — the guard is the rule. Dependency: any series spanning a future crossing must read the stamp, and the ration decision inherits an engine-vintage consequence through `mktN` |
| M22 (2026-07-30) | the LEDGER-PROVENANCE axis — no measured effect depends on it (it cannot reach results or money, and cannot touch the HRR funding population by market) | **unfixed by design**: the three mutable subfields are the guard's intent; only prior-value retention is missing. Dependency: the export's v-field census |
| M24 (2026-07-30) | the SIZING axis — every measured effect on disk was produced in `ev_gated` (where the ceiling binds at 1.00×), so no archived measurement restates; it governs live money in the two ceiling-free modes | **unfixed, operator-mitigated**: rule #3 (no legacy mode opened) plus rule #1 (2% slip cap). Dependency: reading 15's realized-overstake query over the 38 tickets |
| M25 (2026-07-30) | the DISCIPLINE-BRANCH axis — every measured effect on disk was produced inside the branch (`ev_gated`), so nothing archived restates; it governs live money whenever the mode is switched or `force` is used | **unfixed, operator-mitigated** (rule #3). Dependency: reading 15's realized-overstake query is the only way to learn whether the branch was ever left in the placed population |
| M26 (2026-07-31) | the DISPLAY axis — no measured effect depends on it; it governs whether a human notices M24/M25 in the moment | **unfixed, spec'd**: worthless until the legacy modes are safe, which operator rule #3 covers. Dependency: the ratio surface and both positivity gates land together |
| M27 (2026-07-31) | the INSTRUMENT axis — no measured effect depends on it; it governs whether a doc-cited sha is provably reachable. Orthogonal to model, allocation and data vintage: it constrains the RECORD, not the engine | **unfixed, owner's decision**: the widening (`ea7445a`) stands until narrowed or accepted. No dependency on any other M or A item, and none depends on it — it is a memory-integrity guard, so its order is FREE and it can be settled at any point before or after freeze exit |
| M28 (2026-07-31) | the ACCESS axis — no measured effect depends on it; it governs whether the ungated `/api/odds` surface can be closed at all. Orthogonal to model, allocation, data vintage and instrument | **unfixed, spec'd**: the helper is six one-line client edits and blocks nothing else. Dependency: it must land BEFORE `APP_PASSCODE` is set in Vercel, or `/api/sharp` 401s on every device and the Sharp page breaks |
| M29 (2026-08-01) | the DEVICE-DIVERGENCE axis, alongside M20 — no measured effect depends on it (server boards carry `dirPref:{}`, so every archived board is untouched); it governs whether a device can silently flip selected SIDES in the disciplined modes without the echo recording it | **unfixed, not yet spec'd**: the echo field is additive and independent of the wire-or-delete decision. Dependency: none blocking — but if the Settings control is ever wired, the `batter_home_runs` waiver in `tests/mirrored-constants.test.ts` expires in the same commit |
| M23 (2026-07-30) | the FACTOR-ACTIVITY axis, alongside `umpKFrozen` — every measured effect on disk was produced with `shPenQF` returning 1, so nothing downstream restates | **blocked twice over**: the pin AND the uncommitted DB. Dependency: any unfreeze must first guard `shPenQFShadow`'s NaN |
...
|---|---|---|---|
| **A1** | **edge-aware base weight** — `base = max(ev,0)/(dec−1)` instead of `prob` | log-growth **126.6 → 187.2 bp**; **crossover 3.05 → 1.40 pp** | nothing. `2292b85` made `prob` deliberate, so this is a reversal, not a bug fix **[src: ARCHIVE — the 2026-07-26 board via tests/a1-shade.test.ts with PL_BOARD]** |
| **A2** | **leg-equivalent EV floor** — `1.02^n − 1` instead of a fixed +2% | **4 of 18 tickets (22%)** admitted only because the floor does not scale; crossover 3.05 → **3.50 pp**. ⚠️ **A2 SELECTS INTO THE WORST MARKET (measured 2026-07-27, owner's finding)**: on the 07-26 counterfactual the floor alone keeps both HRR-O0.5 tickets (+7.0%, +6.6%) and kills both TB tickets (+3.3%, +2.4%) — it is an EV-quality filter, and HRR's EV is highest precisely BECAUSE it is the most mispriced market. **A2 is not a safety measure on defect grounds and must not be reasoned about as one**; it corrects an over-admission asymmetry, and unaccompanied it concentrates exposure toward defects. Floor and suspension are complements, never substitutes | nothing **[src: ARCHIVE — the 2026-07-26 board (collection-period L4757, singles-vs-parlays L686)]** |
| **A3** | **`consMinEv` multiplicative structure** | the per-leg bar **tightens** with leg count — **−1.000% / −0.501% / −0.334%** at 1/2/3 legs — driven by the **1.071** Caesars overround. A **structure filter wearing a quality filter's name**, running opposite to `coreEvMin` (+2.00 / +1.00 / +0.66%) | nothing. Josh's call: not changed — **but see the requirement below** **[src: CLOSED-FORM ARITHMETIC on the measured 1.071 Caesars overround (collection-period L1182, n=1,336) — no run produced the −1.000/−0.501/−0.334 triple]** |

| **A4** | **stake concentration under `perParlayCap`** | 1/8 vs 1/4 Kelly: **+6.4 bp** at 6 tickets, **+5.1** at 10/15; singles **+0.0 at every size**. Total staked **$250 in every cell**. The mechanism is **concentration, not the Kelly fraction** — the fraction stays at ¼ | A1. With edge-aware weighting the concentration profile changes and this must be re-measured before it is applied **[src: ARCHIVE — the 2026-07-26 board (singles-vs-parlays L465-531), same board and same $250 daily as A1]** |

...
| top-up | max 2, retry 45 min (`NFL_TOPUP`, literals mirroring CFB) | `:115-118` |
...
The lock rail is driven by the EXTERNAL cron-job.org scheduler row: every 15 min during UTC hours **15–23 and 0–2** (`docs/cron-jobs.md`). With a 60-minute lead the window for a kickoff at T is (T − 60 min, T]:

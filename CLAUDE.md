...
- **Built-in model (default, free, no key)** — `shAnalyzeLocal()`: recency-weighted per-AB/per-game rates → Poisson (compound hit-mix distribution for Total Bases), adjusted for opposing-starter WHIP/ERA, lineup spot → expected ABs, Coors, and 10+ mph wind; probabilities shrunk 45% toward the vig-removed market; -300 trap rule; diversity caps (3/game, 5/market-side). Calibration knobs: the 0.55/0.45 shrink and conviction/edge thresholds — tuned 2026-07-07 against a live slate because raw Poisson over-loved TB unders.

...
### The Sharp (build 23) — AI MLB prop board via the Claude API
Fourth bottom tab. The system prompt is **`prompts/mlb_prop_handicapper_prompt.md`** — edit it there and push; the app fetches it at runtime (sw.js serves `/prompts/` network-first so edits are never stale). `shRun()` collects the live slate — statsapi schedule with `hydrate=probablePitcher,weather,lineups,venue`, Odds API h2h+totals plus 5 prop markets per un-started game (~80 quota credits/run), and 7/15/30-day player form filtered to slate players — states unavailable inputs in a `data_gaps` array (umps, xStats, BvP, line movement; never fabricate), and sends it as the user message to `claude-opus-4-8` (adaptive thinking, `output_config.format` json_schema → 15 ranked picks + parlays + trap). **The Claude API key is user-supplied via the ⚙ (localStorage `pl_claudekey`) — never embed one in source; the site is public.** Browser calls require the `anthropic-dangerous-direct-browser-access: true` header. Board persists in `pl_sharp` for the day (~$0.30–0.60 API cost per run, 1–4 min with thinking); picks add to the slip with the model probability pre-filled as EST WIN % so the EV math works. Handle `stop_reason` `refusal`/`max_tokens` before reading content.

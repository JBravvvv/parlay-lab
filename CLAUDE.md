...
| **M14** | **the allocator is NON-MONOTONE IN PRICE — mechanism LOCALIZED TO COMPOSITION (owner's three controls, same day)**: composition-frozen bump sweep is strictly monotone INCREASING (pricing/sizing clean); in-loop it cliffs −40 bp at +1.0→+1.5 pp via floor-admitted high-prob/low-edge newcomers (census: newcomer stake $0→$133, legs 24→16); **negative bumps: "Worse prices produce better cards"** (+117.8 at −1.0 → +124.9 at −1.5); `prob` in the ranking is MODEL probability, not price-implied — no feedback loop. **`coreEvMin=2` is an UNMEASURED chosen value** (`06d3cbc`) and E[ln] is monotone increasing in it (1→+113.0, 4→+139.1). The measured two-book gain +1.07 sits INSIDE the cliff interval | **NEW 2026-07-28, documented defect, NO SHIP (freeze)** — n=1 board, rerun gated on credits. Any fixed-card PRICE-response measurement inherits the failure mode (bundle audit: one family, corrected). A1+A2 are the two halves — evidence joins their exit sign-off. Multi-book adoption downstream of M14 |

...
- **Brand is hot pink (2026-09-18).** Josh: "the whole website should match the new color of the background" — the llama footage plays in its native pink, so `--color-pos` is #ff5fb8, surfaces sit at hue 330 and the brand gradient is violet→magenta→hot pink. The `--color-acc-teal / acc-green / acc-lime` tokens and `text-pos`/`bg-pos` classes keep their NAMES (tests key on them) — a new green anywhere is a regression. −EV red-orange, gold, live ice-blue, CFB amber and NFL blue are unchanged; new UI must not reintroduce green as the default accent.

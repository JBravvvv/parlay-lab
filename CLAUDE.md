...
- **Eight-item UI pass (2026-09-18).** Sport order is NFL / CFB / MLB (`SPORTS`); the brand wordmark links `/games`, never the landing. Bet % / money % come ONLY from `src/lib/splits.ts` + `/api/splits` (public scoresandodds consensus, 10-min cache, HTTP 200 with `games: []` on failure) through `SplitsChip` — never type a split, never show a chip for a side with no match, and do not invent player-prop splits (no public source has them). `useSplits` must stay safe outside a QueryClientProvider (card render tests mount cards bare). Logos draw raw on the dark ground (no disc/ring behind a logo — Josh: "the team logo is not visible with the black circle background"); Stats rows carry headshots. Every `OddsGrid` cell carries its grade badge. The generator is STACKED (filters above, ticket below), slots are numbered 1..n, the pin is the "Lock in" button, and `useParlayGen.reorder` moves a pin with its leg without touching the seed (a drag never re-rolls). `RankedPicks` (S→F, category chips) is the DEFAULT view under the generator on every desk; it grades with `gradeFromEv` only — no second ladder.
- **Generator + hit rates (2026-09-18).** The parlay generator has NO build styles, odds presets or "Your mix", and NO sportsbook toggle — DraftKings prices and grades everything (`SETTLE_BOOK`); do not bring a "Safer/Balanced" or "Favorites/Longshots" control back (Josh: those bands are not consistent across props). Categories are multi-select (`GenSpec.markets`, union pool, spread rule seats every category once), with a hit-rate floor (`minHit`) and a games filter. Hit rates come from the free MLB statsapi game logs (`/api/mlb/hit-rates`, `src/lib/prop-hit-rate.ts`): a count of games already played over the last 7/15/30/60/120 (window in localStorage `pl:props:hit-window`), never a forecast — say so wherever they are shown. Chips are Caesars-tight (28–30px pills, gap-1); the exclude control is a 24px ghost ✕, never a full-width button.
- **Builder speed + half-hour slider + reveal (2026-09-26).** Josh: "Parlay Builder is moving EXTREMELY SLOW … Game start slider … needs to have immediate response … 30 minute increments … start at 9am … remove [Season Lab] from the left side list … Make the parlay generation more interactive like some kind of spinnings wheel, reveal". Rules:
  - Never build an `Intl.DateTimeFormat` per call on a per-leg path; `gameHour` caches the formatter and the hour.
  - `strategyGenerate` passes one `GenMemo` to its runs. Anything added to `prepare()` must be a pure function of (pool, spec). Seed, `avoid` and `recentPlayers` enter only after it.
  - `GameTimeRange` drags a local draft and commits once on native `change` inside `startTransition`. Never commit per `input` tick.
  - `[0,24]` / `ALL_DAY` means no filter. The track floor comes from `slateTimeBounds`: 9am PT, earlier only for an earlier game.
  - The category rail and the Season Lab nav entry are gone; `/season` still loads by URL.
  - The reveal (`ParlayReveal.tsx`) spins only real pool legs, and the odds count-up ends on `amFmt(calc.am)`. Reduced motion skips it.
- **Generator follow-up (2026-09-26).** Josh: "After parlay generator spins and rolls out the picks … It shouldn't change anything after it rolls them out one by one … move 'x' button from right below 'lock' … press on pick in parlay generator and drag it to wherever on list … go up as high as 20 picks". Rules:
  - A generated ticket is held per REQUEST (`holdTicket`). A new pool alone never replaces a firm OK ticket.
  - Pins are NOT in the request: locking or dragging never re-rolls. The rail market is nulled while several markets are on.
  - Anything added to the request must be something Josh changes on purpose.
  - `withBoard` swaps in the pool's copy only when the price is identical (`samePrice`). Never re-price or swap a held leg; `movedLegs` flags it and Add refuses.
  - The reveal lands when `spinKey` changes, never at the press. Football is `frozen` while a Live/Mixed refresh-then-spin is pending.
  - Drag is pointer events via `useSlotDrag`. The iOS touchmove gate must stay registered BEFORE the touch.
  - The phone zoom is measured, never assumed. Controls carry `data-no-drag`.
  - `LEG_MAX` is 20. Keep the Longshot ceiling finite (1e300 beyond 8 legs).
  - A style's shape rule that refuses every draw returns `style-shape` with its `why`, never a short-pool of 0.
- **1H bets on NFL & CFB (2026-09-19).** Josh: "1H bets should be included on NFL & CFB". The three first-half markets (`h2h_h1` / `spreads_h1` / `totals_h1`) ride the per-event props pull on both leagues (`CFB_PROPS_ODDS_MARKETS` and both `feeds.oddsPropMarkets` end with them; no new pull, no budget/reserve change, +3 nominal credits per priced event only when a book returns the half). `src/lib/cfb/markets.ts` is the market vocabulary (`isH1Market`, `baseMarketOf`, `marketWord`, `SIDE_MARKETS`, `H1_SIGMA_SCALE = 1/√2`); `src/lib/cfb/h1.ts` holds `parseH1` (the full game's `readBooksOf` under the half's σ, same Pinnacle-weighted median, `pHome` = ML + spread-implied blend with NO FPI term) and `attachH1` (joins the stored set onto the slate's games by ESPN id, six rows per game through the very `sideRow` that prices full-game sides; idempotent). Stored under `<board prefix><date>:h1` by `propsStore.readH1/writeH1` (written by `football-props.ts` after the board, carried like the stored prop rows); both slate routes read it and pass `{ h1 }` to `slateFromEspn(Of)`. Rows: keys `<game>|spread_1h|home|-3.5`, labels "1H Indiana -3.5" / "1H Over 27.5", `model.h1`. Surfaces: Board categories 1H ML / 1H SPREAD / 1H TOTAL + three parlay sets (13 pregame categories now), the every-pick list, ranked chips, game card + sandbox grid (two "1H" rows only when posted, with the half-time score), slip/ticket wording (`marketWord`), Sharp "1H lines · N games". `legFits` counts the half with the full game (one leg per market per game). Grading: on ESPN's half-time score (`linescores` periods 1 + 2 → `homeH1/awayH1/h1Final` on the game, `finals[id].h1`), settled the moment the half is over even while live; pending with an honest detail until both quarters post; full-game legs byte-identical. The auto paper card stays FULL-GAME (`card.ts` excludes 1H rows) — Josh's call. The splits feed has no 1H sides (chips guarded). Pinned by `tests/cfb-h1.test.ts`.
- **Desktop density pass (2026-09-19).** Josh: "On the web version of builder tab & every other tab, boxes need to be shrunk. the way you put the daily board on a horizontal scroll with no scroll is embarrassing & unacceptable. It all goes vertical & every box shrunk vertically". Measured on production first (DOM-only, no screenshots): the football Builder's ticket `.carousel` held 3,508px of tickets in a 1,003px box with the scrollbar hidden; the football Board's featured carousel and category chip-row, the `/props` game chips, the Games date rail and the Calc chips all overflowed the same way; at 1,100px the MLB Board table itself overflowed its box. Rule now: **from `md` (768px) nothing scrolls sideways** — a `@media (min-width: 768px)` block in `globals.css` wraps `.chip-row` and `.carousel` into rows; `DataTable` has no height cap and no overflow from `md` (`maxHeight` feeds `--dt-max-h` below `md` only), text cells wrap (`md:whitespace-normal`), `fit`/`numeric` cells never do, rows `md:py-1`; `CfbBuilder` TicketStack is a 1/2/3-column grid; the football Board's featured cards fill thirds and its parlay filter strip wraps; `DateRail`, `GenSheet` chip rows and `RankedPicks` tabs wrap. **Every box shrunk vertically on desktop:** Panel `p-4`/`py-2` header, PageHeader `mb-4` + `--text-display` capped at 36px, StatTile 20px figure + 8×12px padding, SportsbookSelector 36px select, generator slots 44px, and the per-tab section gaps (Board/Builder/football Board `space-y-3` + `mt-5` parlays, Ledger 22px figures, Sharp `px-4 py-3`, Games `gap-2`, …). Phones keep the phone-density shape — every rule is a `md:`/`sm:` override or a min-width block. Pinned by `tests/desktop-density.test.ts`; the carousel pins in `cfb-builder-ui`/`cfb-card-ui` are now grid pins. Left as they were: the expanded MLB game boxes (`min-w-max` innings tables) and the Stats/player-sheet `min-w-[…px]` tables, which fit their desktop containers.
- **Ranked-list odds range + price sort pass (2026-09-19).** Josh: "Need to be able to sort 'Every pick today' underneath parlay builder by odds … filter between -200 to +250 for players or whatever other odds I want" / "Prop bets are not loading for CFP games that start in 6 hours 15 minutes." **RankedPicks** (every desk) has an Odds row under the market chips: `AmField` min/max (`parseAmerican`-gated, empty = open bound, junk → `aria-invalid` and no change) and a `RANKED_SORTS` select (grade / shortest / longest). **`inOddsRange` compares American prices numerically — −200 < −150 < +100 < +250 — so a range is two numeric bounds on `am`, and `rangeText` prints "-200 to +250" / "-200 or longer" / "+250 or shorter".** Chip counts are taken over `ranged` (inside the range), the header names range and sort, `sortRanked` tie-breaks price sorts by grade → EV → label. Own-state with optional controlled `range/onRange`, `sort/onSort`; **neither mount passes them — `tests/ranked-picks.test.ts` pins no `onFilter=`/`onRange=` on `CfbProps` and the props page.** **Football props routes** (`app/api/cfb/props`, `app/api/nfl/props`) declare `export const maxDuration = 300;` — they ran on the platform default and a cold 60-event pull at `CONCURRENCY = 4` could be killed mid-pull, which answered nothing while the ranked view showed sides and silently no props; **both props shells now have TWO `export const` lines** (`tests/nfl-props-route.test.ts` and `tests/cfb-props.test.ts` pin the count — the CFB pin was the one red on this pass's first gate) and `tests/football-props-duration.test.ts` pins the ceiling arithmetic against the live constants. The `CfbProps` ranked view shows "Player props did not load — …" + Retry (`propsQ.refetch()`) while `propsQ.isError`. **Spend decisions deliberately left open (Josh's):** a scheduled Tuesday/Wednesday pre-pricing warm-up (≈1,860 CFB + 496 NFL credits/day; on-demand next-date pricing already exists through `useDeskOf` + `slateDates`), the 60-event CFB cap (cut 8 late kicks on 2026-09-19), and the Odds API plan (843/20,000 credits left at 02:47 PDT). Never upgrade the plan or raise a cap for him.
- **Board refresh pass (2026-09-19).** Josh: "The CFB & NFL boards should function the same way as the MLB one does. It should show the time of last board refresh. MLB should also do a FULL refresh every single time i refresh." **Football:** `?refresh=1` WITH the sync phrase in `x-pl-sync` (never the URL) on `/api/cfb`, `/api/nfl` and both props routes is a FULL re-pull — `oddsPayloadOf(cfg, { fresh })` bypasses the 240 s odds cache, `footballPropsGet` skips the fresh-board rail, marks every selected game with rows `refresh` (live ones `live`), fetches every event `no-store`, stores the answer, `refreshed: true`. **The budget rails are never lifted by a refresh; the EMPTY-EVENT RULE is the one carry kept; the flag must short-circuit on the query before `syncAuthed(req)` (the props-route test mocks the store without it).** `loadSlate/loadProps(date, { refresh: true })` send the flag only with a phrase stored. `forceRefreshLeagueBoard(qc, L)` re-pulls every ACTIVE slate + props query; `CfbBoardStamp`/`NflBoardStamp` (`boardStampOf`, active queries only, props `generatedAt` before slate) print "updated h:mm" — desktop appended to the header sentence, phone the whole `subMobile`. **MLB:** the tap is `/api/generate?live=1&force=1` every time; `manualReprice = force && boardOnly` lifts the run cap for the board-only pass alone, is tallied under `pl:gen:manual:<date>` (K_MANUAL) and **must never INCR the shared `runs` key nor SET `K_LASTGEN`** (that would spend the card ladder's headroom and pace the scheduled fires); never write a new `if (boardOnly) {` block (the card-region proof). `live-board-client` adopts the stored board (`adoptServerBoard` + `setQueryData(["board"])`) BEFORE invalidating — `bestBoard()`'s "never fewer priced games" rule would otherwise keep the morning board. `refillRepricedBoard(body)` is the one early return on the page; `regen` is the forced pass's `onFallback` only. **Installed app:** `SwRegister` compares `NEXT_PUBLIC_BUILD_SHA` (next.config `env`) with `/api/version` on open and on every `visibilitychange`/`pageshow` and reloads once per new build — iOS keeps a home-screen page alive for days and the SW is network-first only for navigations. `tests/board-refresh-full.test.ts` pins all of it.
- **Phone density pass — the home-screen app (2026-09-19).** Josh, with eight iPhone screenshots: "The mobile version that is added to home screen is not optimized for iPhone whatsoever … the 4 icons other than settings in top right of header need to be a dropdown … the pick boxes are so unbelievably big. They can be shrunk by 70% vertically … & the info can become expandable … It should be stuck in portrait mode at all times." **Rule: mobile-first Tailwind** — the phone shape is the class default and `sm:` (640px) restores the desktop shape; never change a desktop class to fix a phone, add the phone default and keep the desktop under `sm:`. Phone-only copy or folds are `sm:hidden` / `hidden sm:inline` spans or a state-driven `${open ? "flex" : "hidden"} sm:contents` wrapper (the server render has no duplicate content). The phone header is one ⋯ **More** `role="menu"` popover (`MORE = NAV.filter(!mobile && href !== "/settings")` in AppShell — a new page must go in `NAV`, never be hand-added to the header) plus the Settings gear; the popover is absolute so `useShellInsets`' measured header height does not move. Portrait: `manifest.webmanifest` `orientation: portrait` + a guarded `screen.orientation.lock` in standalone + the CSS-only `.rotate-lock` sheet — **iOS ignores the manifest field and the lock API; the sheet is the iPhone behaviour; do not add a fake lock.** `DataTable` columns take `hideBelowSm`; the Board's market pills are a native `<select>` below 640px fed by the same `marketKeys`; Builder tickets keep name · stake · price · EV · result on the phone row and everything else behind the ▾ drawer; a generator slot is 36px on the phone (`.gen-player-card` phone block in globals.css). `tests/mobile-density.test.ts` pins all of it.
- **One leg per TEAM in the parlay generator — R2b (2026-09-18, late evening).** Josh: "Add filter on parlay generator alongside 'Two legs from one game' that says 'Two legs from one team' so i can prevent a 3 teamer from having 2 players from same team". `GenSpec.onePerTeam` (src/lib/parlay-gen.ts) — at most one leg per folded `team` tag; an untagged leg is never blocked; **undefined = OFF** (saved recipes and pinned fixtures keep their exact tickets and seeds — `specSeed` adds `t1` only when on); both desks default it ON and only the user relaxes it. It is enforced in every place R2 (one per game) is — seating, the price lookahead, the mixed-phase and spread checks (one `blocks()` helper), payout repair, pinned conflicts (`why: "same-team"`), and `capacity`, which is exact under both rules (the club is the seat-owning node; the doubleheader lookahead arms on a club spanning two games). `relaxHint` names `same-game` first whenever the game switch alone or with the team switch would fit, `same-team` only when it is the one binding. The sheet's Toggle is "Two legs from one team", directly under "Two legs from one game" in Advanced — keep them adjacent and keep the same grammar (ON = allowed). Never silently relax either rule.
- **Live legs, the HR line rule, the rail-driven ranked list (2026-09-18, evening).** Josh: "Why won't it generate parlays right now for HRs? There are a ton of HR live props on the board" / "no HR bets shown EVER should be over 1.5 HR unless its a live bet in which the player already has 1 HR live OR it is a manual filter by myself". The board's OWN `live:true` rows are a live-price source: `liveMarketBoard` projects them when statsapi says the game is priceable and the row's settle-book `bookQuotes[*].at` is inside the 30-minute gate, stamping `quoteAt` and nulling `pO` (an in-play row's engine number is the pregame model — never present it as one); the live-props overlay still wins when it has the row, and a started game the board lists as `live:false` never has its pregame prices re-badged live. `hrLineAllowed` is the one HR rule: above 0.5 only live with the homers already in the boxscore, or under the browse-only "Show O1.5 HR" chip — the generator, the ranked list and the Board's ALL scope never see an O1.5 pregame HR. The ranked list follows the market rail (`RankedPicks` controlled `filter`) and is built over "mixed" so it is never empty on a started slate. The generator sheet is compact (selects for Legs/Sides/Timing/hit floor/window, tooltips not paragraphs) — do not bring the tall hero, the chip rows for those controls, or the explanatory paragraphs back.
- **Cerulean on graphite + compact Board + prop lean (2026-09-18, later the same day).** Josh: "change the theme to Cerulean Blue … the neon type of color with the green & pink is what makes it too much to look at with the black background. Maybe we gray the background more than black." `--color-pos` is #3ab0e8, the ground is hsl(216 9% 13%) — a real gray, never near-black — the brand tokens are deep-cerulean→cerulean→sky, −EV is coral #f2664f, live is lavender #b794f6 (ice blue vanished into cerulean); gold, CFB amber and NFL blue are unchanged and pinned. The `--color-acc-teal / acc-green / acc-lime` tokens and `text-pos`/`bg-pos` classes keep their NAMES (tests key on them). The llama mp4 is untouched and recoloured by `hue-rotate(230deg) saturate(0.8)` in `VideoBackdrop` under a 62% scrim — no green, no pink, no neon anywhere new (`tests/theme-cerulean.test.ts`). Board tables: `DataTable` columns take `fit` (shrink to content) and `headerTitle`; every grade/money column is `fit`, and a settled row's explanation sits behind its own "why" toggle (`SettledGrade`), never inline in a nowrap cell. Player props carry a **price-implied market lean** (`src/lib/prop-lean.ts`, `LeanChip`, `GenLeg.lean`): the vig-free share of the settlement book's over/under pair, always labelled "price-implied, not a bet count" — no public source publishes prop bet%/money%, so never present the lean as one and never invent a prop split.
- **Brand was hot pink (2026-09-18, superseded the same day by the cerulean rule above).** The rule that a new green anywhere is a regression still stands.
- **MLB variety card + $350/day (INSTRUCTION 72, 2026-09-17).** `PAPER.daily` is $350 from `VARIETY_SINCE` (2026-09-18) and $150 before; **read it through `paperDaily(date)`** (lock card, refill, `/api/generate`, `deskPaperOf` in the merge kernel, banner) — never `PAPER.daily` bare, or a historic $150 day gets re-bounded at $350. Variety days lock shape V (`VARIETY_SHAPE`: market-typed slots — `hrr`, `team`, untyped, 1-leg straights); the `hrr` slot is the ONLY place an H+R+RBI-over ticket is admitted (rule 5 stands elsewhere); `buildStraightPool` composes 1-leg tickets from priced board rows so straights can be seated. Pre-2026-09-18 days re-fire exactly as before (P / $150) — the record is append-only.
- **Daily ballpark factor (INSTRUCTION 68, 2026-09-17).** `src/lib/mlb/ballpark.ts` is the one park model (30 parks; temp × wind mph × direction × elevation, roof-aware). The legacy blob's `windNote` consults the `shParkDaily` hook only when `SH_CFG.parkDaily` is armed — `applyParkDaily(cfg)` + `bindParkDaily(engine)` in every generator, never one without the other; unarmed = byte-identical (fixtures never arm it). Any edit to the engine string means the engine-ship protocol (extract, `SERVED_ENGINE_SHA_VERIFIED`, `tests/served-verification.json` pending, post-deploy re-grep within 24h). The `/ballpark` tab is MLB-only (`mlbOnly` on the NAV entry).
- **Sorting (INSTRUCTION 69, 2026-09-17).** A Grade column sorts on `gradeSortKey(rank, ev)` — never on `gradeRank` alone (unlettered rows all key to 0 and ties keep insertion order). Every `DataTable` that switches views passes `defaultSort` + `resetKey`, or the previous tab's sort direction leaks into the next.
- **Marks on every pick (INSTRUCTION 70, 2026-09-17).** Every pick label renders through `BoardLabel`: player label → headshot + team badge, team label → club logo via `clubFromLabel`. A new surface that prints a pick as bare text is a regression.
- **Parlay variety (INSTRUCTION 71, 2026-09-17).** `buildParlaySet` honours `SH_CFG.parlayGameCap / parlayMore / parlayCap`, armed 2 / 2 / 5 by `applyParlayVariety(cfg)` in every generator (dormant in fixtures). The Board's My parlay bar (`src/lib/my-parlay.ts`) is page state only — never stored, never ledgered — and prices with `combineTicket` (the engine's prob × dec − 1); it must keep saying that same-game correlation is not modelled.
- **Settlement book = DraftKings (INSTRUCTION 67, 2026-09-17).** `SETTLE_BOOK` in `src/lib/sportsbook/books.ts` is the book every grade, edge %, EV and Kelly stake is computed at, on every desk (Board, Builder, Parlay Generator, lock card, CLV, CFB/NFL/MLB-live `settleBook` constants). The legacy blob still names its variable `CAESARS_KEY`; every `createEngine` caller passes `settlementBook: SETTLE_BOOK` and the facade rebinds it — never call `createEngine` without it. Rows, boards, tickets and CLV legs carry `settlementBook`; anything WITHOUT the stamp is Caesars-priced (pre-2026-09-17) and the client repricers treat it so. The `cz*` field names, `CAESARS_KEY`, and the "Caesars-only" wording in the historical bullets below are names, not the book. Fixtures hand-checked at Caesars are loaded through `tests/helpers/settle-book.ts` (keys exchanged, prices unchanged).
...
**PUSHED AND DEPLOYED (2026-07-29, third post-compaction turn — owner's
authorizations)**: stack + echo landed `1617d1b..9753fb9` (cfSel carrier `0914eeb`
audited: the ONE runtime-touching commit of the six; echo commit `9753fb9`); deploy
CONFIRMED by the named instruments — HTML age reset 0 / NEW etag / PRERENDER, engine
chunk name unchanged, served engine sha256 re-extracted = `f6cf1513…` IDENTICAL (the
push moved the model nowhere). NO zero-credit probe discriminates deployed-with-cfSel
— the first curl response's `echo`+`cfSel` fields are that instrument, as
pre-committed. **BOT PAUSE: commit `a46c1f` built (hashes in its message); the
main-branch push is permission-blocked this session — THE OWNER RUNS:
`git push origin a46c1f:main`** (one command; the 22:30Z-queued context run may
still commit once — the window-start stamp is conditional, collection-period).
**MIN_GAP CLUSTER-BLINDNESS pre-committed before the 08:02Z test** (committed-state
read + no concurrency group + no push-retry → expected two-or-three-paid if runs
overlap; fix = concurrency group + pull-rebase retry, SPEC-ONLY — collection-period).
**SHRINK RESULT (item 3)**: the +60.6 level gap is EDGE-FRAGILE — A1's case restates
...
1. ⚠️ **Deployed-sha grep — CLIENT HALF DONE 2026-07-28, SERVER HALF NOT GREPPABLE**: served
   ...
   **THE SHA INSTRUMENT — ✅ SHIPPED 2026-07-29 (owner's authorization, rode the cfSel
   push `9753fb9`): `src/lib/engine-echo.ts` + route — `data.echo` attached BEFORE the
   board encode (rides KV + archive) and returned in the response body; fields:
   engineSha, priorsSha/ctxSha (content hashes of the fetched artifacts), hrrAltMax,
   coreEvMin, coreCzEvMin, consMinN/consMinEv, coreMaxLegs/maxCoreTickets/coreMaxDec/
   perParlayCap, kellyStakeMult, dailyBankrollCap, selMode, outsSusp, damping
   (EXTRACTED from the live allocator source at module load), cfSelEnabled. WRITE-ONLY
   enforced by `tests/engine-echo.test.ts` (one assignment, zero reads, source scan)
   + runtime-vs-served hash equality (`SERVED_ENGINE_SHA_VERIFIED` updates only beside
   a fresh re-grep) + invalid-by-value plant. The original spec follows for the
   record:** (i) **additive-only**: `engineSha = sha256(LEGACY_SRC)`
...
**It fires on its own** from four git hooks — `post-commit`, `post-merge`, `post-checkout`,
`post-rewrite`. It is change-gated on a fingerprint of HEAD + the dirty-tree listing + every
doc/config mtime, so a no-op sync costs nothing, and it serialises on a `mkdir` lock in `$TMPDIR`

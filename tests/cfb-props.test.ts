...
import { ATD_YES_ONLY_OVERROUND, median, parseEventProps, playerSlug, propLabel, selectPropEvents } from "@/lib/cfb/props";
...
describe("the contract", () => {
  it("six markets, six odds keys, anytime TD the only yes-kind", () => {
    ...
    expect(CFB_PROPS).toEqual({ maxEvents: 12, revalidateSec: 7200, regions: "us", minBooks: 2, settleBook: "williamhill_us", dailyBudget: 1200, measuredCreditsPerEvent: 31 });
  });
...
describe("selectPropEvents", () => {
  ...
  it("caps at max and flags it; excludes kicked-off games and games without a Caesars price", () => {
...
describe("app/api/cfb/props/route.ts + client — source pins", () => {
  ...
  it("caches each event call for CFB_PROPS.revalidateSec (2 h since 2026-09-05; was 30 min) and builds the slate through the shared helper", () => {
    expect(route).toMatch(/revalidate: PROPS_TTL/);
    expect(route).toMatch(/const PROPS_TTL = CFB_PROPS\.revalidateSec/);

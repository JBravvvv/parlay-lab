...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const guarded = restoreShrunkDays(cur?.ledger ?? [], merged0, () => true);
    for (const r of guarded.restored) console.warn(`[mlb-ledger] APPEND ONLY: kept the stored ${r.date} card over the device copy — ${r.violation}`);
